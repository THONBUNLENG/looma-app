import 'package:flutter/material.dart';
import 'package:shopping_app/constants/string_extension.dart';
import 'package:shopping_app/src/network/crud_firebase/migrate_to_firestore.dart';
import 'package:shopping_app/src/widget/text_widget.dart';

import '../../../widget/button.dart';

class SecuritySettingsScreen extends StatefulWidget {
  const SecuritySettingsScreen({super.key});

  @override
  State<SecuritySettingsScreen> createState() => _SecuritySettingsScreenState();
}

class _SecuritySettingsScreenState extends State<SecuritySettingsScreen> {
  bool rememberMe = true;
  bool faceId = false;
  bool biometricId = true;

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final isDark = theme.brightness == Brightness.dark;

    return Scaffold(
      backgroundColor: theme.scaffoldBackgroundColor,
      appBar: AppBar(
        backgroundColor: Colors.transparent,
        elevation: 0,
        centerTitle: false,

        title: Text(
          'Security'.tr,
          style: theme.textTheme.headlineSmall?.copyWith(
            fontWeight: FontWeight.bold,
            color: isDark ? Colors.white : Colors.black,
          ),
        ),
      ),
      body: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20.0),
        child: Column(
          children: [
            const SizedBox(height: 20),

            _buildSwitchTile(context, "Remember me".tr, rememberMe, (val) {
              setState(() => rememberMe = val);
            }),
            _buildSwitchTile(context, "Face ID".tr, faceId, (val) {
              setState(() => faceId = val);
            }),
            _buildSwitchTile(context, "Biometric ID".tr, biometricId, (val) {
              setState(() => biometricId = val);
            }),

            _buildNavigationTile(context, "Google Authenticator".tr, () {}),

            const SizedBox(height: 40),

            MyCustomButton(
              text: 'Change PIN'.tr,
              onPressed: () {},
              width: double.infinity,
              height: 58,
              borderRadius: 30,
            ),
            const SizedBox(height: 16),
            MyCustomButton(
              text: 'Change Password'.tr,
              onPressed: () {},
              width: double.infinity,
              height: 58,
              borderRadius: 30,
            ),
            const SizedBox(height: 40),
            const Divider(),
            const SizedBox(height: 20),
            TextWidget(
              "Developer Tools",
              fontSize: 18,
              fontWeight: FontWeight.bold,
              color: isDark ? Colors.white70 : Colors.black54,
            ),
            const SizedBox(height: 20),
            MyCustomButton(
              text: 'Start Data Migration',
              onPressed: () => FirestoreMigrator().startMigration(context),
              width: double.infinity,
              height: 50,
              borderRadius: 15,
              gradientColors: const [Colors.green, Colors.greenAccent],
            ),
            const SizedBox(height: 16),
            MyCustomButton(
              text: 'Clear All Products',
              onPressed: () => FirestoreMigrator().clearAllProducts(context),
              width: double.infinity,
              height: 50,
              borderRadius: 15,
              gradientColors: const [Colors.orange, Colors.orangeAccent],
            ),
          ],
        ),
      ),
    );
  }

  Widget _buildSwitchTile(BuildContext context,
      String title,
      bool value,
      ValueChanged<bool> onChanged,) {
    final isDark = Theme
        .of(context)
        .brightness == Brightness.dark;

    return ListTile(
      contentPadding: EdgeInsets.zero,
      title: TextWidget(
        title,
        fontSize: 16,
        fontWeight: FontWeight.w500,
        color: isDark ? Colors.white : Colors.black87,
      ),
      trailing: Switch(
        value: value,
        onChanged: onChanged,

        activeTrackColor: isDark ? Theme.of(context).primaryColor : Colors.black87,
        activeThumbColor: Colors.white,
      ),
    );
  }

  Widget _buildNavigationTile(BuildContext context,
      String title,
      VoidCallback onTap,) {
    final isDark = Theme
        .of(context)
        .brightness == Brightness.dark;

    return ListTile(
      contentPadding: EdgeInsets.zero,
      onTap: onTap,
      title: TextWidget(
        title,
        fontSize: 16,
        fontWeight: FontWeight.w500,
        color: isDark ? Colors.white : Colors.black87,
      ),
      trailing: Icon(
        Icons.chevron_right,
        color: isDark ? Colors.white54 : Colors.black,
      ),
    );
  }
}
