const Map<String, String> khmer = {
  'Country and Language': 'ប្រទេស និងភាសា',
  'Personalize your shopping with your location preferences.': 'ធ្វើឱ្យការទិញទំនិញរបស់អ្នកកាន់តែមានលក្ខណៈផ្ទាល់ខ្លួនជាមួយនឹងចំណូលចិត្តទីតាំងរបស់អ្នក។',
  'Select your language preferences.': 'ជ្រើសរើសចំណូលចិត្តភាសារបស់អ្នក។',
  'Continue': 'បន្ត',
  'Select Language': 'ជ្រើសរើសភាសា',
  ' No user found': ' No user found',
  ' | %a sold': ' | %a លក់ដាច់',
  '\$\${(unitPrice * quantity).toStringAsFixed(2)}':
      '\$\${(unitPrice * quantity).toStringAsFixed(2)}',
  '\$\${priceRange.start.round()} - \$\${priceRange.end.round()}':
      '\$\${priceRange.start.round()} - \$\${priceRange.end.round()}',
  '\$sold sold': '\$sold sold',
  '\${"Color".tr} ': '\${"Color".tr} ',
  '\${"Qty".tr} = 1': '\${"Qty".tr} = 1',
  '\${"Size".tr} = 40': '\${"Size".tr} = 40',
  '\${\'Added to Cart\'.tr}: \${widget.product[\'title\'] ?? \'Product Item\'}':
      '\${\'Added to Cart\'.tr}: \${widget.product[\'title\'] ?? \'Product Item\'}',
  '\${_total.toStringAsFixed(2)} USD': '\${_total.toStringAsFixed(2)} USD',
  '\${currentProducts.length} \${\'items\'.tr}':
      '\${currentProducts.length} \${\'items\'.tr}',
  '\${items.length} \${\'popular items\'.tr}':
      '\${items.length} \${\'popular items\'.tr}',
  '\${order[\'items\']} \${\'Item\'.tr}\${order[\'items\'] > 1 ? \'s\'.tr : \'\'}':
      '\${order[\'items\']} \${\'Item\'.tr}\${order[\'items\'] > 1 ? \'s\'.tr : \'\'}',
  '\${product[\'sold\'] ?? \'0\'} sold': '\${product[\'sold\'] ?? \'0\'} sold',
  '\${products.length} items': '\${products.length} items',
  '\${sold.toString()} \${\'sold\'.tr}': '\${sold.toString()} \${\'sold\'.tr}',
  '\${visualSearchResults.length} items':
      '\${visualSearchResults.length} items',
  '\${widget.product[\'rating\'] ?? \'4.8\'} (\${widget.product[\'reviews\'] ?? \'0\'} \${':
      '\${widget.product[\'rating\'] ?? \'4.8\'} (\${widget.product[\'reviews\'] ?? \'0\'} \${',
  '\${widget.product[\'rating\'] ?? \'4.8\'} (\${widget.product[\'reviews\'] ?? \'0\'} \${\'reviews\'.tr})':
      '\${widget.product[\'rating\'] ?? \'4.8\'} (\${widget.product[\'reviews\'] ?? \'0\'} \${\'reviews\'.tr})',
  '\'No results for {0}': '\'No results for {0}',
  '\'No results found for {0}': '\'No results found for {0}',
  'No results found for %a': 'រកមិនឃើញលទ្ធផលសម្រាប់ %a',
  '+855 11 820 595': '+855 11 820 595',
  '-50%': '-50%',
  '0': '0',
  '1. Types of Data We Collect': '១. ប្រភេទនៃទិន្នន័យដែលយើងប្រមូល',
  '18K Gold Emblem Velvet Loafer':
      'ស្បែកជើងដេររូបសញ្ញាមាស 18K សាច់វល្លិ៍ Loafer',
  '18K Gold Zip Compression Jacket': 'អាវរងាខ្សែរ៉ូតមាស 18K Compression Jacket',
  '2. Use of Your Personal Data': '២. ការប្រើប្រាស់ទិន្នន័យផ្ទាល់ខ្លួនរបស់អ្នក',
  '273628           837279': '273628           837279',
  '3. Disclosure of Your Personal Data':
      '៣. ការបង្ហាញទិន្នន័យផ្ទាល់ខ្លួនរបស់អ្នក',
  '30% Special Discount!': 'បញ្ចុះតម្លៃពិសេស ៣០%!',
  '4. Data Retention Policy': '៤. គោលការណ៍រក្សាទុកទិន្នន័យ',
  '5. Your Rights and Choices': '៥. សិទ្ធិ និងជម្រើសរបស់អ្នក',
  '6. Updates to This Policy': '៦. ការធ្វើបច្ចុប្បន្នភាពលើគោលការណ៍នេះ',
  'A bold, single-lens wrap-around design that offers a futuristic look and maximum wind protection for outdoor activities.':
      'ការរចនាកញ្ចក់តែមួយធំទូលាយ បង្ហាញពីភាពទាន់សម័យបែបអនាគត និងការពារខ្យល់បានកម្រិតអតិបរមាសម្រាប់ការធ្វើសកម្មភាពក្រៅផ្ទះ។',
  'A calming desk accessory featuring fine white sand, natural stones, and a miniature rake for relaxation.':
      'ឧបករណ៍លម្អតុធ្វើការដើម្បីភាពស្ងប់ចិត្ត រួមមានខ្សាច់សម៉ដ់ ថ្មធម្មជាតិ និងចបកាស់ខ្នាតតូចសម្រាប់បន្ធូរអារម្មណ៍ធុញថប់។',
  'A classic crew neck sweatshirt crafted from heavy cotton fleece for ultimate comfort and warmth.':
      'អាវរងាកមូលបែប Classic ផលិតពីសាច់ក្រណាត់កប្បាសក្រាស់ ផ្តល់នូវផាសុកភាព និងភាពកក់ក្តៅកម្រិតអតិបរមា។',
  'A curated collection of luxury items wrapped in a beautiful premium gold-accented box.':
      'បណ្តុំទំនិញប្រណីតៗដែលបានជ្រើសរើសយ៉ាងសម្រិតសម្រាំង ខ្ចប់ក្នុងប្រអប់ដ៏ស្រស់ស្អាតលម្អដោយពណ៌មាសកម្រិត Premium។',
  'A high-performance backpack designed for urban commuters and outdoor enthusiasts, featuring a water-resistant shell and a 15-inch laptop sleeve.':
      'កាតាបស្ពាយដែលមានគុណភាពខ្ពស់ ឌីហ្សាញឡើងសម្រាប់អ្នកធ្វើដំណើរក្នុងក្រុង និងអ្នកចូលចិត្តសកម្មភាពក្រៅផ្ទះ មានសមត្ថភាពមិនជ្រាបទឹក និងមានថតដាក់កុំព្យូទ័រយួរដៃទំហំ ១៥ អ៊ីញ។',
  'A high-performance dive watch with a bold aesthetic, combining Swiss craftsmanship with contemporary design.':
      'នាឡិកាមុជទឹកដែលមានគុណភាពខ្ពស់ និងរចនាបថលេចធ្លោ ដោយរួមបញ្ចូលគ្នានូវសិល្បៈច្នៃប្រឌិតស្វីសជាមួយនឹងឌីហ្សាញបែបសហសម័យ។',
  'A luxurious satin evening gown in emerald green, featuring a high-slit and elegant draping.':
      'រ៉ូបរាត្រីសមោសរសាច់សូត្ររលោងប្រណីត ពណ៌បៃតងត្បូងមរកត រចនាម៉ូដឆែកជើងខ្ពស់ និងធ្លាក់ផ្នត់យ៉ាងស្រស់ស្អាត។',
  'A masterpiece of Italian tailoring, blending premium wool and cashmere for ultimate warmth and luxury.':
      'ស្នាដៃកាត់ដេរដ៏ឆ្នើមរបស់អ៊ីតាលី ដោយរួមបញ្ចូលគ្នានូវសាច់រោមសត្វ និងរោមពពែ (Cashmere) កម្រិតប្រណីត ដើម្បីភាពកក់ក្តៅ និងភាពស៊ីវិល័យបំផុត។',
  'A pinnacle of evening elegance, crafted from premium Italian velvet with satin peak lapels.':
      'កំពូលនៃភាពឆើតឆាយពេលរាត្រី កាត់ដេរពីសាច់វល្លិ៍អ៊ីតាលីលំដាប់ខ្ពស់ ជាមួយកអាវឆែកសាច់សូត្ររលោង Satin។',
  'A premium heavyweight cotton hoodie with a relaxed oversized fit, perfect for everyday comfort.':
      'អាវរងាមានមួក (Hoodie) សាច់ក្រណាត់កប្បាសក្រាស់កម្រិតប្រណីត ម៉ូដទំហំធំ (Oversized) ធូរស្រឡះ ល្អឥតខ្ចោះសម្រាប់ផាសុកភាពប្រចាំថ្ងៃ។',
  'A set of three premium crystal clips that catch the light beautifully. Secure grip design for all hair types.':
      'ឈុតដង្កៀបសក់គ្រីស្តាល់ប្រណីតចំនួនបី ដែលចាំងពន្លឺយ៉ាងស្អាត។ ឌីហ្សាញឡើងឱ្យកៀកជាប់មាំល្អសម្រាប់សក់គ្រប់ប្រភេទ។',
  'A sophisticated all-black stainless steel watch with a sapphire crystal face. Designed for the modern executive who values stealth and style.':
      'នាឡិកាដែកមិនច្រេះពណ៌ខ្មៅរលោងដ៏ស៊ីវិល័យ ជាមួយមុខកញ្ចក់ត្បូងកណ្តៀង (Sapphire)។ ឌីហ្សាញសម្រាប់ថ្នាក់ដឹកនាំសម័យថ្មីដែលឱ្យតម្លៃលើភាពថ្លៃថ្នូរ និងស្ទីលលាក់កំបាំង។',
  'A sophisticated evening clutch designed with premium materials to complement your best outfits.':
      'កាបូបយួរដៃរាត្រីសមោសរដ៏ស៊ីវិល័យ ផលិតពីវត្ថុធាតុដើមលំដាប់ខ្ពស់ ដើម្បីបង្កើនសម្រស់សម្លៀកបំពាក់ដ៏ល្អបំផុតរបស់អ្នក។',
  'A stunning 18k white gold engagement ring featuring a brilliant-cut center diamond. A timeless symbol of love and commitment.':
      'ចិញ្ចៀនដណ្តឹងមាសស 18k ដ៏ស្រស់ស្អាត ដាំពេជ្រកាត់យ៉ាងចែងចាំងនៅចំកណ្តាល។ ជាទម្រង់និមិត្តរូបនៃសេចក្តីស្រឡាញ់ និងការប្តេជ្ញាចិត្តមិនចេះប្រែប្រួល។',
  'A stunning glitter clutch perfect for evening parties and weddings. Includes a detachable chain strap and a compact interior for essentials.':
      'កាបូបយួរដៃចែងចាំងយ៉ាងស្រស់ស្អាត ល្អឥតខ្ចោះសម្រាប់ពិធីជប់លៀងពេលល្ងាច និងពិធីមង្គលការ។ រួមបញ្ចូលទាំងខ្សែច្រវាក់ដែលអាចដោះចេញបាន និងមានកន្លែងខាងក្នុងល្មមសម្រាប់របស់របរចាំបាច់។',
  'A stylish and spacious duffel bag crafted from high-quality leather, perfect for weekend getaways.':
      'កាបូបធ្វើដំណើរ (Duffel) ដែលមានស្ទីលទាន់សម័យ និងធំទូលាយ ផលិតពីស្បែកមានគុណភាពខ្ពស់ ល្អឥតខ្ចោះសម្រាប់ការដើរកម្សាន្តចុងសប្តាហ៍។',
  'A timeless design for everyday wear, these white urban sneakers match perfectly with any outfit.':
      'រចនាឡើងយ៉ាងទាក់ទាញមិនចេះហួសសម័យសម្រាប់ការដកពាក់ប្រចាំថ្ងៃ ស្បែកជើងប៉ាតាទីក្រុងពណ៌សនេះ ស័ក្តិសមឥតខ្ចោះជាមួយគ្រប់សម្លៀកបំពាក់។',
  'A timeless leather messenger bag featuring a padded laptop compartment and adjustable shoulder strap for maximum comfort.':
      'កាបូបស្ពាយចំហៀងស្បែកមិនចេះហួសសម័យ មានថតការពារកុំព្យូទ័រយួរដៃ និងខ្សែស្ពាយលៃតម្រូវបាន ដើម្បីផាសុកភាពកម្រិតអតិបរមា។',
  'A timeless Oxford shirt crafted from premium cotton with a clean button-down collar.':
      'អាវសឺមី Oxford មិនចេះហួសសម័យ កាត់ពីសាច់កប្បាសលំដាប់ខ្ពស់ ជាមួយកអាវឡេវប៊ូតុងស្អាតរៀបរយ។',
  'A timeless wool-felt fedora featuring a classic ribbon band. Ideal for formal occasions or adding a sophisticated touch to your casual outfit.':
      'មួកហ្វេដូរ៉ាធ្វើពីក្រណាត់រោមសត្វមិនចេះហួសសម័យ លម្អដោយខ្សែបូរបែបបុរាណ។ ស័ក្តិសមសម្រាប់ឱកាសផ្លូវការ ឬបន្ថែមភាពស៊ីវិល័យដល់សម្លៀកបំពាក់ធម្មតារបស់អ្នក។',
  'A unique piece of home decor, individually signed by the artist.':
      'វត្ថុដេគ័រផ្ទះដ៏ប្លែកប្លែក និងមានតែមួយគត់ មានហត្ថលេខាផ្ទាល់ពីសិល្បករ។',
  'A versatile brown leather belt with a textured finish, ideal for everyday office wear or weekend outings.':
      'ខ្សែក្រវាត់ស្បែកពណ៌ត្នោតដែលអាចប្រើប្រាស់បានច្រើនទម្រង់ ជាមួយផ្ទៃមានឡាយឆ្នូត ល្អបំផុតសម្រាប់ការងារការិយាល័យប្រចាំថ្ងៃ ឬការដើរលេងចុងសប្តាហ៍។',
  'A versatile canvas tote with reinforced stitching and a spacious interior for your laptop and books.':
      'កាបូបយួរធំក្រណាត់កង់វ៉ាស់ដែលអាចប្រើបានច្រើនទម្រង់ មានថ្នេរដេរយ៉ាងមាំ និងមានកន្លែងខាងក្នុងធំទូលាយសម្រាប់ដាក់កុំព្យូទ័រយួរដៃ និងសៀវភៅ។',
  'A versatile charcoal grey suit in a modern slim fit. Perfect for business meetings, weddings, and formal events.':
      'ឈុតគ្របស៊ុតពណ៌ប្រផេះធ្យូងដែលអាចប្រើបានគ្រប់ឱកាស រាងស្លីមហ្វីត (Slim Fit) ទំនើប។ ល្អឥតខ្ចោះសម្រាប់កិច្ចប្រជុំធុរកិច្ច ពិធីមង្គលការ និងកម្មវិធីផ្លូវការនានា។',
  'A versatile polo shirt crafted from breathable cotton pique, featuring a subtle embroidered logo.':
      'អាវប៉ូឡូដែលអាចពាក់បានច្រើនទម្រង់ ផលិតពីសាច់ក្រណាត់កប្បាសខ្យល់ចេញចូលបានល្អ មានប៉ាក់ឡូហ្គោតូចមួយយ៉ាងប្រណីត។',
  'Accessories': 'គ្រឿងលម្អ/គ្រឿងបន្លាស់',
  'Account': 'គណនី',
  'Account Setup Successful!': 'ការរៀបចំគណនីបានជោគជ័យ!',
  'Acid Blue Ripped Jeans': 'ខោខូវប៊យរហែកពណ៌ខៀវអាស៊ីត Acid Blue',
  'Acid Wash Retro Denim': 'ខោខូវប៊យម៉ូដបុរាណលាងពណ៌ Acid Wash',
  'Acid Wash Retro Pullover': 'អាវរងាម៉ូដបុរាណលាងពណ៌ Acid Wash Retro',
  'Acid Wash Street Tee': 'អាវយឺតស្ទីលផ្លូវដើរលាងពណ៌ Acid Wash Street',
  'Active': 'Active',
  'Active Flex Trainer': 'ស្បែកជើងកីឡា Active Flex Trainer',
  'Active Sport Waist Pack': 'កាបូបចង្កេះកីឡា Active Sport',
  'Active Tech Polo': 'អាវប៉ូឡូបច្ចេកវិទ្យាកីឡា Active Tech',
  'Active Trekking Sandals': 'ស្បែកជើងរាត់ដើរភ្នំ Active Trekking',
  'Active-Dry Training Pants': 'ខោកីឡាហ្វឹកហាត់ក្រណាត់ស្ងួតលឿន Active-Dry',
  'Activewear': 'Activewear',
  'Add Address': 'បន្ថែមអាសយដ្ឋាន',
  'Add Card': 'បន្ថែមប័ណ្ណ',
  'Add New Address': 'បន្ថែមអាសយដ្ឋានថ្មី',
  'Add New Card': 'បន្ថែមប័ណ្ណថ្មី',
  'Add to Cart': 'បន្ថែមទៅក្នុងកន្ត្រក',
  'Added to Cart': 'បានបន្ថែមទៅក្នុងកន្ត្រក',
  'Address': 'អាសយដ្ឋាន',
  'Address \${user?.address.toString() ?? ':
      'Address \${user?.address.toString() ?? ',
  'Address Details': 'ព័ត៌មានលម្អិតអាសយដ្ឋាន',
  'Adventure Waterproof Duffel': 'កាបូបធ្វើដំណើរមិនជ្រាបទឹកបែបផ្សងព្រេង',
  'Aero Cushion Max': 'ស្បែកជើងទ្រនាប់ខ្យល់ Aero Cushion Max',
  'Aero Glide Flats': 'ស្បែកជើងក្បាលទាប Aero Glide Flats',
  'Aero Tech Blue': 'ស្បែកជើងកីឡា Aero Tech Blue',
  'Aesthetic Graphic Tote': 'កាបូបយួរធំស្ទីល Aesthetic Graphic',
  'Age :  \${user?.age.toString() ?? ': 'Age :  \${user?.age.toString() ?? ',
  'AHA Exfoliating Peel': 'សេរ៉ូមជម្រុះកោសិកាចាស់ AHA Exfoliating Peel',
  'AI Matches Found': 'AI Matches Found',
  'Air Dynamic Performance Sneaker': 'ស្បែកជើងប៉ាតាស្ទុះខ្លាំង Air Dynamic',
  'All': 'ទាំងអស់',
  'All Accessories': 'គ្រឿងបន្លាស់និងគ្រឿងលម្អទាំងអស់',
  'ALL BRANDS': 'ម៉ាកយីហោទាំងអស់',
  'All CATEGORIES': 'ប្រភេទទាំងអស់',
  'All Discounts': 'ការបញ្ចុះតម្លៃទាំងអស់',
  'All Styles': 'គ្រប់ម៉ូដទាំងអស់',
  'Alpine Hybrid Shell': 'អាវក្រៅការពារធាតុអាកាស Alpine Hybrid',
  'Already have an account?': 'មានគណនីរួចហើយមែនទេ?',
  'technical_problem_contact_aba': 'មានបញ្ហាអាក់រអួល។ សូមព្យាយាមម្តងទៀតនៅពេលក្រោយ ឬទាក់ទងមក ABA ដើម្បីដោះស្រាយបញ្ហានេះឲ្យបានឆាប់រហ័ស។',
  'agree': 'យល់ព្រម',
  'American Flag Splicing Hoodie': 'អាវរងាមានមួកតភ្ជាប់ឡាយទង់ជាតិអាមេរិក',
  'Amethyst Drop Earrings': 'ក្រវិលត្បូងពណ៌ស្វាយ Amethyst Drop',
  'Ametrine Statement Ring': 'ចិញ្ចៀនត្បូងអាមេត្រីនខ្នាតធំ Ametrine',
  'Amount': 'Amount',
  'Animal Print Mesh Top': 'អាវសំណាញ់ឡាយរូបសត្វ Animal Print',
  'Ankle Strap Platform': 'ស្បែកជើងកែងក្រាស់មានខ្សែចងកជើង',
  'Ankle Suede Flat Boots': 'ស្បែកជើងប៊ូតកជើងសំប៉ែតស្បែកសត្វទន់',
  'Anniversary': 'Anniversary',
  'Anniversary Silk Box': 'ប្រអប់សូត្រសម្រាប់ថ្ងៃខួបអនុស្សាវរីយ៍',
  'Anorak Streetwear Jacket': 'អាវក្រៅស្ទីលយុវវ័យ Anorak Streetwear',
  'Anti-Chafe Comfort Short': 'ខោខ្លីទន់ស្រួលពាក់ការពារការកកិត',
  'Anti-Frizz Smoothing Balm': 'គ្រីមបន្ទន់សក់កុំឱ្យរួញជំពាក់ Anti-Frizz',
  'Anti-Ladder Tech Sheer': 'ស្រោមជើងវែងស្តើងបច្ចេកវិទ្យាការពារការរហែក',
  'Anti-Odor Copper Blend': 'ស្រោមជើងក្រណាត់ល្បាយទង់ដែងការពារក្លិនជើង',
  'Anti-Slip Bath Slippers': 'ស្បែកជើងផ្ទាត់ក្នុងបន្ទប់ទឹកការពារការរអិល',
  'Anti-Theft Commuter Bag': 'កាបូបប្រឆាំងការលួចសម្រាប់អ្នកធ្វើដំណើរ',
  'Apartment': 'អាផាតមិន',
  'App Updates': 'ការធ្វើបច្ចុប្បន្នភាពកម្មវិធី',
  'App-Controlled Smart Chess Board': 'ក្តារអុកវៃឆ្លាតបញ្ជាតាមកម្មវិធីទូរស័ព្ទ',
  'Apply': 'អនុវត្ត',
  'APPLY': 'អនុវត្ត',
  'Apricot Cuticle Oil': 'ប្រេងថែរក្សាគល់ក្រចកក្លិនផ្លែអាព្រីកត',
  'Arch Sculpt Brow Gel': 'ជែលម៉ត់ចិញ្ចើម Arch Sculpt',
  'Arctic Fur-Lined Winter Boot': 'ស្បែកជើងប៊ូតរដូវរងាទ្រនាប់រោមសត្វអាកទិក',
  'Arctic White Fleece Joggers': 'ខោកីឡាសាច់ក្រណាត់ទន់ពណ៌សអាកទិក',
  'Arctic White Puffer': 'អាវរងាប៉ោងពណ៌សអាកទិក Arctic White Puffer',
  'Are you sure you want to delete': 'តើអ្នកប្រាកដជាចង់លុបឬទេ',
  'Are you sure you want to log out of your account?':
      'តើអ្នកប្រាកដជាចង់ចាកចេញពីគណនីរបស់អ្នកមែនទេ?',
  'Are you sure you want to remove this address?':
      'តើអ្នកប្រាកដជាចង់លុបអាសយដ្ឋាននេះមែនទេ?',
  'Argan Infusion Shampoo': 'សាប៊ូកក់សក់ផ្សំពីប្រេងអាហ្គាន Argan Infusion',
  'Artisan Ceramic Vase': 'ថូផ្កាដីដុតបែបសិប្បកម្ម Artisan Ceramic',
  'Artisan Chocolate Box': 'ប្រអប់សូកូឡាធ្វើដោយដៃបែបសិប្បកម្ម',
  'Artisan Hand-Stitched': 'កាបូបដេរដោយដៃបែបសិប្បកម្ម Artisan',
  'Artisan Watercolor Paint Kit': 'ឈុតពណ៌ទឹកលំដាប់សិល្បករ Artisan Watercolor',
  'Artistic Floral Canvas': 'កាបូបក្រណាត់កង់វ៉ាស់ឡាយផ្កាបែបសិល្បៈ',
  'Asymmetric Drape Vest': 'អាវកាក់ម៉ូដផ្នត់មិនស្មើគ្នា Asymmetric Drape',
  'Athletic Mesh Shorts': 'ខោខ្លីកីឡាក្រណាត់សំណាញ់ខ្យល់ចេញចូល',
  'Audiophile Gold Edition Headphones':
      'កាសស្តាប់ត្រចៀកកម្រិតសំឡេងច្បាស់ល្អស៊េរីមាស',
  'Automatic Skeleton X': 'នាឡិកាមេកានិចមើលធ្លាយក្នុង Automatic Skeleton X',
  'Aviation Pilot Watch': 'នាឡិកាដៃស្ទីលអ្នកបើកបរយន្តហោះ Aviation Pilot',
  'Aww ..Snap. Your wish list is empty!':
      'អូហូ... បញ្ជីបំណងប្រាថ្នារបស់អ្នកនៅទំនេរទេ!',
  'Baby': 'ទារក',
  'Back to School': 'ត្រៀមចូលរៀនវិញ',
  'Back-Seam Vintage Classic': 'ស្រោមជើងវែងម៉ូដឆ្នូតក្រោយបែបបុរាណ Classic',
  'Back-Smoothing Bralette': 'អាវទ្រនាប់គ្មានដែកជំនួយសាច់ខ្នងរលោង',
  'Backpacks': 'កាតាបស្ពាយក្រោយ',
  'Baggy Fit Hip-Hop Jeans': 'ខោខូវប៊យរាងធំទូលាយស្ទីលហ៊ីបហប Baggy Fit',
  'Baggy Y2K Street Jeans': 'ខោខូវប៊យរាងធំស្ទីលយុវវ័យសម័យ Y2K',
  'Bags': 'កាបូប',
  'Bags & Shoes': 'កាបូប និង ស្បែកជើង',
  'Bakuchiol Retinol Alt': 'សេរ៉ូមជំនួសរ៉េទីណុលសរីរាង្គ Bakuchiol',
  'Bamboo Breathable Soft-cup': 'អាវទ្រនាប់សាច់ឫស្សីទន់ខ្យល់ចេញចូលបានល្អ',
  'Bamboo Charcoal Wash': 'ហ្វូមលាងមុខធ្យូងឫស្សី Bamboo Charcoal',
  'Bamboo Comfort Seamless': 'ខោក្នុងគ្មានថ្នេរក្រណាត់សរសៃឫស្សីទន់ស្រួលពាក់',
  'Bamboo Cooling Chemise': 'រ៉ូបគេងមិនប្រហាយសាច់ក្រណាត់ឫស្សីត្រជាក់',
  'Bamboo Weave Slippers': 'ស្បែកជើងផ្ទាត់ក្នុងផ្ទះត្បាញពីឫស្សី',
  'Bank transfer': 'ផ្ទេរប្រាក់តាមធនាគារ',
  'Basic Navy 2-Pack': 'ឈុតអាវយឺតពណ៌ខៀវក្រម៉ៅមូលដ្ឋាន (កញ្ចប់ ២ អាវ)',
  'Batman: Legacy of the Dark Knight Game':
      'ហ្គេមក្តារ Batman: Legacy of the Dark Knight',
  'Beaded Boho Necklace': 'ខ្សែករ موដអង្កាំបែប Boho',
  'Beaded Hair Band': 'ខ្សែចងសក់ដាំអង្កាំប្រណីត',
  'BEAUTY': 'សម្រស់ (BEAUTY)',
  'Beauty sleep starts here. Reduces hair frizz and skin irritation.':
      'ការគេងដ៏មានសម្រស់ចាប់ផ្តើមពីទីនេះ។ ជួយកាត់បន្ថយការរួញជំពាក់នៃសក់ និងការរំញោចស្បែក។',
  'Beauty Tools': 'Beauty Tools',
  'Bedroom': 'បន្ទប់គេង',
  'Beginner Wooden Acoustic Guitar':
      'ហ្គីតាឈើ Acoustic សម្រាប់អ្នកចាប់ផ្តើមដំបូង',
  'Beige Comfort Heels': 'ស្បែកជើងកែងទន់ស្រួលពាក់ពណ៌បន៍ត្នោតខ្ចី',
  'Beige Sand Desert Hoodie': 'អាវរងាមានមួកពណ៌ខ្សាច់វាលខ្សាច់ Beige Sand',
  'Belts': 'ខ្សែក្រវាត់',
  'Beret French Style': 'មួកបេរ៉េស្ទីលបារាំង Beret French Style',
  'Bergamot & Sea Salt': 'ទឹកអប់ក្លិន Bergamot & Sea Salt',
  'Bermuda Formal Shorts': 'ខោខ្លីផ្លូវការម៉ូដប៊ឺមូដា Bermuda Shorts',
  'Big Sale': 'ការលក់បញ្ចុះតម្លៃធំ',
  'BIG SALE': 'ការលក់បញ្ចុះតម្លៃធំ',
  'Biker Goggles': 'វ៉ែនតាការពារសម្រាប់អ្នកបើកបរម៉ូតូ Biker',
  'Biking Padded Shorts': 'ខោខ្លីជិះកង់មានទ្រនាប់ការពារការឈឺចង្កេះ',
  'Biometric ID': 'Biometric ID',
  'Biotin Growth Elixir': 'សេរ៉ូមជំនួយសក់ដុះលឿន Biotin Growth',
  'Birthday': 'Birthday',
  'Birthday Cake Bundle': 'ឈុតនំខេកខួបកំណើត Birthday Cake',
  'Birthday Cupcake Set': 'ឈុតនំ Cupcake ខួបកំណើត',
  'Birthday Dessert Pack': 'ឈុតបង្អែមខួបកំណើត Birthday Dessert',
  'Birthday Flower Set': 'ឈុតបាច់ផ្កាខួបកំណើត',
  'Birthday Photo Frame': 'ក្របថតរូបខួបកំណើត',
  'Birthday Roses Basket': 'កន្ត្រកផ្កាកុលាបខួបកំណើត',
  'Birthday Sneakers': 'ស្បែកជើងប៉ាតាសម្រាប់កាដូខួបកំណើត',
  'Birthday Teddy Bear': 'តុក្កតាខ្លាឃ្មុំកាដូខួបកំណើត Teddy Bear',
  'Black': 'ពណ៌ខ្មៅ',
  'Black High-Waist Skinny':
      'ខោខូវប៊យរាងរឹបចង្កេះខ្ពស់ពណ៌ខ្មៅ High-Waist Skinny',
  'Black Saffron': 'ទឹកអប់ក្លិន Black Saffron',
  'Black Tassel Loafers':
      'ស្បែកជើងស្បែកមានកញ្ចុំសក់ពណ៌ខ្មៅ Black Tassel Loafers',
  'Blouses': 'Blouses',
  'Blue': 'ពណ៌ខៀវ',
  'Blue Dial Business': 'នាឡិកាធុរកិច្ចមុខខៀវ Blue Dial Business',
  'Blue Suede Block Heels': 'ស្បែកជើងកែងដុំស្បែកសត្វទន់ពណ៌ខៀវ Blue Suede',
  'Bluetooth Headphones': 'កាសស្តាប់ត្រចៀកប៊្លូធូស',
  'Bluetooth Retro Speaker': 'បាសប៊្លូធូសម៉ូដបុរាណ Retro Speaker',
  'Bluetooth Speaker': 'បាសប៊្លូធូស',
  'Body Mist': 'Body Mist',
  'Bodycon Ribbed Mini': 'រ៉ូបខ្លីរឹបរាងសាច់ឆ្នូត Bodycon Ribbed Mini',
  'Bodysuit': 'Bodysuit',
  'Bohemian Rope Sandals': 'ស្បែកជើងរាត់ខ្សែខ្សែត្បាញស្ទីលបូហ៊ីមៀន (Boho)',
  'Boho Embroidered Blouse': 'អាវប៉ាក់ស្ទីលបូហូ Boho Embroidered',
  'Boho Floral Crown': 'មកុដផ្កាស្ទីលបូហូ Boho Floral',
  'Boho Maxi Lace Dress': 'រ៉ូបវែងក្រណាត់ប៉ាក់ស្ទីលបូហូ Boho Maxi',
  'Boho Round Tint': 'វ៉ែនតាមូលលេងពណ៌ស្ទីល Boho',
  'Boho Tassel Scarf': 'កន្សែងបង់កមានព្រុយស្ទីលបូហូ Boho Tassel',
  'Bold Burgundy Fleece': 'អាវរងាសាច់ទន់ពណ៌ក្រហមក្រម៉ៅលេចធ្លោ',
  'Bold Color-Block Polo': 'អាវប៉ូឡូដេញប្លុកពណ៌លេចធ្លោ Bold Color-Block',
  'Bond Repair Serum': 'សេរ៉ូមស្តារសរសៃសក់ខូច Bond Repair',
  'Bonded Edge Sport Brief': 'ខោក្នុងកីឡាគែមអ៊ុតកម្តៅគ្មានថ្នេរ Bonded Edge',
  'Boots': 'Boots',
  'Botanical Facial Oil': 'ប្រេងលាបមុខផ្សំពីឱសថរុក្ខជាតិ Botanical',
  'Botanical Shine Pomade': 'ជែលលាបសក់ឱ្យរលោងផ្សំពីធម្មជាតិ រុក្ខជាតិ',
  'Boys': 'ក្មេងប្រុស',
  'Bra': 'Bra',
  'Braided Casual Belt': 'ខ្សែក្រវាត់ត្បាញធម្មតា Braided Casual Belt',
  'Brands': 'ម៉ាកយីហោ',
  'Breathable linen blend for maximum comfort.':
      'សាច់ក្រណាត់ល្បាយលីនិនខ្យល់ចេញចូលបានល្អ ដើម្បីផាសុកភាពកម្រិតអតិបរមា។',
  'Breathable linen blend shirt, perfect for warm weather and beach outings.':
      'អាវក្រណាត់ល្បាយលីនិនខ្យល់ចេញចូលបានល្អ ល្អឥតខ្ចោះសម្រាប់អាកាសធាតុក្តៅ និងការដើរលេងឆ្នេរខ្សាច់។',
  'Breathable Mesh Babydoll': 'ឈុតគេងសិចស៊ីក្រណាត់សំណាញ់ស្តើងដកដង្ហើមបាន',
  'Breathable Mesh Padded Tech': 'ស្បែកជើងកីឡាក្រណាត់សំណាញ់មានទ្រនាប់ខ្យល់',
  'Breathable Mesh Slip-ons': 'ស្បែកជើងស៊កក្រណាត់សំណាញ់ខ្យល់ចេញចូល',
  'Bridal': 'Bridal',
  'Bridal Lace Veil': 'ស្បៃកូនក្រមុំក្រណាត់ប៉ាក់ Bridal Lace Veil',
  'Brightening Vit C Drop': 'សេរ៉ូមវីតាមីនស៊ីជំនួយស្បែកភ្លឺ Brightening Vit C',
  'British Tan Tweed Suit': 'ឈុតគ្របស៊ុតសាច់ក្រណាត់ធ្វីតពណ៌ត្នោតស្ទីលអង់គ្លេស',
  'Brown': 'ពណ៌ត្នោត',
  'Burgundy Fringe Loafers': 'ស្បែកជើង Loafers មានព្រុយពណ៌ក្រហមក្រម៉ៅ',
  'Burgundy Heritage Polo': 'អាវប៉ូឡូម៉ូដបេតិកភណ្ឌពណ៌ក្រហមក្រម៉ៅ Burgundy',
  'Burgundy Luxe Fleece': 'អាវរងាសាច់ទន់ប្រណីតពណ៌ក្រហមក្រម៉ៅ',
  'Burgundy Velvet Evening Coat': 'អាវធំរាត្រីសមោសរសាច់វល្លិ៍ពណ៌ក្រហមក្រម៉ៅ',
  'Burgundy Velvet Gown': 'រ៉ូបរាត្រីសមោសរសាច់វល្លិ៍ពណ៌ក្រហមក្រម៉ៅ',
  'Burgundy Wine V-Neck': 'អាវកវីពណ៌ក្រហមទំពាំងបាយជូរ Burgundy Wine',
  'Business Carry-On Trolley':
      'វ៉ាលីយួរឡើងយន្តហោះសម្រាប់ធុរកិច្ច Business Carry-On',
  'Business Professional Wallet': 'កាបូបលុយធុរកិច្ចអាជីព Business Professional',
  'Butter-soft fabric with 4-way stretch, perfect for yoga.':
      'សាច់ក្រណាត់ទន់ល្មើយដូចប៊ឺ យឺតបាន ៤ ទិស ល្អឥតខ្ចោះសម្រាប់យូហ្គា។',
  'Butterfly Wing Clips': 'ដង្កៀបសក់រាងស្លាបមេអំបៅ Butterfly Wing',
  'Camo Field Jacket': 'អាវក្រៅឡាយទាហាន Camo Field Jacket',
  'Camo Print Adventure': 'ស្បែកជើងប៉ាតាឡាយទាហានបែបផ្សងព្រេង Camo Print',
  'Cancel': 'បោះបង់',
  'Canvas Flat Sneakers': 'ស្បែកជើងប៉ាតាបាតសំប៉ែតក្រណាត់កង់វ៉ាស់',
  'Canvas Webbed Belt': 'ខ្សែក្រវាត់ក្រណាត់កង់វ៉ាស់បែបកីឡា Canvas Webbed',
  'Canvas Weekend Explorer': 'កាបូបក្រណាត់កង់វ៉ាស់សម្រាប់ដើរលេងចុងសប្តាហ៍',
  'Carbon Fiber Grey Tech':
      'នាឡិកាបច្ចេកវិទ្យាពណ៌ប្រផេះសរសៃកាបូន (Carbon Fiber)',
  'Carbon Fiber RFID Blocker':
      'កាបូបកាតប្រឆាំងការលួចទិន្នន័យ Carbon Fiber RFID',
  'Carbon Fiber Runner': 'ស្បែកជើងរត់ប្រណាំងស៊េរីសរសៃកាបូន Carbon Fiber',
  'Carbon Fiber Slim Wallet': 'កាបូបលុយស្តើងស៊េរីសរសៃកាបូន Carbon Fiber',
  'Carbon Fiber Sport': 'នាឡិកាកីឡាស៊េរីសរសៃកាបូន Carbon Fiber Sport',
  'Carbon Flex Pro': 'ស្បែកជើងកីឡា Carbon Flex Pro',
  'CARD HOLDER': 'ម្ចាស់ប័ណ្ណ',
  'Card Holder': 'ម្ចាស់ប័ណ្ណ',
  'Card Holder Name': 'ឈ្មោះម្ចាស់ប័ណ្ណ',
  'Card Number': 'លេខប័ណ្ណ',
  'Cardigans': 'Cardigans',
  'Care Bears: Caring Cuties Plush':
      'តុក្កតាខ្លាឃ្មុំ Care Bears: Caring Cuties',
  'Cargo Utility Shorts': 'ខោខ្លីហោប៉ៅច្រើន Cargo Utility Shorts',
  'Cargo Utility Skirt': 'សំពត់ហោប៉ៅច្រើនបែបស្ព័រ Cargo Utility Skirt',
  'Carpenter Utility Jeans': 'ខោខូវប៊យហោប៉ៅជាង Carpenter Utility',
  'Case diameter is 40 mm / thickness is 11 mm and strap width is 20 mm.':
      'Case diameter is 40 mm / thickness is 11 mm and strap width is 20 mm.',
  'Cash On Delivery': 'ទូទាត់ប្រាក់ពេលទទួលទំនិញ (COD)',
  'Cashback': 'ប្រាក់ត្រឡប់មកវិញ',
  'Cashmere Winter Overcoat': 'អាវធំរងាសាច់រោមពពែ Cashmere Winter',
  'Cashmere Winter Wrap': 'កន្សែងដណ្តប់រដូវរងាសាច់រោមពពែ Cashmere',
  'Cashmere-Blend Opaque': 'ស្រោមជើងវែងក្រាស់សាច់ល្បាយរោមពពែ Cashmere',
  'Cashmere-Blend Sleep Socks': 'ស្រោមជើងគេងសាច់ល្បាយរោមពពែ Cashmere',
  'Casual': 'ស្ទីល Casual',
  'Casual Beanie Hat': 'មួកចាក់រោមសត្វធម្មតា Casual Beanie',
  'Casual Bomber Baseball Jacket': 'អាវក្រៅ Casual Bomber ស្ទីលបេស្បល',
  'Casual Canvas Loafers': 'ស្បែកជើងស៊កក្រណាត់កង់វ៉ាស់ធម្មតា Casual Canvas',
  'Casual Charcoal Hoodie - Premium':
      'អាវរងាមានមួកពណ៌ប្រផេះធ្យូងធម្មតា - Premium',
  'Casual Charcoal Hoodie - Premium Cotton':
      'អាវរងាមានមួកពណ៌ប្រផេះធ្យូង - កប្បាស Premium',
  'Casual Denim Purse': 'កាបូបដៃក្រណាត់ខូវប៊យ Casual Denim',
  'Casual Denim Sandals': 'ស្បែកជើងរាត់ក្រណាត់ខូវប៊យធម្មតា Casual Denim',
  'Casual Denim Skirt Dress': 'រ៉ូបសំពត់ក្រណាត់ខូវប៊យបែបធម្មតា Casual Denim',
  'Casual Fabric Sport Wallet': 'កាបូបលុយក្រណាត់ស្ទីលកីឡា Casual Sport',
  'Casual Linen Shirt': 'អាវសឺមីក្រណាត់លីនិនបែបធម្មតា Casual Linen',
  'Casual Linen Slippers': 'ស្បែកជើងស៊កក្នុងផ្ទះក្រណាត់លីនិន Casual Linen',
  'Casual Strappy Sandals': 'ស្បែកជើងខ្សែរាត់បែបធម្មតា Casual Strappy',
  'Casual Streetwear Pack': 'កាតាបស្ពាយបែបយុវវ័យ Casual Streetwear',
  'Casual Suede Loafers': 'ស្បែកជើងស៊កស្បែកសត្វទន់បែបធម្មតា Casual Suede',
  'Cat Eye Fashion': 'វ៉ែនតាម៉ូដភ្នែកឆ្មា Cat Eye Fashion',
  'Categories': 'ប្រភេទ',
  'Category': 'ប្រភេទ',
  'Category ': 'Category ',
  'Category "{0}" is coming soon!': 'Category "{0}" is coming soon!',
  'Celestial Highlighter': 'ម្សៅហាយឡាយធ័រ Celestial Highlighter',
  'Celestial Moon Lamp': 'ចង្កៀងគោមរូបព្រះច័ន្ទ Celestial Moon Lamp',
  'Celestial Moon Pendant': 'ខ្សែបន្ដោងរូបព្រះច័ន្ទ Celestial Moon',
  'Ceramic Bonsai Planter': 'ថូដាំដើមបនសាយធ្វើពីសេរ៉ាមិច Ceramic',
  'Ceramic Nail File Set': 'ឈុតដែកឆាបក្រចកសេរ៉ាមិច Ceramic Nail File',
  'Ceramic White Edition': 'នាឡិកាស៊េរីពណ៌សសេរ៉ាមិច Ceramic White',
  'Ceramide Night Balm': 'គ្រីមលាបមុខពេលយប់ផ្សំពីសេរ៉ាម៉ាយ Ceramide',
  'Chain Strap Crossbody Bag': 'កាបូបស្ពាយចំហៀងខ្សែច្រវាក់ Chain Strap',
  'Chain Strap Crossbody Bag - Pink Edition':
      'កាបូបស្ពាយចំហៀងខ្សែច្រវាក់ - ស៊េរីពណ៌ផ្កាឈូក',
  'Chambray Everyday Shirt': 'អាវសឺមីក្រណាត់សំបកបេកប្រចាំថ្ងៃ Chambray',
  'Champagne Gold Metallic Espadrille':
      'ស្បែកជើងផ្ទាត់ពណ៌មាសសាំប៉ាញលោហៈ Metallic',
  'Champagne Satin Slip': 'រ៉ូបគេងសូត្ររលោងពណ៌មាសសាំប៉ាញ Champagne',
  'Champagne Silk Blend Tux': 'ឈុតតុកស៊ីដូសូត្រល្បាយពណ៌មាសសាំប៉ាញ',
  'Change Password': 'ប្តូរពាក្យសម្ងាត់',
  'Change PIN': 'ប្តូរលេខកូដ PIN',
  'Chantilly Lace Brazilian': 'ខោក្នុងក្រណាត់ប៉ាក់ម៉ូដប្រេស៊ីល Chantilly',
  'Charcoal Knit Hoodie': 'អាវរងាមានមួកសាច់ចាក់ពណ៌ប្រផេះធ្យូង Charcoal',
  'Charcoal Knit Sweater': 'អាវរងាសាច់ចាក់ពណ៌ប្រផេះធ្យូង Charcoal Knit',
  'Charcoal Melange Cardigan': 'អាវក្រៅសាច់ចាក់ពណ៌ប្រផេះធ្យូង Charcoal Melange',
  'Charcoal Slim Denim': 'ខោខូវប៊យរាងស្លីមពណ៌ប្រផេះធ្យូង Charcoal Slim',
  'Check out our latest arrivals and stay up to date with our latest styles!':
      'ពិនិត្យមើលទំនិញថ្មីៗបំផុតរបស់យើង និងបន្ដធ្វើបច្ចុប្បន្នភាពជាមួយម៉ូដទាន់សម័យចុងក្រោយបង្អស់!',
  'Checked Flannel Shirt': 'អាវសឺមីក្រណាត់ហ្វ្លានែលឡាយការ៉ូ Checked Flannel',
  'Checkered Collar Polo': 'អាវប៉ូឡូកអាវឡាយការ៉ូ Checkered Collar',
  'Checkered Gingham Blouse': 'អាវនារីឡាយការ៉ូតូច Gingham Blouse',
  'Checkered School Skirt Set': 'ឈុតសំពត់រៀនឡាយការ៉ូ Checkered School',
  'Checkered Tennis Skirt': 'សំពត់កីឡាវាយកូនបាល់ឡាយការ៉ូ Checkered Tennis',
  'Checkered Wool Blend Coat': 'អាវធំសាច់រោមសត្វល្បាយឡាយការ៉ូ Checkered',
  'Checking connection...': 'Checking connection...',
  'Chef Special Spice Rack': 'ធ្នើរដាក់ប្រអប់គ្រឿងទេសពិសេសរបស់មេចុងភៅ',
  'Chic Gray Felt Tote': 'កាបូបយួរធំសាច់ក្រណាត់ហ្វឺតពណ៌ប្រផេះ',
  'Chic Gray Handbag': 'កាបូបយួរដៃពណ៌ប្រផេះបែបស៊ីវិល័យ',
  'Chic Gray Office Bag': 'កាបូបការិយាល័យពណ៌ប្រផេះបែបស៊ីវិល័យ',
  'Chic Office Leather Briefcase': 'កាបូបយួរដៃស្បែកការិយាល័យបែបស៊ីវិល័យ',
  'Chinese': 'ភាសាចិន',
  'Chocolate Box': 'ប្រអប់សូកូឡា Chocolate Box',
  'Chocolate Surprise Box': 'ប្រអប់សូកូឡាផ្ដល់ភាពភ្ញាក់ផ្អើល Surprise Box',
  'Chunky Cable-Knit Boot Sock': 'ស្រោមជើងវែងសាច់ចាក់ធំៗសម្រាប់ពាក់ជាមួយប៊ូត',
  'Classic Aviator': 'វ៉ែនតាម៉ូដ Classic Aviator',
  'Classic Beige Tote': 'កាបូបយួរធំពណ៌បន៍ត្នោតខ្ចីម៉ូដ Classic',
  'Classic Black Leather Clutch': 'កាបូបយួរដៃស្បែកពណ៌ខ្មៅម៉ូដ Classic Clutch',
  'Classic Black Pouch': 'កាបូបដៃតូចពណ៌ខ្មៅម៉ូដ Classic',
  'Classic Black Pumps': 'ស្បែកជើងកែងបិទមុខពណ៌ខ្មៅម៉ូដ Classic Pumps',
  'Classic Brown Leather Duffel':
      'កាបូបធ្វើដំណើរស្បែកពណ៌ត្នោតម៉ូដ Classic Duffel',
  'Classic Brown Leather Loafers': 'ស្បែកជើងស្បែកពណ៌ត្នោតម៉ូដ Classic Loafers',
  'Classic Brown Leather Wallet': 'កាបូបលុយស្បែកពណ៌ត្នោតម៉ូដ Classic',
  'Classic Brown Pouch': 'កាបូបដៃតូចពណ៌ត្នោតម៉ូដ Classic',
  'Classic Brown Waistband': 'ខ្សែក្រវាត់ពណ៌ត្នោតម៉ូដ Classic Brown',
  'Classic Button-Down PJ Top': 'អាវគេងឡេវមុខម៉ូដ Classic PJ Top',
  'Classic Charcoal 3-Piece': 'ឈុតគ្របស៊ុត ៣ ដុំ ពណ៌ប្រផេះធ្យូង Classic',
  'Classic Charcoal Pajamas': 'ឈុតខោអាវគេងពណ៌ប្រផេះធ្យូងម៉ូដ Classic',
  'Classic Charcoal Suit': 'ឈុតគ្របស៊ុតពណ៌ប្រផេះធ្យូងម៉ូដ Classic Suit',
  'Classic Coin Purse Edition': 'កាបូបដាក់កាក់ម៉ូដ Classic Edition',
  'Classic Comfort Bootcut': 'ខោខូវប៊យជើងប៉ាត់ទន់ស្រួលពាក់ម៉ូដ Classic',
  'Classic Cowboy Hat': 'មួកគង្វាលគោម៉ូដបុរាណ Classic Cowboy',
  'Classic Denim Shirt': 'អាវសឺមីក្រណាត់ខូវប៊យម៉ូដ Classic Denim',
  'Classic Denim Shorts': 'ខោខ្លីក្រណាត់ខូវប៊យម៉ូដ Classic Denim Shorts',
  'Classic denim skirt with a high-waisted fit and slightly frayed hem for a modern look.':
      'សំពត់ក្រណាត់ខូវប៊យម៉ូដ Classic រាងចង្កេះខ្ពស់ និងមានគែមព្រុយបន្ដិច បង្ហាញពីស្ទីលទាន់សម័យ។',
  'Classic Essential White Sneaker': 'ស្បែកជើងប៉ាតាពណ៌សចាំបាច់ម៉ូដ Classic',
  'Classic Evening Clutch Bag': 'កាបូបយួរដៃរាត្រីសមោសរម៉ូដ Classic Clutch',
  'Classic Harrington Jacket': 'អាវធំ Harrington ម៉ូដ Classic Harrington',
  'Classic Hoodie': 'អាវរងាមានមួកម៉ូដ Classic Hoodie',
  'Classic Ivory Button-Up': 'អាវសឺមីឡេវមុខពណ៌សភ្លុកដំរីម៉ូដ Classic',
  'Classic leather driving gloves designed for maximum grip and comfort. Made from genuine lambskin for a soft, premium feel.':
      'ស្រោមដៃស្បែកបើកបរម៉ូដ Classic ឌីហ្សាញឡើងដើម្បីការក្តាប់ជាប់ល្អបំផុត និងផាសុកភាព។ ផលិតពីស្បែកកូនចៀមពិតប្រាកដ ផ្តល់អារម្មណ៍ទន់ល្មើយកម្រិតប្រណីត។',
  'Classic Leather Messenger': 'កាបូបស្ពាយចំហៀងស្បែកម៉ូដ Classic',
  'Classic Leather Quartz':
      'នាឡិកាខ្សែស្បែកប្រព័ន្ធក្វាត្សម៉ូដ Classic Leather',
  'Classic Leather Slides': 'ស្បែកជើងស៊កស្បែកម៉ូដ Classic Leather Slides',
  'Classic Mocha Plaid': 'អាវសឺមីឡាយការ៉ូពណ៌ម៉ូកាម៉ូដ Classic Mocha',
  'Classic Olive Green Parka': 'អាវរងាជួរវែងពណ៌បៃតងអូលីវម៉ូដ Classic Parka',
  'Classic Panama Hat': 'មួកប៉ាណាម៉ាម៉ូដបុរាណ Classic Panama',
  'Classic Patent Red Heels': 'ស្បែកជើងកែងស្បែករលោងពណ៌ក្រហមម៉ូដ Classic',
  'Classic Plaid Scarf': 'កន្សែងបង់កឡាយការ៉ូម៉ូដ Classic Plaid',
  'Classic Pointed Toes Flat': 'ស្បែកជើងបាតសំប៉ែតក្បាលស្រួចម៉ូដ Classic',
  'Classic Red Court': 'ស្បែកជើងកែងពណ៌ក្រហមម៉ូដ Classic Red Court',
  'Classic Red Ribbon Gift': 'ប្រអប់កាដូបូពណ៌ក្រហមម៉ូដ Classic Red Ribbon',
  'Classic Rubber Flip-Flops': 'ស្បែកជើងផ្ទាត់កៅស៊ូម៉ូដ Classic Rubber',
  'Classic School Backpack': 'កាតាបរៀនម៉ូដបុរាណ Classic',
  'Classic Silver Chronograph': 'នាឡិកាដៃកាត់ម៉ោងពណ៌ប្រាក់ម៉ូដ Classic',
  'Classic Sneakers': 'ស្បែកជើងប៉ាតាម៉ូដ Classic Sneakers',
  'Classic stonewashed denim with a comfortable straight-leg fit.':
      'ខោខូវប៊យម៉ូដបុរាណលាងទឹកថ្ម (Stonewashed) ជើងត្រង់ស្រួលពាក់។',
  'Classic Striped Polo': 'អាវប៉ូឡូឡាយឆ្នូតម៉ូដ Classic Striped',
  'Classic Tan H-Strap Sandals':
      'ស្បែកជើងរាត់ខ្សែរាងអក្សរ H ពណ៌ត្នោតខ្ចី Classic',
  'Classic Tennis Bracelet': 'កងដៃពេជ្រជួរម៉ូដ Classic Tennis',
  'Classic Watch': 'នាឡិកាដៃម៉ូដ Classic Watch',
  'Classic Wayfarer Black': 'វ៉ែនតាពណ៌ខ្មៅម៉ូដ Classic Wayfarer',
  'Classic White Button-Up': 'អាវសឺមីឡេវមុខពណ៌សមូដ Classic White',
  'Classic White Carryall': 'កាបូបដាក់ឥវ៉ាន់ធំពណ៌សមូដ Classic',
  'Classic White Crew': 'អាវយឺតកមូលពណ៌សមូដ Classic White Crew',
  'Classic White Urban': 'ស្បែកជើងប៉ាតាទីក្រុងពណ៌សមូដ Classic White',
  'Classic Zip-Up Hoodie': 'អាវរងាមានមួកខ្សែរ៉ូតម៉ូដ Classic Zip-Up',
  'Clear': 'សម្អាត',
  'Clear all': 'Clear all',
  'Clothes': 'សម្លៀកបំពាក់',
  'Clothing': 'សម្លៀកបំពាក់',
  'Cloud Glide 360': 'ស្បែកជើងទន់ស្រាល Cloud Glide 360',
  'Cloud-Soft Jogger Pants': 'ខោកីឡាសាច់ទន់ខ្លាំង Cloud-Soft Jogger',
  'Clubmaster Classic': 'វ៉ែនតាម៉ូដ Clubmaster Classic',
  'Clutches': 'កាបូបយួរដៃតូច (Clutches)',
  'Coats': 'Coats',
  'Collectible Fantasy Card Deck':
      'ឈុតបៀកាតហ្វង់តាស៊ីសម្រាប់ប្រមូលទុក Collectible',
  'Color': 'ពណ៌',
  'Color-Block Modern': 'អាវយឺតដេញប្លុកពណ៌ទំនើប Color-Block',
  'Combat Style Flat Sole': 'ស្បែកជើងប៊ូតបាតសំប៉ែតស្ទីលប្រយុទ្ធ Combat',
  'Comfort Cork Sole Slides': 'ស្បែកជើងស៊កបាតឈើឆ្នុក Comfort Cork',
  'Comfort Walking Loafers': 'ស្បែកជើងស៊កដើរស្រួល Comfort Walking',
  'Comfortable high-waisted denim shorts with a classic five-pocket design and raw hem.':
      'ខោខ្លីក្រណាត់ខូវប៊យចង្កេះខ្ពស់ពាក់ស្រួល ជាមួយឌីហ្សាញហោប៉ៅប្រាំបែបបុរាណ និងគែមព្រុយធម្មជាតិ។',
  'Compact Daypack': 'កាតាបស្ពាយខ្នាតបង្រួមប្រចាំថ្ងៃ',
  'Compact Overnight Bag': 'កាបូបស្នាក់នៅមួយយប់ខ្នាតបង្រួម',
  'Compact Tabletop Pool Table': 'តុប៊ីយែរខ្នាតតូចដាក់លើតុ Compact Tabletop',
  'Compact Travel Pouch': 'កាបូបដៃតូចសម្រាប់ធ្វើដំណើរខ្នាតបង្រួម',
  'Completed': 'Completed',
  'Compression Energizing 40D': 'ស្រោមជើងវែងជំនួយចរន្តឈាម Compression 40D',
  'Compression Sport Leggings': 'ខោរឹបកីឡា Compression Sport Leggings',
  'Compression Travel Tech': 'ស្រោមជើងបច្ចេកវិទ្យាធ្វើដំណើរ Compression Tech',
  'Concrete Orchid': 'ទឹកអប់ក្លិន Concrete Orchid',
  'Confirm': 'Confirm',
  'Confirm Payment': 'បញ្ជាក់ការទូទាត់',
  'Connetix Magnetic Tiles: Light Star Pack':
      'កូនជញ្ជាំងមេដែក Connetix Magnetic Tiles',
  'Contact us': 'ទាក់ទងមកយើង',
  'Contour Sculpt Stick': 'ដំបងបន្លិចរាងមុខ Contour Sculpt Stick',
  'Contrast Collar Polo': 'អាវប៉ូឡូកអាវពណ៌ផ្ទុយ Contrast Collar',
  'Cool-Dry Golf Edition': 'អាវកីឡាវាយកូនហ្គោលក្រណាត់ត្រជាក់ស្ងួតលឿន',
  'Corduroy Button Skirt': 'សំពត់ឡេវមុខក្រណាត់កូឌុយរ៉យ (Corduroy)',
  'Corduroy Lined Field Coat':
      'អាវធំទ្រនាប់ក្រណាត់កូឌុយរ៉យ Corduroy Field Coat',
  'Corduroy Retro Shorts': 'ខោខ្លីក្រណាត់កូឌុយរ៉យស្ទីលរ៉េត្រូ Corduroy Retro',
  'Corduroy Sherpa Jacket': 'អាវក្រៅក្រណាត់កូឌុយរ៉យទ្រនាប់រោមចៀម Sherpa',
  'Corduroy Texture Tote': 'កាបូបយួរធំក្រណាត់កូឌុយរ៉យ (Corduroy)',
  'Corduroy Utility Shirt': 'អាវសឺមីហោប៉ៅច្រើនក្រណាត់កូឌុយរ៉យ Utility',
  'Core Balance Trainer': 'ឧបករណ៍ហ្វឹកហាត់លំនឹង Core Balance Trainer',
  'Core Sculpting Bodysuit': 'ឈុតរឹបរាងឱ្យស្អាត Core Sculpting Bodysuit',
  'Corporate Executive Gift': 'ឈុតកាដូសម្រាប់ថ្នាក់ដឹកនាំក្រុមហ៊ុន Corporate',
  'Cosmetic Glam Pack': 'ឈុតគ្រឿងសម្អាងកែសម្ផស្ស Cosmetic Glam Pack',
  'Cosmetics': 'គ្រឿងសម្អាង',
  'Cotton Bandana Set': 'ឈុតកន្សែងរុំក្បាលកប្បាស Cotton Bandana',
  'Cotton Cargo Shorts': 'ខោខ្លីហោប៉ៅថង់ក្រណាត់កប្បាស Cotton Cargo',
  'Cotton Chino Shorts': 'ខោខ្លីឈីណូក្រណាត់កប្បាស Cotton Chino',
  'Cotton Stretch Hipster Pack': 'កញ្ចប់ខោក្នុងយឺតសាច់កប្បាសម៉ូដ Hipster',
  'Cotton-Stretch Daily Pack':
      'កញ្ចប់ខោក្នុងយឺតសាច់កប្បាសប្រចាំថ្ងៃ Daily Pack',
  'Court King Basketball Shoe': 'ស្បែកជើងកីឡាបាល់បោះ Court King',
  'Court Pro Flat Shoes': 'ស្បែកជើងបាតសំប៉ែតកីឡាវាយកូនបាល់ Court Pro',
  'Crafted with a 14k gold nib for an exceptionally smooth writing experience.':
      'ផលិតឡើងជាមួយក្បាលប៊ិចមាស 14k ដើម្បីបទពិសោធន៍សរសេរដ៏រលូនអស្ចារ្យ។',
  'Cream': 'ពណ៌គ្រីម',
  'Cream Cashmere Sleeveless': 'អាវវាលក្លៀកសាច់រោមពពែពណ៌គ្រីម Cashmere',
  'Cream Fleece Basic': 'អាវរងាសាច់ទន់ពណ៌គ្រីមមូលដ្ឋាន Cream Fleece',
  'Create Account': 'បង្កើតគណនី',
  'Create\nAccount': 'Create\nAccount',
  'Credit Card Connected!': 'បានភ្ជាប់កាតឥណទានរួចរាល់!',
  'Credit Card has been linked!': 'កាតឥណទានត្រូវបានភ្ជាប់រួចរាល់!',
  'Crimson Luxury Tote': 'កាបូបយួរធំប្រណីតពណ៌ក្រហមឆ្អៅ Crimson',
  'Crimson Red Active Hoodie': 'អាវរងាមានមួកកីឡាពណ៌ក្រហមឆ្អៅ Crimson Red',
  'Crimson Red Slim Jogger': 'ខោកីឡារាងរឹបជើងពណ៌ក្រហមឆ្អៅ Crimson Red',
  'Criss-Cross Strap Heels': 'ស្បែកជើងកែងខ្សែរាត់ខ្វែង Criss-Cross Strap',
  'Cropped Button Sweater': 'អាវរងាចាក់ខ្នាតខ្លីឡេវមុខ Cropped Sweater',
  'Cross-Train Elite': 'ស្បែកជើងកីឡាហ្វឹកហាត់ Cross-Train Elite',
  'Crystal Bow Evening': 'កាបូបរាត្រីសមោសរបូគ្រីស្តាល់',
  'Crystal Detail Evening Bra': 'អាវទ្រនាប់រាត្រីសមោសរលម្អគ្រីស្តាល់ Crystal',
  'Crystal Detail Evening G-String':
      'ខោក្នុងសិចស៊ីរាត្រីសមោសរលម្អគ្រីស្តាល់ G-String',
  'Crystal Hair Clip Set': 'ឈុតដង្កៀបសក់គ្រីស្តាល់ Crystal Hair Clip',
  'Crystal Studded Pumps': 'ស្បែកជើងកែងបិទមុខដាំគ្រីស្តាល់ Crystal Studded',
  'Crystal Zen Garden Set': 'ឈុតសួនគ្រីស្តាល់ហ្សេន (Zen) សម្រាប់បន្ធូរអារម្មណ៍',
  'Curl Defining Soufflé': 'គ្រីមលាបសក់រួញឱ្យចេញរលកស្អាត Curl Defining',
  'Customer Service': 'សេវាកម្មអតិថិជន',
  'Cute Bunny Ear Slippers': 'ស្បែកជើងស៊កក្នុងផ្ទះរូបត្រចៀកទន្សាយគួរឱ្យស្រឡាញ់',
  'Cute Plush Collection': 'បណ្តុំតុក្កតាសាច់ទន់គួរឱ្យស្រឡាញ់ Cute Plush',
  'Cute Teddy Bear': 'តុក្កតាខ្លាឃ្មុំគួរឱ្យស្រឡាញ់ Cute Teddy Bear',
  'CVV': 'លេខកូដ CVV',
  'Cyberpunk Style LED Glasses':
      'វ៉ែនតាមានភ្លើង LED ស្ទីលស៊ីប៊ឺផាំង (Cyberpunk)',
  'Cycling Gel Padded': 'ខោជិះកង់មានទ្រនាប់ជែលការពារការកកិត',
  'Daily Comfort Walker': 'ស្បែកជើងដើរប្រចាំថ្ងៃស្រួលពាក់ Daily Comfort',
  'Daily Grind Backpack': 'កាតាបស្ពាយប្រចាំថ្ងៃ Daily Grind',
  'Daily UV Shield SPF50':
      'គ្រីមការពារកម្តៅថ្ងៃប្រចាំថ្ងៃ Daily UV Shield SPF50',
  'Dark Grey Loafers': 'ស្បែកជើងស៊កពណ៌ប្រផេះដិត Dark Grey Loafers',
  'Dark Mocha Formal Polo': 'អាវប៉ូឡូផ្លូវការពណ៌ម៉ូកាដិត Dark Mocha',
  'Dark Mocha Leather': 'កាបូបស្បែកពណ៌ម៉ូកាដិត Dark Mocha',
  'Dark Mocha Pocket Knit': 'អាវចាក់មានហោប៉ៅពណ៌ម៉ូកាដិត Dark Mocha',
  'Dark Mode': 'មុខងារងងឹត',
  'Date': 'កាលបរិច្ឆេទ',
  'Date of Birth': 'ថ្ងៃខែឆ្នាំកំណើត',
  'December 22, 2026': 'ថ្ងៃទី ២២ ខែធ្នូ ឆ្នាំ ២០២៦',
  'Decor': 'ការតុបតែង',
  'Decorative Hair Pins': 'ស្នៀតសក់លម្អសក់ឱ្យស្អាត Decorative Hair Pins',
  'Deep Burgundy Tailored Fit': 'ឈុតគ្របស៊ុតកាត់រាងស្អាតពណ៌ក្រហមឈាមជ្រូកដិត',
  'Deep Forest Cargo Joggers': 'ខោកីឡាហោប៉ៅថង់ពណ៌បៃតងព្រៃជ្រៅ Deep Forest',
  'Deep Forest Wool Gown': 'រ៉ូបរាត្រីសមោសរសាច់រោមសត្វពណ៌បៃតងព្រៃជ្រៅ',
  'Deep Olive Chino Slim': 'ខោឈីណូរាងស្លីមពណ៌បៃតងអូលីវដិត Deep Olive',
  'Deep Sea Blue Pique': 'អាវប៉ូឡូសាច់ក្រណាត់ពីកេពណ៌ខៀវសមុទ្រជ្រៅ',
  'Deep V-Neck Chiffon Blouse': 'អាវនារីក្រណាត់ស៊ីហ្វុងកឆែក V ជ្រៅ Deep V-Neck',
  'Deep-V Plunge Microfiber': 'អាវទ្រនាប់កឆែក V ជ្រៅសាច់ម៉ៃក្រូហ្វាយបឺ',
  'Default': 'អាសយដ្ឋានលំនាំដើម',
  'Delete': 'លុប',
  'Delete Address': 'លុបអាសយដ្ឋាន',
  'Delete Address?': 'លុបអាសយដ្ឋាន?',
  'deleted': 'បានលុប',
  'Delivery': 'ការដឹកជញ្ជូន',
  'Delivery Fee': 'ថ្លៃដឹកជញ្ជូន',
  'Delivery Methods': 'វិធីសាស្ត្រដឹកជញ្ជូន',
  'Denim Architect Vest': 'អាវកាក់ក្រណាត់ខូវប៊យស្ទីលស្ថាបត្យករ Denim',
  'Denim Overall Dress': 'រ៉ូបខោអៀមកាត់ពីក្រណាត់ខូវប៊យ Denim Overall',
  'Denim Soft-Wash Blouse': 'អាវនារីក្រណាត់ខូវប៊យសាច់ទន់ Denim Blouse',
  'Denim Style Slippers': 'ស្បែកជើងស៊កក្នុងផ្ទះស្ទីលក្រណាត់ខូវប៊យ',
  'Denim Vest Streetwear': 'អាវកាក់ខូវប៊យស្ទីលយុវវ័យ Denim Vest',
  'Deodorants': 'Deodorants',
  'Description': 'ការពិពណ៌នា',
  'Desert Cedar': 'ទឹកអប់ក្លិន Desert Cedar',
  'Desert Sand Slim Chinos': 'ខោឈីណូរាងស្លីមពណ៌ខ្សាច់វាលខ្សាច់ Desert Sand',
  'Desert Tactical Combat Boot': 'ស្បែកជើងប៊ូតប្រយុទ្ធបែបយោធាវាលខ្សាច់',
  'Designed for performance, these runners provide excellent grip and stability for all terrains.':
      'រចនាឡើងសម្រាប់ប្រសិទ្ធភាពខ្ពស់ ស្បែកជើងរត់ទាំងនេះផ្តល់នូវការខាំជាប់ដីយ៉ាងល្អ និងលំនឹងខ្ពស់សម្រាប់គ្រប់ស្ថានភាពផ្លូវ។',
  'Designer Bowler Hat': 'មួកមូល Bowler រចនាពិសេស Designer',
  'Designer Chandelier Earrings': 'ក្រវិលយារម៉ូដរចនាពិសេស Chandelier',
  'Designer Embossed Belt': 'ខ្សែក្រវាត់ស្បែកឆ្លាក់ឡាយរចនាពិសេស Designer',
  'Designer Gradient': 'វ៉ែនតាកញ្ចក់ដេញពណ៌ Designer Gradient',
  'Designer Street Pack': 'កាតាបស្ពាយចំហៀងរចនាបថផ្លូវដើរ Designer',
  'Designer Wallet': 'កាបូបលុយរចនាពិសេស Designer Wallet',
  'Designer Wool Scarf': 'កន្សែងបង់កសាច់រោមសត្វរចនាពិសេស Designer',
  'Diamond Bezel Edition': 'នាឡិកាដៃស៊េរីកងមុខដាំពេជ្រ Diamond Bezel',
  'Diamond Engagement Ring': 'ចិញ្ចៀនពេជ្រដណ្តឹង Diamond Engagement',
  'Diamond Lattice Pattern': 'ស្រោមជើងវែងឡាយក្រឡាពេជ្រ Diamond Lattice',
  'Diamond Studded Bag': 'កាបូបដាំពេជ្រប្រណីត',
  'Discover your favorite fashion pieces.\nShop effortlessly and enjoy a new online clothing experience.':
      'Discover your favorite fashion pieces.\nShop effortlessly and enjoy a new online clothing experience.',
  'Distressed Denim Joggers': 'ខោកីឡាក្រណាត់ខូវប៊យម៉ូដហែក Distressed Denim',
  'Distressed Jean Shorts': 'ខោខ្លីខូវប៊យម៉ូដហែកព្រុយ Distressed Jean',
  'Distressed Knee Rip Jeans': 'ខោខូវប៊យរហែកជង្គង់ឡាយស្ទាយ Distressed Knee',
  'Distressed Retro Leather': 'កាបូបស្បែកម៉ូដបុរាណ Distressed Retro',
  'Distressed Tan Leather': 'ស្បែកជើងស្បែកពណ៌ត្នោតខ្ចីម៉ូដបុរាណ Distressed',
  'DIY Hydraulic Robot Arm': 'ដៃរ៉ូបូតប្រព័ន្ធអ៊ីដ្រូលីកធ្វើដោយខ្លួនឯង (DIY)',
  'Done': 'Done',
  'Double Ring Belt': 'ខ្សែក្រវាត់ក្បាលកងទ្វេ Double Ring Belt',
  'Double-Breasted Classic': 'អាវធំឡេវពីរជួរម៉ូដបុរាណ Double-Breasted',
  'Double-Breasted Navy Elite': 'ឈុតគ្របស៊ុតឡេវពីរជួរពណ៌ខៀវនាវិកលំដាប់ Elite',
  'Double-Breasted Navy Pea Coat': 'អាវរងា Pea Coat ឡេវពីរជួរពណ៌ខៀវនាវិក',
  'Double-Breasted Overcoat': 'អាវធំរងាជួរវែងឡេវពីរជួរ Double-Breasted',
  'Double-Layer Firm Bodice': 'អាវកាក់រឹបរាងក្រណាត់ពីរជាន់ Double-Layer',
  'Double-Layer Smoothing': 'ខោក្នុងរឹបរាងសាច់រលោងពីរជាន់ Double-Layer',
  'Double-Lined Sculpting Brief': 'ខោក្នុងរឹបរាងមានទ្រនាប់ពីរជាន់ Double-Lined',
  'Download E-Receipt': 'ទាញយកបង្កាន់ដៃ',
  'Dresses': 'Dresses',
  'Durable and lightweight, this hard-shell suitcase features 360-degree spinner wheels for effortless travel.':
      'ធន់ និងទម្ងន់ស្រាល វ៉ាលីសម្បករឹងនេះមានកង់វិលបាន ៣៦០ ដឺក្រេ ធ្វើឱ្យការធ្វើដំណើររបស់អ្នកកាន់តែងាយស្រួលដោយមិនបាច់ប្រឹង។',
  'Dynamic Sport Runner': 'ស្បែកជើងប៉ាតាកីឡាស្ទុះខ្លាំង Dynamic Runner',
  'D’Orsay Suede Pumps': 'ស្បែកជើងកែងបិទមុខចំហចំហៀងស្បែកសត្វទន់ D’Orsay',
  'E-Receipt': 'បង្កាន់ដៃអេឡិចត្រូនិក',
  'e.g. Home': 'ឧទាហរណ៍៖ ទំព័រដើម',
  'Earth Tone Brown': 'អាវយឺតពណ៌ត្នោតស្រទន់បែបធម្មជាតិ Earth Tone',
  'Earth Tone Relaxed Jogger': 'ខោកីឡារាងធូរស្រឡះពណ៌ស្រទន់បែបធម្មជាតិ',
  'Earth Tone Relaxed Polo': 'អាវប៉ូឡូរាងធូរស្រឡះពណ៌ស្រទន់បែបធម្មជាតិ',
  'Eco-Friendly Bamboo Building Blocks':
      'ដុំឈើឫស្សីក្មេងលេងមិត្តភាពបរិស្ថាន Eco-Friendly',
  'Eco-Friendly Cotton Tote': 'កាបូបយួរធំក្រណាត់កប្បាសបរិស្ថាន Eco-Friendly',
  'Eco-Friendly Wine Carrier': 'កាបូបដាក់ដបស្រាឡេវបរិស្ថាន Eco-Friendly',
  'Eco-Run Sustainable': 'ស្បែកជើងរត់ប្រណាំងបែបនិរន្តរភាព Eco-Run',
  'Edit': 'កែប្រែ',
  'Edit Address': 'Edit Address',
  'Edit Profile': 'Edit Profile',
  'Egyptian Cotton Crew Pack': 'កញ្ចប់អាវយឺតកមូលកប្បាសអេស៊ីប Egyptian Cotton',
  'Elastic Sport Bands': 'ខ្សែយឺតហាត់ប្រាណកីឡា Elastic Sport Bands',
  'Electric Blue Y2K Hoodie': 'អាវរងាមានមួកពណ៌ខៀវភ្លឺស្ទីល Y2K Electric Blue',
  'Electric Heated Gloves': 'ស្រោមដៃផ្ដល់កម្តៅដោយប្រព័ន្ធអគ្គិសនី Electric',
  'Electric Neon Track Joggers': 'ខោកីឡាពណ៌ណេអុងភ្លឺ Electric Neon Track',
  'Electric Pottery Wheel Kit for Kids':
      'ម៉ាស៊ីនពូនដីដុតអគ្គិសនីខ្នាតតូចសម្រាប់កុមារ',
  'Electric Purple Party Polo':
      'អាវប៉ូឡូពណ៌ស្វាយភ្លឺសម្រាប់ជប់លៀង Electric Purple',
  'Elegant 14k yellow gold chain necklace, perfect for daily wear or layering with other pieces. Durable and stylish.':
      'ខ្សែករមាសលឿង 14k ដ៏ប្រណីត ល្អឥតខ្ចោះសម្រាប់ការដកពាក់ប្រចាំថ្ងៃ ឬពាក់លេងម៉ូដឡាយជាមួយគ្រឿងផ្សេងទៀត។ ធន់ និងទាន់សម័យ។',
  'Elegant and high-quality leather satchel with multiple compartments for organized storage. Perfect for formal and casual outings.':
      'កាបូបយួរដៃស្បែកមានគុណភាពខ្ពស់ និងប្រណីត មានថតជាច្រើនងាយស្រួលរៀបចំរបស់របរ។ ល្អឥតខ្ចោះសម្រាប់ការដើរលេងផ្លូវការ និងធម្មតា។',
  'Elegant Ballet Flats': 'ស្បែកជើងបាតសំប៉ែតស្ទីលបាឡេដ៏ប្រណីត Elegant',
  'Elegant Bracelet': 'កងដៃដ៏ប្រណីត Elegant Bracelet',
  'Elegant Crocodile Print': 'កាបូបលុយព្រីនឡាយស្បែកក្រពើដ៏ប្រណីត',
  'Elegant Evening Clutch': 'កាបូបយួរដៃរាត្រីសមោសរដ៏ប្រណីត',
  'Elegant Gold Tea Infuser': 'ឧបករណ៍ត្រងតែពណ៌មាសដ៏ប្រណីត Elegant Gold',
  'Elegant headband adorned with high-quality pearls. Perfect for weddings, parties, or adding a touch of class to your daily look.':
      'ខ្សែចងក្បាលដ៏ប្រណីតលម្អដោយគជ់ខ្យងគុណភាពខ្ពស់។ ល្អឥតខ្ចោះសម្រាប់ពិធីមង្គលការ ជប់លៀង ឬបង្កើនភាពទាក់ទាញដល់ស្ទីលប្រចាំថ្ងៃរបស់អ្នក។',
  'Elegant Lace Clutch': 'កាបូបយួរដៃក្រណាត់ប៉ាក់ប្រណីត Elegant Lace',
  'Elegant ladies watch featuring a mother-of-pearl dial and a rose gold plated mesh strap. A perfect accessory for evening events.':
      'នាឡិកាដៃនារីដ៏ប្រណីតមានមុខងាវងក់ខ្យង និងខ្សែសំណាញ់ស្រោបពណ៌មាសផ្កាឈូក។ ជាគ្រឿងលម្អដ៏ល្អឥតខ្ចោះសម្រាប់កម្មវិធីពេលល្ងាច។',
  'Elegant Leather Satchel': 'កាបូបយួរដៃស្បែកប្រណីត Elegant Leather',
  'Elegant midnight navy tuxedo with satin lapels. The ultimate choice for black-tie occasions.':
      'ឈុតតុកស៊ីដូពណ៌ខៀវក្រម៉ៅ Midnight Navy ដ៏ប្រណីត ជាមួយកអាវសូត្ររលោង Satin។ ជាជម្រើសដ៏ល្អបំផុតសម្រាប់កម្មវិធីលំដាប់ខ្ពស់។',
  'Elegant Midnight Shawl': 'កន្សែងដណ្តប់រាត្រីដ៏ប្រណីត Elegant Midnight',
  'Elegant Necklace': 'ខ្សែករដ៏ប្រណីត Elegant Necklace',
  'Elegant Pearl Flat Sandals': 'ស្បែកជើងរាត់បាតសំប៉ែតលម្អគជ់ខ្យងដ៏ប្រណីត',
  'Elegant Pearl Jewelry Box': 'ប្រអប់គ្រឿងអលង្ការលម្អគជ់ខ្យងដ៏ប្រណីត',
  'Elegant pleated midi skirt with an elastic waistband. Lightweight and perfect for both office and casual outings.':
      'សំពត់បត់ផ្នត់ប្រណីតខ្នាតកណ្តាលមានចង្កេះយឺត។ ទម្ងន់ស្រាល និងល្អឥតខ្ចោះសម្រាប់ទាំងការិយាល័យ និងការដើរលេងធម្មតា។',
  'Elegant satin finish with a smooth touch. This evening bag adds a sophisticated look to any formal gown.':
      'ផ្ទៃសាច់សូត្ររលោងប្រណីត និងទន់ល្មើយ។ កាបូបរាត្រីសមោសរនេះជួយបង្កើនភាពស៊ីវិល័យដល់រ៉ូបរាត្រីសមោសរគ្រប់ប្រភេទ។',
  'Elegant silk-satin blouse with a clean V-neckline, perfect for professional office wear.':
      'អាវនារីសាច់សូត្ររលោងដ៏ប្រណីតជាមួយកអាវរាងអក្សរ V ស្អាតស្អំ ល្អឥតខ្ចោះសម្រាប់សម្លៀកបំពាក់ការិយាល័យអាជីព។',
  'Elegant Wedge Sandals': 'ស្បែកជើងរាត់បាតក្រាស់ (Wedge) ដ៏ប្រណីត',
  'Elegant Wide Brim': 'មួកគែមធំទូលាយដ៏ប្រណីត Elegant Wide Brim',
  'Elegant Women': 'ទឹកអប់សម្រាប់នារីស៊ីវិល័យ Elegant Women',
  'Elite Business Briefcase': 'កាបូបយួរដៃធុរកិច្ចលំដាប់ Elite',
  'Elite Metallic Silver Spinner': 'វ៉ាលីកង់វិលពណ៌ប្រាក់លោហៈលំដាប់ Elite',
  'Email': 'អ៊ីមែល',
  'Embossed Leather Loafers': 'ស្បែកជើងស៊កស្បែកឆ្លាក់ឡាយក្រឡា Embossed Leather',
  'Embroidered Ethnic Bag': 'កាបូបប៉ាក់បែបប្រពៃណី Ethnic',
  'Embroidered Shawl': 'កន្សែងដណ្តប់ស្មាមានប៉ាក់ឡាយ Embroidered Shawl',
  'Embroidered Tulle Slip': 'រ៉ូបគេងក្រណាត់សំណាញ់ស្តើងមានប៉ាក់ឡាយ',
  'Emerald Green Ribbed Wrap': 'កន្សែងរុំសាច់ឆ្នូតពណ៌បៃតងត្បូងមរកត Emerald',
  'Emerald Green Shades': 'វ៉ែនតាកញ្ចក់ពណ៌បៃតងត្បូងមរកត',
  'Emerald Green Stilettos': 'ស្បែកជើងកែងស្រួចខ្លាំងពណ៌បៃតងត្បូងមរកត',
  'Emerald Stud Earrings': 'ក្រវិលត្បូងមរកត Emerald Stud',
  'Emotional Support Highland Cow Plush':
      'តុក្កតាគោ Highland សាច់ទន់ជួយបន្ធូរអារម្មណ៍',
  'English (US)': 'English (US)',
  'Engraved Locket': 'ខ្សែបន្ដោងរូបថតមានឆ្លាក់ឡាយ Engraved Locket',
  'Enter Promo Code': 'បញ្ចូលកូដប្រូម៉ូសិន',
  'Please enter a promo code': 'សូមបញ្ចូលកូដប្រូម៉ូសិន',
  'Promo code applied:': 'បានអនុវត្តកូដប្រូម៉ូសិន៖',
  'Invalid or expired promo code': 'កូដប្រូម៉ូសិនមិនត្រឹមត្រូវ ឬផុតកំណត់',
  'Enter your phone number': 'Enter your phone number',
  'Enter your promo code at the checkout page under the \'Discount\' section.':
      'Enter your promo code at the checkout page under the \'Discount\' section.',
  'Envelope Style Evening Clutch':
      'កាបូបយួរដៃរាត្រីសមោសរម៉ូដស្រោមសំបុត្រ Envelope',
  'Error: \${snapshot.error}': 'Error: \${snapshot.error}',
  'Espresso Sharkskin Suit': 'ឈុតគ្របស៊ុតសាច់ក្រណាត់ឡាយស្បែកឆ្លាមពណ៌កាហ្វេ',
  'Essential Black Joggers': 'ខោកីឡាពណ៌ខ្មៅចាំបាច់ Essential Black Joggers',
  'Essential Core Vest': 'អាវកាក់ស្នូលចាំបាច់ Essential Core Vest',
  'Essential Daily Sneaker': 'ស្បែកជើងប៉ាតាប្រចាំថ្ងៃចាំបាច់ Essential Daily',
  'Essential Grey Crew': 'អាវយឺតកមូលពណ៌ប្រផេះចាំបាច់ Essential Grey',
  'Essential Grey Pullover': 'អាវរងាកមូលពណ៌ប្រផេះចាំបាច់ Essential Grey',
  'Essential Heather Grey': 'អាវយឺតពណ៌ប្រផេះដេញស្រទន់ Essential Heather Grey',
  'Essential Hoodies': 'Essential Hoodies',
  'Essential Seamless Bralette':
      'អាវទ្រនាប់គ្មានថ្នេរចាំបាច់ Essential Seamless',
  'Essential white crew neck t-shirt made from 100% premium cotton. Soft, breathable, and durable for everyday wear.':
      'អាវយឺតកមូលពណ៌សចាំបាច់ ផលិតពីកប្បាស Premium ១០០%។ ទន់ ខ្យល់ចេញចូលបានល្អ និងធន់សម្រាប់ការប្រើប្រាស់ប្រចាំថ្ងៃ។',
  'Essential White Polo': 'អាវប៉ូឡូពណ៌សចាំបាច់ Essential White Polo',
  'Everyday T-Shirt Contour':
      'អាវទ្រនាប់រឹបរាងសម្រាប់ពាក់ជាមួយអាវយឺតប្រចាំថ្ងៃ',
  'Evira is a premium shopping platform that provides high-quality products with fast delivery and secure order options.':
      'Evira គឺជាវេទិកាទិញទំនិញលំដាប់ខ្ពស់ ដែលផ្តល់ជូននូវផលិតផលដែលមានគុណភាពខ្ពស់ ជាមួយការដឹកជញ្ជូនរហ័ស និងជម្រើសការបញ្ជាទិញដែលមានសុវត្ថិភាព។',
  'Executive Business Pack': 'កាតាបធុរកិច្ចថ្នាក់ដឹកនាំ Executive',
  'Executive Carbon Fiber': 'វ៉ែនតាស៊ុម Carbon Fiber ថ្នាក់ដឹកនាំ Executive',
  'Executive Charcoal Polo': 'អាវប៉ូឡូថ្នាក់ដឹកនាំពណ៌ប្រផេះធ្យូង Executive',
  'Executive Emerald Suit': 'ឈុតគ្របស៊ុតថ្នាក់ដឹកនាំពណ៌បៃតងត្បូងមរកត',
  'Executive Fountain Pen': 'ប៊ិចទឹកលំដាប់ថ្នាក់ដឹកនាំ Executive Fountain Pen',
  'Executive Leather Brief': 'កាបូបយួរដៃស្បែកធុរកិច្ច Executive',
  'Executive Leather Case': 'កាបូបស្បែកធុរកិច្ចថ្នាក់ដឹកនាំ Executive',
  'Executive Pinstripe Slacks':
      'ខោសាច់ក្រណាត់ឡាយឆ្នូតតូចថ្នាក់ដឹកនាំ Executive',
  'Experience ultimate comfort with the Nike Air Max Pro, featuring advanced cushioning and breathable mesh.':
      'បទពិសោធន៍ផាសុកភាពកម្រិតអតិបរមាជាមួយ Nike Air Max Pro មានទ្រនាប់ជើងទំនើប និងក្រណាត់សំណាញ់ខ្យល់ចេញចូលបានល្អ។',
  'Expiry Date': 'កាលបរិច្ឆេទផុតកំណត់',
  'Eyelet Lace Sleep Shorts': 'ខោខ្លីគេងក្រណាត់ប៉ាក់មានរន្ធឡាយ Eyelet Lace',
  'Face ID': 'Face ID',
  'Facebook': 'ហ្វេសប៊ុក',
  'Family Size Extra Large Luggage':
      'វ៉ាលីខ្នាតធំពិសេសសម្រាប់គ្រួសារ Family Size XL',
  'FAQ': 'សំណួរដែលសួរញឹកញាប់',
  'Fashion Handbag': 'កាបូបយួរដៃម៉ូដទាន់សម័យ Fashion Handbag',
  'Fashionista Pink Pouch': 'កាបូបដៃតូចពណ៌ផ្កាឈូក Fashionista',
  'Fast Delivery': 'Fast Delivery',
  'Faux Leather Long Trench': 'អាវធំជួរវែងសាច់ស្បែកសិប្បនិម្មិត Faux Leather',
  'Female': 'ស្រី',
  'Festive Hamper Basket': 'កន្ត្រកកាដូសម្រាប់ឱកាសបុណ្យទាន Festive Hamper',
  'FIFA World Cup 2026 Official Emblem Set':
      'ឈុតរូបសញ្ញាផ្លូវការនៃព្រឹត្តិការណ៍ FIFA World Cup 2026',
  'filter': 'តម្រង',
  'Filter': 'Filter',
  'Fitness Smart Band': 'ខ្សែដៃវៃឆ្លាតតាមដានសុខភាព Fitness Smart Band',
  'Flash Sale': 'Flash Sale',
  'FLASH SALE': 'ការលក់បញ្ចុះតម្លៃបន្ទាន់ (FLASH SALE)',
  'Flats': 'Flats',
  'Fleece Running Gloves': 'ស្រោមដៃរត់ប្រណាំងសាច់ទន់កក់ក្តៅ Fleece Running',
  'Fleece Zip-Up Jacket': 'អាវក្រៅខ្សែរ៉ូតសាច់ទន់កក់ក្តៅ Fleece Jacket',
  'Flexi-Sole Running Flats': 'ស្បែកជើងរត់បាតទន់យឺត Flexi-Sole',
  'Floral A-Line Skirt': 'សំពត់រាងអក្សរ A ឡាយផ្កា Floral A-Line Skirt',
  'Floral Arrangement Box': 'ប្រអប់ផ្កាកែច្នៃយ៉ាងស្រស់ស្អាត Floral Box',
  'Floral Embroidery Cardigan': 'អាវក្រៅចាក់មានប៉ាក់ឡាយផ្កា Cardigan',
  'Floral Embroidery Cheeky': 'ខោក្នុងនារីមានប៉ាក់ឡាយផ្កាម៉ូដ Cheeky',
  'Floral Embroidery Demi': 'អាវទ្រនាប់មានប៉ាក់ឡាយផ្កាម៉ូដ Demi Bra',
  'Floral Embroidery Knit': 'អាវចាក់មានប៉ាក់ឡាយផ្កា Floral Embroidery Knit',
  'Floral Lace Pattern Tights': 'ស្រោមជើងខោវែងក្រណាត់ប៉ាក់ឡាយផ្កា',
  'Floral Lace Shaping Brief': 'ខោក្នុងរឹបរាងក្រណាត់ប៉ាក់ឡាយផ្កា',
  'Floral Print Handbag': 'កាបូបយួរដៃព្រីនឡាយផ្កា',
  'Floral Print Lounge Set': 'ឈុតខោអាវសម្រាកនៅផ្ទះព្រីនឡាយផ្កា Floral',
  'Floral Print Pumps': 'ស្បែកជើងកែងបិទមុខព្រីនឡាយផ្កា Floral Print',
  'Floral Print Summer Sandals':
      'ស្បែកជើងរាត់រដូវក្តៅព្រីនឡាយផ្កា Floral Print',
  'Floral Puff Sleeve Top': 'អាវនារីដៃប៉ោងឡាយផ្កា Floral Puff Sleeve',
  'Floral Spring Wrap': 'កន្សែងដណ្តប់រដូវផ្ការីកឡាយផ្កា Floral Spring',
  'Floral Summer Hat': 'មួកដើរលេងរដូវក្តៅឡាយផ្កា Floral Summer',
  'Floral Summer Sundress': 'រ៉ូបវាលកម្តៅថ្ងៃរដូវក្តៅឡាយផ្កា Floral Summer',
  'Floral Vacation Shorts': 'ខោខ្លីដើរលេងសម្រាកកាយឡាយផ្កា Floral Vacation',
  'Flower Collection': 'បណ្តុំបាច់ផ្កាស្រស់ Flower Collection',
  'Forest Camo Outdoor Polo': 'អាវប៉ូឡូក្រៅផ្ទះឡាយទាហានព្រៃឈើ Forest Camo',
  'Forest Green Heavy': 'អាវរងាសាច់ក្រាស់ពណ៌បៃតងព្រៃឈើ Forest Green',
  'Forest Green Organic': 'អាវយឺតកប្បាសសរីរាង្គពណ៌បៃតងព្រៃឈើ Forest Green',
  'Forest Green Outdoor': 'ខោកីឡាក្រៅផ្ទះពណ៌បៃតងព្រៃឈើ Forest Green',
  'Formal Black Leather': 'ស្បែកជើងស្បែកផ្លូវការពណ៌ខ្មៅ Formal Black',
  'Formal Patent Loafers': 'ស្បែកជើងស៊កស្បែករលោងផ្លូវការ Formal Patent',
  'Formal White Dress Shirt': 'អាវសឺមីសផ្លូវការ Formal White Dress Shirt',
  'Fragrances': 'Fragrances',
  'FREE DELIVERY\nON ORDERS\nOVER \$160+':
      'FREE DELIVERY\nON ORDERS\nOVER \$160+',
  'Free size': 'ទំហំទូទៅ (Free size)',
  'Fringe Pashmina': 'កន្សែងបង់កមានព្រុយសាច់ម៉ត់ Fringe Pashmina',
  'From 1 - 3 days': 'From 1 - 3 days',
  'Front-Close Lace Racer': 'អាវទ្រនាប់ក្ដោបមុខក្រណាត់ប៉ាក់ស្ទីលកីឡា Racerback',
  'Full Coverage Concealer': 'គ្រីមបិទបាំងស្នាមកម្រិតខ្ពស់ Full Coverage',
  'FULL NAME': 'FULL NAME',
  'Full Name': 'ឈ្មោះពេញ',
  'Full-Body Invisible Suit': 'ឈុតរឹបរាងពេញតួខ្លួនមើលមិនធ្លាយ Invisible Suit',
  'Full-Coverage Power Mesh': 'អាវទ្រនាប់ក្ដោបពេញសាច់សំណាញ់ពង្រឹងរាង',
  'Futuristic Cyclops': 'វ៉ែនតាបែបអនាគត Futuristic Cyclops',
  'Gaming Headset': 'កាសស្តាប់ត្រចៀកសម្រាប់លេងហ្គេម Gaming Headset',
  'Gardening Soft Grip': 'ស្រោមដៃធ្វើសួនទន់ដៃចាប់ជាប់មាំ Gardening',
  'Gender': 'ភេទ',
  'Gender :  \${user?.gender.toString() ?? ':
      'Gender :  \${user?.gender.toString() ?? ',
  'General': 'ទូទៅ',
  'General Notification': 'ការជូនដំណឹងទូទៅ',
  'Geometric Hair Claw': 'ដង្កៀបសក់រាងធរណីមាត្រ Geometric Hair Claw',
  'Geometric Hexagon': 'វ៉ែនតារាងឆកោនធរណីមាត្រ Geometric',
  'Geometric Jacquard Knit': 'អាវចាក់ឡាយក្រឡាធរណីមាត្រ Jacquard Knit',
  'Geometric Lace High-Cut': 'ខោក្នុងជើងខ្ពស់ក្រណាត់ប៉ាក់ធរណីមាត្រ',
  'Geometric Lace Longline': 'អាវទ្រនាប់ជួរវែងក្រណាត់ប៉ាក់ធរណីមាត្រ',
  'Geometric Pattern Dress Sock': 'ស្រោមជើងផ្លូវការឡាយក្រឡាធរណីមាត្រ',
  'Geometric Print Polo': 'អាវប៉ូឡូព្រីនឡាយធរណីមាត្រ Geometric Print',
  'Geometric Print Silk': 'កន្សែងសូត្រព្រីនឡាយធរណីមាត្រ Geometric Silk',
  'Get discount up to 50% today only.': 'Get discount up to 50% today only.',
  'Get Organized': 'រៀបចំឱ្យមានសណ្តាប់ធ្នាប់',
  'Get Started': 'Get Started',
  'Giant Inflatable Rainbow Sprinkler': 'ឧបករណ៍បាញ់ទឹកពោងឥន្ទធនូខ្នាតយក្ស',
  'Gift': 'កាដូ',
  'Gilded Executive Leather': 'កាបូបស្បែកធុរកិច្ចគែមមាស Gilded Executive',
  'Gilded Executive Wallet': 'កាបូបលុយធុរកិច្ចគែមមាស Gilded Executive',
  'Gingham Summer Skirt': 'សំពត់រដូវក្តៅឡាយការ៉ូតូច Gingham Summer',
  'Girls': 'ក្មេងស្រី',
  'Gladiator High Sandals': 'ស្បែកជើងរាត់ខ្សែខ្ពស់ស្ទីលរ៉ូម៉ាំង Gladiator',
  'Glass Shine Gloss': 'ក្រែមរលោងបបូរមាត់ចែងចាំងដូចកញ្ចក់ Glass Shine',
  'Glass Strengthening Base': 'ទឹកថ្នាំទ្រនាប់ពង្រឹងក្រចកឱ្យរឹងមាំ Glass Base',
  'Glitter Lurex Party Tights': 'ស្រោមជើងខោវែងចែងចាំងសម្រាប់ជប់លៀង',
  'Glitter Mesh Party Sock': 'ស្រោមជើងសំណាញ់ចែងចាំងសម្រាប់ជប់លៀង',
  'Glitter Party Clutch': 'កាបូបយួរដៃរាត្រីសមោសរចែងចាំង Glitter',
  'Glitters Night Out Mini':
      'រ៉ូបខ្លីចែងចាំងសម្រាប់រាត្រីកម្សាន្ត Glitters Mini',
  'Global Traveler 50L': 'កាតាបអ្នកធ្វើដំណើរជុំវិញពិភពលោក 50L',
  'Glossing Glaze Treatment': 'គ្រីមថែរក្សាសក់ឱ្យរលោងរលើប Glossing Glaze',
  'Gloves': 'ស្រោមដៃ',
  'Glow Setting Powder': 'ម្សៅផាត់មុខឱ្យមានពន្លឺ Glow Setting Powder',
  'Glute-Enhancing Tech Short':
      'ខោខ្លីបច្ចេកវិទ្យាជួយលើករាងគូទ Glute-Enhancing',
  'Gold Chain Necklace': 'ខ្សែករខ្សែច្រវាក់មាស Gold Chain',
  'Gold Leaf Top Coat': 'ទឹកថ្នាំលាបការពារក្រចកឡាយសន្លឹកមាស Gold Leaf',
  'Gold Myrrh': 'ទឹកអប់ក្លិន Gold Myrrh',
  'Gold-Thread Evening Sock': 'ស្រោមជើងវែងរាត្រីសមោសរត្បាញសរសៃមាស',
  'Golden Celebration Package': 'កញ្ចប់កាដូសម្រាប់ពិធីអបអរសាទរពណ៌មាស',
  'Golden Chain Satchel': 'កាបូបយួរដៃខ្សែច្រវាក់ពណ៌មាស',
  'Golden Elixir Serum': 'សេរ៉ូមបំប៉នស្បែកកម្រិតមាស Golden Elixir',
  'Golden Hue Designer Sunglasses': 'វ៉ែនតាការពារកម្តៅថ្ងៃពណ៌មាសរចនាពិសេស',
  'Golden Leaf Hair Vine': 'ខ្សែលម្អសក់រាងស្លឹកឈើពណ៌មាស Golden Leaf',
  'Golden Metal Frame Clutch': 'កាបូបយួរដៃស៊ុមលោហៈពណ៌មាស',
  'Golden Rush Edition': 'នាឡិកាដៃស៊េរីពណ៌មាសលេចធ្លោ Golden Rush',
  'Golden Skeleton Auto': 'នាឡិកាមេកានិចស្វ័យប្រវត្តិកញ្ចក់ធ្លាយពណ៌មាស',
  'Golden Wedding Heels': 'ស្បែកជើងកែងអាពាហ៍ពិពាហ៍ពណ៌មាស Golden Wedding',
  'Golf Stretch Shorts': 'ខោខ្លីយឺតសម្រាប់លេងកីឡាវាយកូនហ្គោល Golf',
  'Good to see you back!': 'រីករាយដែលបានឃើញអ្នកត្រឡប់មកវិញ!',
  'Google Authenticator': 'Google Authenticator',
  'Gothic Rectangle': 'វ៉ែនតារាងចតុកោណរចនាបថ Gothic',
  'Gothic Skeleton Print Shirt':
      'អាវសឺមីព្រីនរូបឆ្អឹងខ្មោចស្ទីលហ្គោធិក (Gothic)',
  'Gothic Style Skull Printed Shirt': 'អាវយឺតព្រីនរូបលលាដ៍ក្បាលស្ទីលហ្គោធិក',
  'Graduation': 'Graduation',
  'Grand Heritage Gold': 'នាឡិកាដៃស៊េរីមាសបេតិកភណ្ឌ Grand Heritage',
  'Grandad Collar Shirt': 'អាវសឺមីកចិន Grandad Collar Shirt',
  'Graphic Print Urban Hoodie': 'អាវរងាមានមួកទីក្រុងព្រីនឡាយក្រាហ្វិក',
  'Graphic Streetwear': 'អាវយឺតស្ទីលយុវវ័យព្រីនឡាយក្រាហ្វិក Graphic',
  'Graphite Brushed Wool': 'អាវធំសាច់រោមសត្វដុសរលោងពណ៌ប្រផេះក្រាហ្វិត',
  'Graphite Ribbed Joggers': 'ខោកីឡាសាច់ឆ្នូតពណ៌ប្រផេះក្រាហ្វិត Graphite',
  'Graphite Stretch Jeans': 'ខោខូវប៊យយឺតពណ៌ប្រផេះក្រាហ្វិត Graphite Stretch',
  'Gray': 'ពណ៌ប្រផេះ',
  'Green': 'ពណ៌បៃតង',
  'Grey Comfort Walker': 'ស្បែកជើងដើរស្រួលពណ៌ប្រផេះ Grey Comfort',
  'Grey Felt Minimalist': 'កាបូបលុយក្រណាត់ហ្វឺតពណ៌ប្រផេះសាមញ្ញ',
  'Grey Marl Basic': 'អាវយឺតពណ៌ប្រផេះដេញអុចៗមូលដ្ឋាន Grey Marl',
  'Grey Mineral Wash Jeans': 'ខោខូវប៊យលាងពណ៌រ៉ែប្រផេះ Grey Mineral Wash',
  'Gucci Collection': 'Gucci Collection',
  'Gym Master Stealth': 'ស្បែកជើងកីឡាហាត់ប្រាណពណ៌ខ្មៅរលោង Gym Master',
  'Hair': 'គ្រឿងលម្អសក់',
  'Haircare': 'Haircare',
  'Hand-Poured Soy Candle Set': 'ឈុតទៀនក្រអូបធ្វើពីសណ្តែកសៀងធ្វើដោយដៃ',
  'Handbags': 'កាបូបយួរដៃ',
  'Handcrafted from full-grain Italian leather, designed for the modern professional.':
      'ធ្វើដោយដៃពីស្បែកអ៊ីតាលីលំដាប់ខ្ពស់ ឌីហ្សាញឡើងសម្រាប់អ្នកជំនាញសម័យថ្មី។',
  'Handcrafted from genuine cowhide leather, this classic bifold wallet offers durability and timeless style.':
      'ធ្វើដោយដៃពីស្បែកគោពិតប្រាកដ កាបូបលុយបត់ពីរម៉ូដ Classic នេះផ្តល់នូវភាពធន់ និងស្ទីលមិនចេះជោគជាំ។',
  'Handcrafted summer Panama hat made from breathable straw. Perfect for beach vacations and sunny outdoor events, providing both shade and style.':
      'មួកប៉ាណាម៉ាសម្រាប់រដូវក្តៅធ្វើដោយដៃពីចំបើងដែលមានខ្យល់ចេញចូលបានល្អ។ ល្អឥតខ្ចោះសម្រាប់ការដើរលេងឆ្នេរ និងព្រឹត្តិការណ៍ក្រៅផ្ទះដែលមានពន្លឺថ្ងៃ ផ្តល់ទាំងម្លប់និងស្ទីលដ៏ទាក់ទាញ។',
  'Handmade Spa Kit': 'ឈុតស្ប៉ាថែរក្សាខ្លួនធ្វើដោយដៃ Handmade Spa Kit',
  'Hape x The Met: Van Gogh Block Set': 'ដុំឈើតម្រៀប Hape x The Met: Van Gogh',
  'Harajuku Hip Hop Letter Jeans': 'ខោខូវប៊យឆ្លាក់អក្សរស្ទីលហាលីជូគូហ៊ីបហប',
  'Hats': 'មួក',
  'Health tracking meets high fashion. Water-resistant with a 5-day battery life.':
      'ការតាមដានសុខភាពរួមបញ្ចូលជាមួយម៉ូដទាន់សម័យ។ មិនជ្រាបទឹក និងមានថ្មប្រើបានរហូតដល់ ៥ ថ្ងៃ។',
  'Heather Purple Crew': 'អាវយឺតកមូលពណ៌ស្វាយដេញស្រទន់ Heather Purple',
  'Heathered Grey Performance': 'អាវកីឡាពណ៌ប្រផេះដេញស្រទន់ Heathered Grey',
  'Heavy Duty Work Gloves': 'Heavy Duty Work Gloves',
  'Heavy Jersey Shorts': 'Heavy Jersey Shorts',
  'Heavy Knit Muffler': 'Heavy Knit Muffler',
  'Heavyweight Charcoal': 'Heavyweight Charcoal',
  'Heavyweight Flannel': 'Heavyweight Flannel',
  'Heeled': 'Heeled',
  'Heeled Loafer Style': 'Heeled Loafer Style',
  'Hello Romina!': 'Hello Romina!',
  'Help Center': 'Help Center',
  'Heritage Brown Suitcase': 'វ៉ាលីពណ៌ត្នោតម៉ូដបេតិកភណ្ឌ Heritage',
  'Heritage Cable Knit': 'Heritage Cable Knit',
  'Heritage Plaid Trench': 'Heritage Plaid Trench',
  'Heritage Tan Leather': 'កាបូបស្បែកពណ៌ត្នោតខ្ចីម៉ូដបេតិកភណ្ឌ Heritage',
  'Heritage Tweed Vest': 'Heritage Tweed Vest',
  'High-Gloss Gel Lacquer': 'High-Gloss Gel Lacquer',
  'High-Impact Performance Run': 'High-Impact Performance Run',
  'High-Impact Sport Sculpt': 'High-Impact Sport Sculpt',
  'High-Leg Sculpting Panty': 'High-Leg Sculpting Panty',
  'High-Neck Victorian Blouse': 'High-Neck Victorian Blouse',
  'High-Performance Compression': 'High-Performance Compression',
  'High-performance moisture-wicking fabric designed to keep you cool and dry during intense workouts. Features 4-way stretch for maximum mobility.':
      'High-performance moisture-wicking fabric designed to keep you cool and dry during intense workouts. Features 4-way stretch for maximum mobility.',
  'High-quality genuine leather belt with a sleek silver buckle. Perfect for both formal suits and casual jeans, providing durability and style.':
      'High-quality genuine leather belt with a sleek silver buckle. Perfect for both formal suits and casual jeans, providing durability and style.',
  'High-quality lenses providing 100% UV protection and durable frames.':
      'High-quality lenses providing 100% UV protection and durable frames.',
  'High-quality materials and craftsmanship for ultimate durability.':
      'High-quality materials and craftsmanship for ultimate durability.',
  'High-quality materials and craftsmanship.':
      'High-quality materials and craftsmanship.',
  'High-Waist Comfort Leggings': 'High-Waist Comfort Leggings',
  'High-Waist Control Top': 'High-Waist Control Top',
  'High-Waist Denim Skirt': 'High-Waist Denim Skirt',
  'High-Waist Mesh Brief': 'High-Waist Mesh Brief',
  'High-Waist Modal Shorts': 'High-Waist Modal Shorts',
  'High-Waist Thong Shaper': 'High-Waist Thong Shaper',
  'High-Waist Yoga Leggings': 'High-Waist Yoga Leggings',
  'High-Waisted Bootcut Leggings': 'High-Waisted Bootcut Leggings',
  'Hiking Adventure Pack': 'កាតាបដើរកម្សាន្តព្រៃ Hiking Adventure',
  'Hiking Water-Resistant Cargo': 'Hiking Water-Resistant Cargo',
  'Hip Hop Embroidered Jeans': 'Hip Hop Embroidered Jeans',
  'Holiday Tropical Print': 'Holiday Tropical Print',
  'Holographic Nail Tint': 'Holographic Nail Tint',
  'Home': 'ទំព័រដើម',
  'HOME DECOR': 'ការដេគ័រផ្ទះ',
  'Hooded Parka Coat': 'Hooded Parka Coat',
  'Hooded Relaxed Cardigan': 'Hooded Relaxed Cardigan',
  'Hot Wheels: Ultimate T-Rex Garage': 'Hot Wheels: Ultimate T-Rex Garage',
  'How do I cancel a orders product?':
      'តើខ្ញុំអាចលុបចោលការបញ្ជាទិញផលិតផលយ៉ាងដូចម្តេច?',
  'How to add promo on Evira?':
      'តើត្រូវបន្ថែមប្រូម៉ូសិនក្នុង Evira យ៉ាងដូចម្តេច?',
  'How to use Evira?': 'តើត្រូវប្រើប្រាស់ Evira យ៉ាងដូចម្តេច?',
  'Hyaluronic Acid Ampoule': 'Hyaluronic Acid Ampoule',
  'Hybrid Fitness Pro': 'Hybrid Fitness Pro',
  'Hydra-Glow Serum': 'Hydra-Glow Serum',
  'Hydrating Lip Oil': 'Hydrating Lip Oil',
  'Hyper Focus Sport': 'Hyper Focus Sport',
  'I already have an account': 'ខ្ញុំមានគណនីរួចហើយ',
  'Iced Matcha EDP': 'Iced Matcha EDP',
  'Icy White Canvas': 'Icy White Canvas',
  'Illuminating Primer': 'Illuminating Primer',
  'Imperial Sandalwood': 'Imperial Sandalwood',
  'Indigo Dye Polo': 'Indigo Dye Polo',
  'Infinity Cushion Air': 'Infinity Cushion Air',
  'Infinity Loop Scarf': 'Infinity Loop Scarf',
  'Infused with rare essential oils of sandalwood and jasmine.':
      'Infused with rare essential oils of sandalwood and jasmine.',
  'Instagram': 'អ៊ិនស្តាក្រាម',
  'Interactive Coding Turtle': 'Interactive Coding Turtle',
  'Invisible Control Slip': 'Invisible Control Slip',
  'Invite': 'Invite',
  'Invite Friends': 'Invite Friends',
  'Invited': 'Invited',
  'Ionic Nano Steamer': 'Ionic Nano Steamer',
  'Is Evira free to use?': 'តើការប្រើប្រាស់ Evira ឥតគិតថ្លៃមែនទេ?',
  'Italian Leather Oxford Derby': 'Italian Leather Oxford Derby',
  'Italian Micro-Net Fishnet': 'Italian Micro-Net Fishnet',
  'Italian Silk Evening Suit': 'Italian Silk Evening Suit',
  'Italian Slim Fit Blue': 'Italian Slim Fit Blue',
  'Italian Wool Cashmere Coat': 'Italian Wool Cashmere Coat',
  'Italian Wool Pinstripe': 'Italian Wool Pinstripe',
  'Item': 'Item',
  'Item summary': 'សេចក្តីសង្ខេបមុខទំនិញ',
  'Item:': 'ទំនិញ:',
  'items': 'ទំនិញ',
  'items found': 'ទំនិញត្រូវបានរកឃើញ',
  'items\'] > 1 ? \'s': 'items\'] > 1 ? \'s',
  'Ivory Wedding Dinner Jacket': 'Ivory Wedding Dinner Jacket',
  'Jackets': 'Jackets',
  'Jade Facial Roller': 'Jade Facial Roller',
  'Jeans': 'Jeans',
  'Jelly Blox: Squish Train Building Set':
      'Jelly Blox: Squish Train Building Set',
  'Jersey Knit Polo': 'Jersey Knit Polo',
  'Jet Black Formal Trousers': 'Jet Black Formal Trousers',
  'Jeweled Hair Comb': 'Jeweled Hair Comb',
  'Jewelry': 'គ្រឿងអលង្ការ',
  'Jewelry & Watches': 'គ្រឿងអលង្ការ និង នាឡិកា',
  'Joggers': 'Joggers',
  'Just for You': 'Just for You',
  'Just For You': 'សម្រាប់អ្នកតែប៉ុណ្ណោះ',
  'Keep your hands warm with these premium thermal gloves. Windproof and touchscreen compatible for your smartphone, perfect for cold commutes.':
      'Keep your hands warm with these premium thermal gloves. Windproof and touchscreen compatible for your smartphone, perfect for cold commutes.',
  'Keratin Straightening Cream': 'Keratin Straightening Cream',
  'Khaki Cargo Basic': 'Khaki Cargo Basic',
  'Khaki Field Messenger': 'កាបូបស្ពាយចំហៀងពណ៌កាគី Khaki Field',
  'Khaki Safari Skirt': 'Khaki Safari Skirt',
  'Khaki Utility Tote': 'កាបូបយួរធំប្រើប្រាស់ទូទៅពណ៌កាគី',
  'Khaki Utility Work Pants': 'Khaki Utility Work Pants',
  'Khmer': 'ភាសាខ្មែរ',
  'Kids Cartoon Slippers': 'Kids Cartoon Slippers',
  'Kids UV Protection': 'វ៉ែនតាកុមារការពារកាំរស្មី UV',
  'Kids Winter Mittens': 'Kids Winter Mittens',
  'KIDS WORLD': 'ពិភពកុមារ',
  'Kitchen': 'ផ្ទះបាយ',
  'KitchenAid Kids Real Working Blender':
      'KitchenAid Kids Real Working Blender',
  'Knitted Cotton Polo': 'Knitted Cotton Polo',
  'L.HOME': 'ទំព័រដើម',
  'Lace Trim Balconette': 'Lace Trim Balconette',
  'Lace Trim Summer Top': 'Lace Trim Summer Top',
  'Lace-Edge Victorian Frill': 'Lace-Edge Victorian Frill',
  'Lace-Trimmed Brazilian': 'Lace-Trimmed Brazilian',
  'Lace-Trimmed Silk Slip': 'Lace-Trimmed Silk Slip',
  'Language': 'Language',
  'Latex Protective Gloves': 'Latex Protective Gloves',
  'Lavender Breeze Casual': 'Lavender Breeze Casual',
  'Lavender Soak Salts': 'Lavender Soak Salts',
  'Leapfrog Twinkle Lights Learning Piano':
      'Leapfrog Twinkle Lights Learning Piano',
  'Learn More': 'Learn More',
  'Leather & Tobacco': 'Leather & Tobacco',
  'Leather Biker Jacket': 'Leather Biker Jacket',
  'Leather Cross Slippers': 'Leather Cross Slippers',
  'Leather Driving Gloves': 'Leather Driving Gloves',
  'Leather Flat Cap': 'មួកសំប៉ែតស្បែក Leather Flat Cap',
  'Leather Handbag': 'Leather Handbag',
  'Leather Mini Skirt': 'Leather Mini Skirt',
  'Leather Moto Gilet': 'Leather Moto Gilet',
  'Leave-In Hydration Milk': 'Leave-In Hydration Milk',
  'LED Light Therapy Mask': 'LED Light Therapy Mask',
  'Leggings': 'Leggings',
  'Lego Smart Play: Mos Eisley Cantina': 'Lego Smart Play: Mos Eisley Cantina',
  'Leopard Print Heels': 'Leopard Print Heels',
  'Leopard Print Scrunchie': 'Leopard Print Scrunchie',
  'Leopard Print Sheer Mesh': 'Leopard Print Sheer Mesh',
  'Leopard Print Wrap': 'Leopard Print Wrap',
  'Let\'s get started': 'Let\'s get started',
  'Li Anna': 'Li Anna',
  'LIFESTYLE': 'របៀបរស់នៅ',
  'Lift-and-Sculpt Booty Short': 'Lift-and-Sculpt Booty Short',
  'Light Blue Pastel Business': 'Light Blue Pastel Business',
  'Light Grey Marl': 'Light Grey Marl',
  'Light Grey Marl Joggers': 'Light Grey Marl Joggers',
  'Lightweight and breathable. Perfect for the stylish city dweller.':
      'Lightweight and breathable. Perfect for the stylish city dweller.',
  'Lightweight and durable, this canvas satchel is ideal for students and daily commuters who value style and function.':
      'ទម្ងន់ស្រាល និងធន់ កាបូបក្រណាត់កង់វ៉ាស់នេះស័ក្តិសមបំផុតសម្រាប់សិស្សានុសិស្ស និងអ្នកធ្វើដំណើរប្រចាំថ្ងៃដែលឱ្យតម្លៃលើស្ទីល និងមុខងារប្រើប្រាស់។',
  'Lightweight and responsive, perfect for marathon runners and speed training.':
      'Lightweight and responsive, perfect for marathon runners and speed training.',
  'Lightweight Coaching Jacket': 'Lightweight Coaching Jacket',
  'Lightweight Foldable Bag': 'កាបូបបត់បានទម្ងន់ស្រាល',
  'Lightweight Linen Duster': 'Lightweight Linen Duster',
  'Lightweight Linen Layer': 'Lightweight Linen Layer',
  'Lightweight Linen Sleepers': 'Lightweight Linen Sleepers',
  'Lightweight Run Shorts': 'Lightweight Run Shorts',
  'Lightweight Summer Cover': 'Lightweight Summer Cover',
  'Lightweight, breathable swim shorts featuring a mesh lining and secure zip pockets.':
      'Lightweight, breathable swim shorts featuring a mesh lining and secure zip pockets.',
  'Linen Summer Blazer': 'Linen Summer Blazer',
  'Linen Summer Scarf': 'Linen Summer Scarf',
  'Linen Summer Shorts': 'Linen Summer Shorts',
  'Linen Summer Wrap': 'Linen Summer Wrap',
  'Linen-Blend Summer Polo': 'Linen-Blend Summer Polo',
  'Lingerie': 'សម្លៀកបំពាក់ក្នុងនារី',
  'Lionel Messi - Lego Soccer Legend': 'Lionel Messi - Lego Soccer Legend',
  'Liquid Amber Essence': 'Liquid Amber Essence',
  'Lite-Walk Everyday': 'Lite-Walk Everyday',
  'Lite-Weight Mesh Sport Loafer': 'Lite-Weight Mesh Sport Loafer',
  'Little Black Party Dress': 'Little Black Party Dress',
  'LIVE': 'LIVE',
  'Living Room': 'បន្ទប់ទទួលភ្ញៀវ',
  'Loafers': 'Loafers',
  'Location details': 'ព័ត៌មានលម្អិតទីតាំង',
  'Login to LOOMA': 'ចូលទៅកាន់ LOOMA',
  'Manage your account, check notifications, rating on product and more.':
      'គ្រប់គ្រងគណនីរបស់អ្នក ពិនិត្យមើលការជូនដំណឹង ការវាយតម្លៃលើផលិតផល និងច្រើនទៀត។',
  'Use phone number': 'ប្រើប្រាស់លេខទូរសព្ទ',
  'Continue with Google': 'បន្ដជាមួយ Google',
  'Don\'t have an account?': 'មិនទាន់មានគណនីមែនទេ?',
  'Sign up': 'ចុះឈ្មោះ',
  'Login': 'ចូលប្រើប្រាស់',
  'Logo Print Sport Tee': 'Logo Print Sport Tee',
  'Logout': 'ចាកចេញ',
  'Lolita Style Plaid Mini Skirt': 'Lolita Style Plaid Mini Skirt',
  'Long Sleeve Cotton Polo': 'Long Sleeve Cotton Polo',
  'Longline Black Curved': 'Longline Black Curved',
  'Longline Open Front Wrap': 'Longline Open Front Wrap',
  'Longwear Setting Spray': 'Longwear Setting Spray',
  'Looks like you haven': 'Looks like you haven',
  'Looks like you haven\'t added anything to your bag yet.':
      'Looks like you haven\'t added anything to your bag yet.',
  'LOOMA': 'LOOMA',
  'LOOMA ACCESSORIES': 'LOOMA ACCESSORIES',
  'LOOMA BAGS': 'LOOMA BAGS',
  'LOOMA BEAUTY': 'LOOMA BEAUTY',
  'LOOMA CLOTHES': 'LOOMA CLOTHES',
  'LOOMA FILTER': 'LOOMA FILTER',
  'LOOMA GIFTS': 'LOOMA GIFTS',
  'Looma Premium': 'Looma Premium',
  'LOOMA PREMIUM': 'LOOMA PREMIUM',
  'LOOMA SHOES': 'LOOMA SHOES',
  'LOOMA SHOPPING': 'LOOMA SHOPPING',
  'LOOMA TOYS': 'LOOMA TOYS',
  'LOOMA WATCH': 'LOOMA WATCH',
  'Loose Fit Skater Jeans': 'Loose Fit Skater Jeans',
  'Lost internet connection': 'Lost internet connection',
  'Luxe Cashmere Polo': 'Luxe Cashmere Polo',
  'Luxe Leather High-Top': 'Luxe Leather High-Top',
  'Luxe Satin Shirt': 'Luxe Satin Shirt',
  'Luxe Travel Brush Set': 'Luxe Travel Brush Set',
  'Luxe Velour Joggers': 'Luxe Velour Joggers',
  'Luxe Velvet Trim Triangle': 'Luxe Velvet Trim Triangle',
  'Luxurious 100% pure silk scarf with a vibrant floral pattern. Smooth, elegant, and perfect for any formal or casual occasion.':
      'Luxurious 100% pure silk scarf with a vibrant floral pattern. Smooth, elegant, and perfect for any formal or casual occasion.',
  'Luxuriously soft 100% cashmere cardigan featuring a classic button-down front and ribbed trims.':
      'Luxuriously soft 100% cashmere cardigan featuring a classic button-down front and ribbed trims.',
  'LUXURY': 'ភាពប្រណីត (LUXURY)',
  'Luxury': 'ស្ទីល Luxury',
  'Luxury Birthday Hamper': 'Luxury Birthday Hamper',
  'Luxury Candle Set': 'Luxury Candle Set',
  'Luxury Choker Set': 'ឈុតខ្សែករកៀកកប្រណីត Luxury Choker',
  'Luxury fragrance components sourced from the finest materials.':
      'Luxury fragrance components sourced from the finest materials.',
  'Luxury Gift Box': 'Luxury Gift Box',
  'Luxury Gold Buckle': 'Luxury Gold Buckle',
  'Luxury Gold Frame': 'វ៉ែនតាស៊ុមមាសប្រណីត Luxury Gold',
  'Luxury Gold Trim Sandals': 'Luxury Gold Trim Sandals',
  'Luxury Leather Satchel': 'កាបូបស្បែកយួរដៃប្រណីត Satchel',
  'Luxury Makeup Kit': 'Luxury Makeup Kit',
  'Luxury Paddle Brush': 'Luxury Paddle Brush',
  'Luxury Patent Formal Shoe': 'Luxury Patent Formal Shoe',
  'Luxury Pearl Clutch': 'កាបូបយួរដៃគជ់ខ្យងប្រណីត Luxury Pearl',
  'Luxury Pearl Headband': 'Luxury Pearl Headband',
  'Luxury Perfume': 'Luxury Perfume',
  'Luxury Perfume Box': 'Luxury Perfume Box',
  'Luxury Perfume Set': 'Luxury Perfume Set',
  'Luxury Ribbed Wool Tights': 'Luxury Ribbed Wool Tights',
  'Luxury Rose Box': 'Luxury Rose Box',
  'Luxury Rose Gold': 'Luxury Rose Gold',
  'Luxury Scented Candle Set': 'Luxury Scented Candle Set',
  'Luxury Silk Pillowcase Set': 'Luxury Silk Pillowcase Set',
  'Luxury Silk-Blend Trouser': 'Luxury Silk-Blend Trouser',
  'Luxury Suede Long Wallet': 'កាបូបលុយវែងស្បែកសត្វទន់ប្រណីត Luxury Suede',
  'Luxury Top Hat': 'មួកស៊ីឡាំងខ្ពស់ប្រណីត Luxury Top Hat',
  'Luxury Tulle Hipster Bloom': 'Luxury Tulle Hipster Bloom',
  'Luxury Watch Gift Set': 'Luxury Watch Gift Set',
  'Magshuto Vert Tower - Kinetic Play': 'Magshuto Vert Tower - Kinetic Play',
  'Make this as the default address': 'កំណត់ជាអាសយដ្ឋានលំនាំដើម',
  'Makeup': 'Makeup',
  'Makeup & Skincare': 'គ្រឿងសម្អាង និង ផលិតផលថែរក្សាស្បែក',
  'Makeup Collection': 'Makeup Collection',
  'Male': 'ប្រុស',
  'Mandarin Collar Casual': 'Mandarin Collar Casual',
  'Map Type': 'ប្រភេទផែនទី',
  'Marble Coffee Mug Trio': 'Marble Coffee Mug Trio',
  'Maroon Velvet Touch Polo': 'Maroon Velvet Touch Polo',
  'Massage Sole Slippers': 'Massage Sole Slippers',
  'Masterpiece Leather Briefcase': 'Masterpiece Leather Briefcase',
  'Matcha Detox Scrub': 'Matcha Detox Scrub',
  'Maternity Over-Belly Support': 'Maternity Over-Belly Support',
  'Matte Black Commuter': 'កាបូបអ្នកធ្វើដំណើរពណ៌ខ្មៅក្រើម',
  'Matte Black Rolling Bag': 'កាបូបមានកង់អូសពណ៌ខ្មៅក្រើម',
  'Matte Black Rubber Sandals': 'Matte Black Rubber Sandals',
  'Matte Finish Claw': 'Matte Finish Claw',
  'Matte Finish Executive': 'Matte Finish Executive',
  'Matte Finish Loafers': 'Matte Finish Loafers',
  'Matte Finish Shoulder Bag': 'Matte Finish Shoulder Bag',
  'Matte Finish Top Coat': 'Matte Finish Top Coat',
  'Matte Grey Performance': 'Matte Grey Performance',
  'Matte Navy Slim Fold': 'កាបូបលុយបត់ស្តើងពណ៌ខៀវក្រម៉ៅក្រើម',
  'Matte Nylon Bodywarmer': 'Matte Nylon Bodywarmer',
  'Matte Rose Gold': 'កាបូបពណ៌មាសផ្កាឈូកក្រើម Matte Rose Gold',
  'Maxi Tulle Skirt': 'Maxi Tulle Skirt',
  'Login Required': 'តម្រូវឱ្យចូលប្រព័ន្ធ',
  'Please login to view your profile and manage your account.':
      'សូមចូលប្រព័ន្ធដើម្បីមើលប្រវត្តិរូប និងគ្រប់គ្រងគណនីរបស់អ្នក។',
  'Login Now': 'ចូលប្រព័ន្ធឥឡូវនេះ',
  'Memory Foam Comfort Slippers': 'Memory Foam Comfort Slippers',
  'Men': 'Men',
  'Men\'s Urban Wear': 'Men\'s Urban Wear',
  'Menu': 'Menu',
  'Merino Wool Hiking Sock': 'Merino Wool Hiking Sock',
  'Merino Wool Layer': 'Merino Wool Layer',
  'Mermaid Hem Skirt': 'Mermaid Hem Skirt',
  'Mesh Trucker Cap': 'មួកប៉ារ៉ាកែបសំណាញ់ Mesh Trucker',
  'Messenger': 'កាបូបស្ពាយចំហៀង (Messenger)',
  'Metallic Eye Pigment': 'Metallic Eye Pigment',
  'Micro-Modal Soft Shaper': 'Micro-Modal Soft Shaper',
  'Micro-Print Dress Shirt': 'Micro-Print Dress Shirt',
  'Microfiber Laser-Cut Thong': 'Microfiber Laser-Cut Thong',
  'Midnight Black Carryall': 'កាបូបដាក់ឥវ៉ាន់ធំពណ៌ខ្មៅ Midnight Black',
  'Midnight Black Fleece': 'Midnight Black Fleece',
  'Midnight Black Hoodie': 'Midnight Black Hoodie',
  'Midnight Black Voyager': 'Midnight Black Voyager',
  'Midnight Blue Pocket': 'Midnight Blue Pocket',
  'Midnight Camo Active': 'Midnight Camo Active',
  'Midnight Cherry Polish': 'Midnight Cherry Polish',
  'Midnight Executive': 'Midnight Executive',
  'Midnight Gold Trim Polo': 'Midnight Gold Trim Polo',
  'Midnight Jasmine': 'Midnight Jasmine',
  'Midnight Kohl Pencil': 'Midnight Kohl Pencil',
  'Midnight Navy Satchel': 'កាបូបយួរដៃពណ៌ខៀវក្រម៉ៅ Midnight Navy',
  'Midnight Navy Sweatpants': 'Midnight Navy Sweatpants',
  'Midnight Navy Tuxedo': 'Midnight Navy Tuxedo',
  'Midnight Red Velvet Stiletto': 'Midnight Red Velvet Stiletto',
  'Midnight Repair Oil': 'Midnight Repair Oil',
  'Midnight Satin Kimono': 'Midnight Satin Kimono',
  'Midnight Stealth Black': 'Midnight Stealth Black',
  'Midnight Velvet Gilet': 'Midnight Velvet Gilet',
  'Midnight Velvet Tuxedo': 'Midnight Velvet Tuxedo',
  'Military Green Cargo': 'Military Green Cargo',
  'Military Green Cargo Polo': 'Military Green Cargo Polo',
  'Military Style Cap': 'មួកប៉ារ៉ាកែបស្ទីលយោធា Military Style',
  'Military Tactical': 'Military Tactical',
  'Mineral Dew Cream': 'Mineral Dew Cream',
  'Mineral Loose Powder': 'Mineral Loose Powder',
  'Mineral Ridge Filler': 'Mineral Ridge Filler',
  'Mini Birthday Treat': 'Mini Birthday Treat',
  'Mini Travel Messenger Pouch': 'Mini Travel Messenger Pouch',
  'Minimal Hoodie': 'Minimal Hoodie',
  'Minimalist': 'ស្ទីល Minimalist',
  'Minimalist Accessories': 'Minimalist Accessories',
  'Minimalist Beige Canvas': 'កាបូបក្រណាត់កង់វ៉ាស់ពណ៌បន៍ត្នោតខ្ចីសាមញ្ញ',
  'Minimalist Bonded Edge': 'Minimalist Bonded Edge',
  'Minimalist Bone Overcoat': 'Minimalist Bone Overcoat',
  'Minimalist Cotton Tote Bag': 'Minimalist Cotton Tote Bag',
  'Minimalist Cream Polo': 'Minimalist Cream Polo',
  'Minimalist Cross-Strap Slides': 'Minimalist Cross-Strap Slides',
  'Minimalist design featuring a genuine brown leather strap and reliable Japanese quartz movement. Perfect for daily office wear.':
      'Minimalist design featuring a genuine brown leather strap and reliable Japanese quartz movement. Perfect for daily office wear.',
  'Minimalist Desk Organizer': 'Minimalist Desk Organizer',
  'Minimalist Gold Watch': 'Minimalist Gold Watch',
  'Minimalist Gold Watch with Leather Strap':
      'Minimalist Gold Watch with Leather Strap',
  'Minimalist Home Slides': 'Minimalist Home Slides',
  'Minimalist Knit Skirt': 'Minimalist Knit Skirt',
  'Minimalist Laptop Bag': 'កាបូបដាក់កុំព្យូទ័ររចនាបថសាមញ្ញ Minimalist',
  'Minimalist Mesh Band': 'Minimalist Mesh Band',
  'Minimalist Silver Barrette': 'Minimalist Silver Barrette',
  'Minimalist Silver Ring': 'ចិញ្ចៀនប្រាក់សាមញ្ញ Minimalist Silver',
  'Minimalist Slate': 'Minimalist Slate',
  'Minimalist Slate Clutch': 'កាបូបយួរដៃពណ៌ប្រផេះសាមញ្ញ Minimalist',
  'Minimalist Slate Gilet': 'Minimalist Slate Gilet',
  'Minimalist Slate Gray Knit Runner': 'Minimalist Slate Gray Knit Runner',
  'Minimalist Slim Card Holder': 'កាបូបដាក់កាតរាងស្តើងសាមញ្ញ Minimalist',
  'Minimalist Tech-Mesh Thong': 'Minimalist Tech-Mesh Thong',
  'Minimalist Tote Travel Bag': 'កាបូបធ្វើដំណើរយួរធំសាមញ្ញ Minimalist',
  'Minimalist White Crew': 'Minimalist White Crew',
  'Minimalist White Flats': 'Minimalist White Flats',
  'Minimalist White Handbag': 'កាបូបយួរដៃពណ៌សសាមញ្ញ Minimalist',
  'Minimalist White Hoodie': 'Minimalist White Hoodie',
  'Minimalist White Smartwatch': 'Minimalist White Smartwatch',
  'Mirrored Lenses': 'វ៉ែនតាកញ្ចក់ចាំងដូចកញ្ចក់ឆ្លុះ Mirrored',
  'ML': 'ML',
  'Mocha Teddy Shearling': 'Mocha Teddy Shearling',
  'Modal Ribbed Boyshort': 'Modal Ribbed Boyshort',
  'Modal Ribbed Sleep Dress': 'Modal Ribbed Sleep Dress',
  'Model info': 'ព័ត៌មានម៉ូដែល',
  'Model is 175 cm tall / 65 kg weight and is wearing size M.':
      'Model is 175 cm tall / 65 kg weight and is wearing size M.',
  'Modern Buckle Loafers': 'Modern Buckle Loafers',
  'Modern Buckle Wallet': 'កាបូបលុយមានគន្លឹះចង្រ្កានទំនើប',
  'Modern Canvas Utility': 'Modern Canvas Utility',
  'Modern Denim Daily Bag': 'កាបូបប្រចាំថ្ងៃក្រណាត់ខូវប៊យទំនើប',
  'Modern Denim Messenger': 'កាបូបស្ពាយចំហៀងក្រណាត់ខូវប៊យទំនើប',
  'Modern Designer Handbag': 'កាបូបយួរដៃម៉ូដទាន់សម័យ Modern Designer',
  'Modern Fit Burgundy Chinos': 'Modern Fit Burgundy Chinos',
  'Modern Flight Case': 'ប្រអប់ដាក់ឥវ៉ាន់ហោះហើរទំនើប Modern Flight Case',
  'Modern Geometric Clutch': 'កាបូបយួរដៃរាងធរណីមាត្រទំនើប',
  'Modern Interior': 'ការរចនាផ្ទៃក្នុងទំនើប',
  'Modern Jet Black Suit': 'Modern Jet Black Suit',
  'Modern Jet Black Tux': 'Modern Jet Black Tux',
  'Modern Minimalist': 'Modern Minimalist',
  'Modern Shield': 'វ៉ែនតាការពារភ្នែកទំនើប Modern Shield',
  'Modern Tech Gadget Box': 'Modern Tech Gadget Box',
  'Modern Urban Rucksack': 'កាតាបទីក្រុងទំនើប Modern Urban',
  'Modern wayfarer frames in matte black finish. Durable polycarbonate material with high-definition clarity.':
      'ស៊ុមវ៉ែនតាម៉ូដ Wayfarer ទំនើប ពណ៌ខ្មៅក្រើម។ ផលិតពីសារធាតុប៉ូលីកាបូណាតដ៏ធន់ ផ្តល់នូវភាពច្បាស់កម្រិតខ្ពស់។',
  'Modular Tech Vest': 'Modular Tech Vest',
  'Moisture-wicking fabric designed to keep you cool and dry on the golf course or the street.':
      'Moisture-wicking fabric designed to keep you cool and dry on the golf course or the street.',
  'Moisture-Wicking Yoga Grip': 'Moisture-Wicking Yoga Grip',
  'Monster High: Moonspell Magic Willow Thorne':
      'Monster High: Moonspell Magic Willow Thorne',
  'Morning Rain': 'Morning Rain',
  'Most Popular': 'Most Popular',
  'MOST POPULAR': 'MOST POPULAR',
  'Musical Electronic Drum Pad Mat': 'Musical Electronic Drum Pad Mat',
  'Mustard Yellow Boxy': 'Mustard Yellow Boxy',
  'Mustard Yellow Pop': 'Mustard Yellow Pop',
  'Mustard Yellow Streetwear': 'Mustard Yellow Streetwear',
  'My E-Wallet': 'កាបូបលុយអេឡិចត្រូនិករបស់ខ្ញុំ',
  'My Orders': 'ការបញ្ជាទិញរបស់ខ្ញុំ',
  'My QR': 'កូដ QR របស់ខ្ញុំ',
  'Gift Card': 'កាតកាដូ',
  'Find a Store': 'ស្វែងរកហាង',
  'My Shopping Bag': 'កាបូបទិញទំនិញរបស់ខ្ញុំ',
  'My Wallet': 'កាបូបលុយរបស់ខ្ញុំ',
  'Nail Care': 'Nail Care',
  'Name': 'ឈ្មោះ',
  'Name Address': 'ឈ្មោះអាសយដ្ឋាន',
  'Navy Blue Driving Mocs': 'Navy Blue Driving Mocs',
  'Navy Blue Soft-Shell Luggage': 'វ៉ាលីសម្បកទន់ពណ៌ខៀវក្រម៉ៅ Navy Blue',
  'Navy Blue Sport Hoodie': 'Navy Blue Sport Hoodie',
  'Navy Blue Tech Fleece': 'Navy Blue Tech Fleece',
  'Navy Blue Work Blouse': 'Navy Blue Work Blouse',
  'Navy Dive Master': 'Navy Dive Master',
  'Navy Slim Fit Tee': 'Navy Slim Fit Tee',
  'Navy Striped Nautical': 'កាបូបឡាយឆ្នូតម៉ូដនាវិក Navy Striped',
  'Navy Work Shirt': 'Navy Work Shirt',
  'Neo Future Sport Sneaker': 'Neo Future Sport Sneaker',
  'Neon Liquid Motion Bubbler': 'Neon Liquid Motion Bubbler',
  'Neon Party Heels': 'Neon Party Heels',
  'Neon Speedster': 'Neon Speedster',
  'Neroli Coast': 'Neroli Coast',
  'NEW': 'NEW',
  'New accessories coming soon': 'New accessories coming soon',
  'New Arrival': 'ទំនិញមកដល់ថ្មី',
  'NEW ARRIVALS': 'ទំនិញមកដល់ថ្មី',
  'New collection coming soon': 'New collection coming soon',
  'New In': 'ទំនិញថ្មី',
  'New items': 'ទំនិញថ្មីៗ',
  'New Items': 'New Items',
  'New Service Available': 'មានសេវាកម្មថ្មី',
  'New Services Available!': 'មានសេវាកម្មថ្មីៗ!',
  'New Tips Available': 'មានគន្លឹះថ្មីៗ',
  'News%': 'ព័ត៌មានបញ្ចុះតម្លៃ%',
  'Next': 'បន្ទាប់',
  'Niacinamide Essence': 'Niacinamide Essence',
  'Night Vision Spy Goggles': 'Night Vision Spy Goggles',
  'Nightwear': 'Nightwear',
  'Nike Air Max Pro': 'Nike Air Max Pro',
  'No backpacks found': 'No backpacks found',
  'No Blouses found': 'No Blouses found',
  'No bodysuits found': 'No bodysuits found',
  'No boots found': 'No boots found',
  'No cardigans found': 'No cardigans found',
  'No clothes found in this category': 'No clothes found in this category',
  'No clutches found': 'No clutches found',
  'No collection available': 'No collection available',
  'No cosmetics available': 'No cosmetics available',
  'No flats available': 'No flats available',
  'No gifts available': 'No gifts available',
  'No gloves found': 'No gloves found',
  'No handbags found': 'No handbags found',
  'No heels available': 'No heels available',
  'No hoodies available': 'No hoodies available',
  'No image': 'No image',
  'No items available': 'No items available',
  'No items found': 'No items found',
  'No items found for': 'រកមិនឃើញទំនិញសម្រាប់',
  'No Jackets found': 'No Jackets found',
  'No jeans found': 'No jeans found',
  'No joggers found': 'No joggers found',
  'No leggings found': 'No leggings found',
  'No loafers available': 'No loafers available',
  'No messenger bags found': 'No messenger bags found',
  'No new items found': 'No new items found',
  'No nightwear available': 'No nightwear available',
  'No orders yet': 'No orders yet',
  'No pants found': 'No pants found',
  'No perfumes available': 'No perfumes available',
  'No polo shirts found': 'No polo shirts found',
  'No products available.': 'No products available.',
  'No products found': 'No products found',
  'No results for': 'គ្មានលទ្ធផលសម្រាប់',
  'No results for \'\$_searchQuery\'': 'No results for \'\$_searchQuery\'',
  'No results for {0}': 'No results for {0}',
  'No results found': 'រកមិនឃើញលទ្ធផលទេ',
  'No results found for': 'រកមិនឃើញលទ្ធផលសម្រាប់',
  'No results found for {0}': 'No results found for {0}',
  'No sandals available': 'No sandals available',
  'No scarves available': 'No scarves available',
  'No shapewear available': 'No shapewear available',
  'No shirts found': 'No shirts found',
  'No shoes found': 'No shoes found',
  'No shorts available': 'No shorts available',
  'No skirts available': 'No skirts available',
  'No slippers available': 'No slippers available',
  'No sneakers found': 'No sneakers found',
  'No socks available': 'No socks available',
  'No sports items found': 'No sports items found',
  'No suits found': 'No suits found',
  'No sweatshirts found': 'No sweatshirts found',
  'No t-shirts available': 'No t-shirts available',
  'No tights available': 'No tights available',
  'No tote bags found': 'No tote bags found',
  'No toys found': 'No toys found',
  'No transactions found': 'រកមិនឃើញប្រតិបត្តិការទេ',
  'No travel bags available': 'No travel bags available',
  'No vests found': 'No vests found',
  'No wallets found': 'No wallets found',
  'No watches available': 'No watches available',
  'No watches found': 'No watches found',
  'Non-slip grip with high-density cushioning for ultimate joint support.':
      'Non-slip grip with high-density cushioning for ultimate joint support.',
  'Non-Toxic Soy Remover': 'Non-Toxic Soy Remover',
  'Nordic Glass Flower Vase': 'Nordic Glass Flower Vase',
  'Not you?': 'Not you?',
  'Notification': 'ការជូនដំណឹង',
  'Notifications': 'ការជូនដំណឹង',
  'Notification deleted': 'បានលុបការជូនដំណឹង',
  'No notifications yet': 'មិនទាន់មានការជូនដំណឹងនៅឡើយទេ',
  'Today': 'ថ្ងៃនេះ',
  'Yesterday': 'ម្សៃលមិញ',
  'Are you sure you want to clear all notifications?': 'តើអ្នកប្រាកដជាចង់លុបការជូនដំណឹងទាំងអស់មែនទេ?',
  'Now you can track orders in real time':
      'ឥឡូវនេះអ្នកអាចតាមដានការបញ្ជាទិញបានភ្លាមៗ',
  'Ocean Blue Shades': 'វ៉ែនតាកញ្ចក់ពណ៌ខៀវសមុទ្រ Ocean Blue',
  'Off-Shoulder Peplum Blouse': 'Off-Shoulder Peplum Blouse',
  'Off-Shoulder Silk Dress': 'Off-Shoulder Silk Dress',
  'Office': 'ការិយាល័យ',
  'Office Gear': 'ឧបករណ៍ការិយាល័យ',
  'Old Money': 'ស្ទីល Old Money',
  'Olive Drab Tactical': 'Olive Drab Tactical',
  'Olive Drab Tactical Polo': 'Olive Drab Tactical Polo',
  'Olympic Red Sport': 'Olympic Red Sport',
  'Online exchange policy': 'គោលការណ៍ប្តូរទំនិញអនឡាញ',
  'Onyx Black Evening Cape': 'Onyx Black Evening Cape',
  'Opaque 80D Matte Sculpt': 'Opaque 80D Matte Sculpt',
  'Open Toe Spa Slippers': 'Open Toe Spa Slippers',
  'Open-Bust Camisole Shaper': 'Open-Bust Camisole Shaper',
  'Open-Toe Sandal Tights': 'Open-Toe Sandal Tights',
  'Orange': 'ពណ៌ទឹកក្រូច',
  'Order Confirm': 'បញ្ជាក់ការបញ្ជាទិញ',
  'Order Now': 'បញ្ជាទិញឥឡូវនេះ',
  'Orders': 'ការបញ្ជាទិញ',
  'Organic Cotton Lounge Panty': 'Organic Cotton Lounge Panty',
  'Organic Cotton Sleep Tee': 'Organic Cotton Sleep Tee',
  'Organic Hemp Daily Crew': 'Organic Hemp Daily Crew',
  'Orthopedic Support Slippers': 'Orthopedic Support Slippers',
  'Ostrich Feather Clutch': 'កាបូបយួរដៃរោមសត្វអូទ្រីស',
  'Other': 'ផ្សេងៗ',
  'Oud Wood & Amber': 'Oud Wood & Amber',
  'Outdoor Altimeter': 'Outdoor Altimeter',
  'Outdoor Sport Sandals': 'Outdoor Sport Sandals',
  'Outdoor Tactical Gloves': 'Outdoor Tactical Gloves',
  'Outdoor Terrain Flats': 'Outdoor Terrain Flats',
  'Outdoor Trekking Pack': 'កាតាបដើរភ្នំ Outdoor Trekking',
  'Over-the-Knee Ribbed Knit': 'Over-the-Knee Ribbed Knit',
  'Overnight Nail Mask': 'Overnight Nail Mask',
  'Overnight Repair Mask': 'Overnight Repair Mask',
  'Oversized Blanket Scarf': 'Oversized Blanket Scarf',
  'Oversized Chunky Knit': 'Oversized Chunky Knit',
  'Oversized Graphic Tee - Streetwear Collection':
      'Oversized Graphic Tee - Streetwear Collection',
  'Oversized Heavyweight Hoodie': 'Oversized Heavyweight Hoodie',
  'Oversized Hoodie': 'Oversized Hoodie',
  'Oversized Knit Sleep Shirt': 'Oversized Knit Sleep Shirt',
  'Oversized Pastel Blue': 'Oversized Pastel Blue',
  'Oversized Pastel Pink': 'Oversized Pastel Pink',
  'Oversized Red Graphic Tee': 'Oversized Red Graphic Tee',
  'Oversized Skater Jeans': 'Oversized Skater Jeans',
  'Oversized Weekend Tote': 'កាបូបយួរធំខ្នាតយក្សសម្រាប់ចុងសប្តាហ៍',
  'Oxford Button-Down': 'Oxford Button-Down',
  'Oxford Navy Stretch Suit': 'Oxford Navy Stretch Suit',
  'Oxford Style Flat Shoes': 'Oxford Style Flat Shoes',
  'Padded Puffer Jacket': 'Padded Puffer Jacket',
  'Paid': 'បានបង់ប្រាក់',
  'Pants': 'Pants',
  'Panty': 'Panty',
  'Password': 'Password',
  'Pastel Breeze Sneakers': 'Pastel Breeze Sneakers',
  'Pastel Green Handbag': 'កាបូបយួរដៃពណ៌បៃតងស្រទន់ Pastel Green',
  'Pastel Lilac Knitwear': 'Pastel Lilac Knitwear',
  'Pastel Lounge Shorts': 'Pastel Lounge Shorts',
  'Pastel Ombre Cotton Blend': 'Pastel Ombre Cotton Blend',
  'Pastel Peach Flare Top': 'Pastel Peach Flare Top',
  'Pastel Pink Oxford': 'Pastel Pink Oxford',
  'Pastel Summer Polo': 'Pastel Summer Polo',
  'Payment': 'ការទូទាត់',
  'Payment Method': 'Payment Method',
  'Payment Methods': 'វិធីសាស្ត្រទូទាត់ប្រាក់',
  'Payments': 'ការទូទាត់',
  'Peak Performance HIIT': 'Peak Performance HIIT',
  'Pearl Bracelet Set': 'ឈុតកងដៃគជ់ខ្យង Pearl Bracelet',
  'Pearl Essence Gloss': 'Pearl Essence Gloss',
  'Pearl White Wedding Polo': 'Pearl White Wedding Polo',
  'Pencil Business Skirt': 'Pencil Business Skirt',
  'Peony Silk Extract': 'Peony Silk Extract',
  'Peptide Eye Lift': 'Peptide Eye Lift',
  'Perfect for the modern minimalist, this slim holder keeps your essential cards organized without the bulk.':
      'ល្អឥតខ្ចោះសម្រាប់អ្នកចូលចិត្តភាពសាមញ្ញទំនើប កាបូបកាតរាងស្តើងនេះជួយរក្សាកាតចាំបាច់របស់អ្នកឱ្យមានសណ្តាប់ធ្នាប់ដោយមិនក្រាស់ហើម។',
  'Perfume Gift Set': 'Perfume Gift Set',
  'Perfumes': 'ទឹកអប់',
  'Personal Care': 'Personal Care',
  'Personalized': 'Personalized',
  'Personalized Photo Cube': 'Personalized Photo Cube',
  'Phones & Gadgets': 'ទូរស័ព្ទ និង ឧបករណ៍វៃឆ្លាត',
  'Pick Up Your Style': 'Pick Up Your Style',
  'Pink': 'ពណ៌ផ្កាឈូក',
  'Pink Pastel Frames': 'វ៉ែនតាស៊ុមពណ៌ផ្កាឈូកស្រទន់ Pink Pastel',
  'Pink Strappy Sandals': 'Pink Strappy Sandals',
  'Platform Wedge Slippers': 'Platform Wedge Slippers',
  'Platinum Wedding Band': 'ចិញ្ចៀនអាពាហ៍ពិពាហ៍ប្លាទីន Platinum',
  'Playaround Firebase': 'Playaround Firebase',
  'Please check your network and try again':
      'Please check your network and try again',
  'Please enter a valid 16-digit card number':
      'សូមបញ្ចូលលេខប័ណ្ណ ១៦ ខ្ទង់ឱ្យបានត្រឹមត្រូវ',
  'Please enter a valid 3-digit CVV':
      'សូមបញ្ចូលលេខកូដ CVV ៣ ខ្ទង់ឱ្យបានត្រឹមត្រូវ',
  'Please enter a valid expiry date (MM/YY)':
      'Please enter a valid expiry date (MM/YY)',
  'Please enter the card holder\'s full name':
      'Please enter the card holder\'s full name',
  'Pleated Chiffon Gown': 'Pleated Chiffon Gown',
  'Pleated Midi Skirt': 'Pleated Midi Skirt',
  'Pleated Satin Romper': 'Pleated Satin Romper',
  'Pocket Utility Leggings': 'Pocket Utility Leggings',
  'Point at an item to search': 'Point at an item to search',
  'Pointed Toe Nude Heels': 'Pointed Toe Nude Heels',
  'Polarized lenses with a premium gold-toned frame. Elevate your summer style.':
      'Polarized lenses with a premium gold-toned frame. Elevate your summer style.',
  'Polarized Night Vision': 'វ៉ែនតាបើកបរពេលយប់ Polarized Night Vision',
  'Polka Dot Bow-Tie Top': 'Polka Dot Bow-Tie Top',
  'Polka Dot Fashion': 'Polka Dot Fashion',
  'Polka Dot Fashion Tote': 'កាបូបយួរធំឡាយអុច Polka Dot',
  'Polka Dot Midi': 'Polka Dot Midi',
  'Polka Dot Neckerchief': 'Polka Dot Neckerchief',
  'Polka Dot Retro Midi': 'Polka Dot Retro Midi',
  'Polka Dot Sheer Tights': 'Polka Dot Sheer Tights',
  'Polka Dot Silk Sheer': 'Polka Dot Silk Sheer',
  'Polos': 'Polos',
  'popular items': 'popular items',
  'Portable Speaker': 'Portable Speaker',
  'Post-Op Compression Short': 'Post-Op Compression Short',
  'Power-Mesh Waist Trainer': 'Power-Mesh Waist Trainer',
  'PowerOn Energy-Generating Soccer Ball':
      'PowerOn Energy-Generating Soccer Ball',
  'Prebiotic Cloud Milk': 'Prebiotic Cloud Milk',
  'Precision Liquid Liner': 'Precision Liquid Liner',
  'Precision Steel Clippers': 'Precision Steel Clippers',
  'Precision switches with customizable RGB lighting and premium aluminum body.':
      'Precision switches with customizable RGB lighting and premium aluminum body.',
  'Premium Black Bifold': 'កាបូបលុយបត់ពីរពណ៌ខ្មៅកម្រិតប្រណីត Premium',
  'Premium Black Leather Tote': 'កាបូបយួរធំស្បែកពណ៌ខ្មៅកម្រិតប្រណីត',
  'Premium Cashmere Cardigan': 'Premium Cashmere Cardigan',
  'Premium Cotton Set': 'Premium Cotton Set',
  'Premium French Lace Thong': 'Premium French Lace Thong',
  'Premium Gift Set': 'Premium Gift Set',
  'Premium Hard-Shell Suitcase': 'វ៉ាលីសម្បករឹងកម្រិតប្រណីត Premium',
  'Premium Headphones': 'Premium Headphones',
  'Premium Item': 'Premium Item',
  'Premium jewelry crafted with high-quality materials and exquisite attention to detail.':
      'Premium jewelry crafted with high-quality materials and exquisite attention to detail.',
  'Premium Leather Backpack': 'កាតាបស្បែកកម្រិតប្រណីត Premium',
  'Premium Leather Belt': 'Premium Leather Belt',
  'Premium Leather Passport Holder': 'Premium Leather Passport Holder',
  'Premium Micro-Modal Robe': 'Premium Micro-Modal Robe',
  'Premium Pima Cotton': 'Premium Pima Cotton',
  'Premium quality fabrics with standard fitting sizes.':
      'Premium quality fabrics with standard fitting sizes.',
  'Premium quality product designed for durability and comfort.':
      'ផលិតផលគុណភាពខ្ពស់ ឌីហ្សាញឡើងដើម្បីភាពធន់ និងភាពងាយស្រួលក្នុងការប្រើប្រាស់។',
  'PREMIUM SHOPPING': 'PREMIUM SHOPPING',
  'Premium Silk Scarf': 'Premium Silk Scarf',
  'Premium Silk Triangle': 'Premium Silk Triangle',
  'Premium Silk-Blend Polo': 'Premium Silk-Blend Polo',
  'Premium Suede Satchel': 'កាបូបយួរដៃស្បែកសត្វ Premium Suede',
  'Premium Sun Hat': 'មួកការពារកម្តៅថ្ងៃកម្រិតប្រណីត Premium',
  'Premium Surprise Box': 'Premium Surprise Box',
  'Premium Waterproof Heritage Boot': 'Premium Waterproof Heritage Boot',
  'Price Range': 'ចន្លោះតម្លៃ',
  'Primal Hatch: Hybrid Hatcher Egg': 'Primal Hatch: Hybrid Hatcher Egg',
  'Printed': 'មានព្រីនឡាយ',
  'Privacy Policy': 'Privacy Policy',
  'Pro Compression Shirt': 'Pro Compression Shirt',
  'Pro Gamer Mechanical Keyboard': 'Pro Gamer Mechanical Keyboard',
  'Pro-Cycle Aerodynamic Pants': 'Pro-Cycle Aerodynamic Pants',
  'Pro-Dry Gym Joggers': 'Pro-Dry Gym Joggers',
  'Pro-Fit Tennis Polo': 'Pro-Fit Tennis Polo',
  'Pro-Grip Buffer Tool': 'Pro-Grip Buffer Tool',
  'Pro-Running Shorts': 'Pro-Running Shorts',
  'Proceed to Checkout': 'បន្ដទៅកាន់ការទូទាត់',
  'Product details': 'ព័ត៌មានលម្អិតផលិតផល',
  'Professional Boxing Gloves': 'Professional Boxing Gloves',
  'Professional Grey Polo': 'Professional Grey Polo',
  'Professional Laptop Bag': 'កាបូបដាក់កុំព្យូទ័រយួរដៃអាជីព',
  'Professional Navy Tapered': 'Professional Navy Tapered',
  'Professional Tech Bag': 'កាបូបបច្ចេកវិទ្យាអាជីព Professional Tech',
  'Professional Tech Travel Bag':
      'កាបូបធ្វើដំណើរបច្ចេកវិទ្យាអាជីព Professional Tech',
  'Profile': 'ប្រវត្តិរូប',
  'Programmable Emoji Backpack': 'Programmable Emoji Backpack',
  'Promo': 'Promo',
  'Promo & Discount': 'ប្រូម៉ូសិន និងការបញ្ចុះតម្លៃ',
  'Promo Code': 'កូដប្រូម៉ូសិន',
  'Puffer Winter Vest': 'Puffer Winter Vest',
  'Pure Shine Conditioner': 'Pure Shine Conditioner',
  'Purple': 'ពណ៌ស្វាយ',
  'Purple Royal Heels': 'Purple Royal Heels',
  'Qty': 'Qty',
  'Qty: \${item[\'quantity\'] ?? 1}': 'Qty: \${item[\'quantity\'] ?? 1}',
  'Quantity': 'បរិមាណ',
  'Quarter-Zip Performance': 'Quarter-Zip Performance',
  'Quick-Dry Diamond Drops': 'Quick-Dry Diamond Drops',
  'Quick-Dry Swim Trunks': 'Quick-Dry Swim Trunks',
  'Quilted Designer Handbag': 'Quilted Designer Handbag',
  'Quilted Down Long Parka': 'Quilted Down Long Parka',
  'Quilted Gilet Vest': 'Quilted Gilet Vest',
  'Quilted Obsidian Puffer': 'Quilted Obsidian Puffer',
  'Racerback Ribbed Bralette': 'Racerback Ribbed Bralette',
  'Racing Biker Gloves': 'Racing Biker Gloves',
  'Rainbow Hair Tinsel': 'Rainbow Hair Tinsel',
  'Receive Address': 'អាសយដ្ឋានទទួល',
  'Receive Address not found': 'រកមិនឃើញអាសយដ្ឋានទទួល',
  'Recent Searches': 'ការស្វែងរកថ្មីៗ',
  'Recommend': 'ណែនាំសម្រាប់អ្នក',
  'Recycled Nylon Athleisure': 'Recycled Nylon Athleisure',
  'Red': 'ពណ៌ក្រហម',
  'Red Silk Clutch': 'កាបូបយួរដៃសូត្រពណ៌ក្រហម',
  'Red Suede Bow Loafers': 'Red Suede Bow Loafers',
  'Red V-Neck Essential': 'Red V-Neck Essential',
  'Red Velvet Stilettos': 'Red Velvet Stilettos',
  'Reflective Night-Run Crew': 'Reflective Night-Run Crew',
  'Reflective Track Jacket': 'Reflective Track Jacket',
  'Relaxed Fit Sand Wash': 'Relaxed Fit Sand Wash',
  'Remember me': 'ចងចាំខ្ញុំ',
  'Restorative Face Cream': 'Restorative Face Cream',
  'Restorative Silk Mask': 'Restorative Silk Mask',
  'Results for ': 'Results for ',
  'Results for "\${_searchController.text}"':
      'Results for "\${_searchController.text}"',
  'Retro 35mm Film Camera for Kids': 'Retro 35mm Film Camera for Kids',
  'Retro 90s High-Top Sneaker': 'Retro 90s High-Top Sneaker',
  'Retro 90s Stripe Polo': 'Retro 90s Stripe Polo',
  'Retro 90s Vibes': 'Retro 90s Vibes',
  'Retro Loose Fit Windbreaker': 'Retro Loose Fit Windbreaker',
  'Retro Oversized Joggers': 'Retro Oversized Joggers',
  'Retro Round Lenses': 'វ៉ែនតាកញ្ចក់មូលបែប Retro',
  'Retro Shoulder Bag with Gold Chain': 'Retro Shoulder Bag with Gold Chain',
  'Retro Street Wear': 'Retro Street Wear',
  'RETURNS': 'RETURNS',
  'Reusable Net Tote Bag': 'កាបូបសំណាញ់ដែលអាចប្រើឡើងវិញបាន',
  'Reversible Duo-Color': 'Reversible Duo-Color',
  '%a reviews': '%a ការវាយតម្លៃ',
  'Write a Review': 'សរសេរការវាយតម្លៃ',
  'Rating & Reviews': 'ការដាក់ពិន្ទុ និងការវាយតម្លៃ',
  'What is your rate?': 'តើអ្នកដាក់ពិន្ទុប៉ុន្មាន?',
  'Please share your opinion about the product':
      'សូមចែករំលែកមតិរបស់អ្នកអំពីផលិតផល',
  'Add Photo': 'បន្ថែមរូបភាព',
  'Submit Review': 'ដាក់បញ្ជូនការវាយតម្លៃ',
  'Your review has been submitted': 'ការវាយតម្លៃរបស់អ្នកត្រូវបានបញ្ជូន',
  'Product Reviews': 'ការវាយតម្លៃផលិតផល',
  'helpful': 'មានប្រយោជន៍',
  'Share': 'ចែករំលែក',
  'Rhinestone Tiara': 'Rhinestone Tiara',
  'Ribbed Cashmere Lounge Sock': 'Ribbed Cashmere Lounge Sock',
  'Ribbed Compression Tank': 'Ribbed Compression Tank',
  'Ribbed Cotton High-Waist': 'Ribbed Cotton High-Waist',
  'Ribbed Knit Beanie': 'មួកចាក់ឆ្នូត Ribbed Knit Beanie',
  'Ribbed Tank & Pant Duo': 'Ribbed Tank & Pant Duo',
  'Rimless Diamond Cut': 'វ៉ែនតាគ្មានស៊ុមគែមជម្រៀកពេជ្រ Rimless',
  'Rimless Minimalist': 'វ៉ែនតាគ្មានស៊ុមបែបសាមញ្ញ Rimless',
  'Rolex Submariner Gold': 'Rolex Submariner Gold',
  'Rose Bouquet': 'Rose Bouquet',
  'Rose Gold Fashion Luggage': 'វ៉ាលីម៉ូដទាន់សម័យពណ៌មាសផ្កាឈូក Rose Gold',
  'Rose Gold Hoop Earrings': 'ក្រវិលកងពណ៌មាសផ្កាឈូក Rose Gold',
  'Rose Gold Manicure Set': 'Rose Gold Manicure Set',
  'Rose Gold Pearl': 'Rose Gold Pearl',
  'Rose Petal Blush': 'Rose Petal Blush',
  'Rose Quartz Absolute': 'Rose Quartz Absolute',
  'Rose Quartz Gua Sha': 'Rose Quartz Gua Sha',
  'Rose Quartz Mist': 'Rose Quartz Mist',
  'Royal Blue Groom Suit': 'Royal Blue Groom Suit',
  'Royal Blue Satin': 'កាបូបសូត្ររលោងពណ៌ខៀវដិត Royal Blue',
  'Royal Blue Slim Fit': 'Royal Blue Slim Fit',
  'Royal Velvet Keepsake': 'Royal Velvet Keepsake',
  'Ruby Pendant Necklace': 'ខ្សែករខ្សែបន្ដោងត្បូងទទឹម Ruby Pendant',
  'Ruffled Chiffon Party Blouse': 'Ruffled Chiffon Party Blouse',
  'Ruffled Pastel Sunsuit': 'Ruffled Pastel Sunsuit',
  'Rugby Heavyweight Polo': 'Rugby Heavyweight Polo',
  'Rugged Field Watch': 'Rugged Field Watch',
  'Rugged Mountain Grip': 'Rugged Mountain Grip',
  'Rugged Outdoor Loafers': 'Rugged Outdoor Loafers',
  'Rugged Tactical Backpack': 'កាតាបស្ពាយយោធាធន់មាំ Rugged Tactical',
  'Rugged Tactical Pack': 'កាតាបយោធាធន់មាំ Rugged Tactical',
  'Rugged Work Flats': 'Rugged Work Flats',
  'Rustic Hunter Brown': 'Rustic Hunter Brown',
  'Rustic Hunter Leather': 'កាបូបស្បែកបែបផ្សងព្រេង Rustic Hunter',
  'Rustic Orange Polo': 'Rustic Orange Polo',
  'Rusty Brown Corduroy': 'Rusty Brown Corduroy',
  's': 's',
  'Safari Adventure Hat': 'មួកផ្សងព្រេងបែបសាផារី Safari Adventure',
  'Sage Green Cotton Jogger': 'Sage Green Cotton Jogger',
  'Sage Green Relaxed': 'Sage Green Relaxed',
  'Sage Green Summer Linen': 'Sage Green Summer Linen',
  'SALE': 'លក់បញ្ចុះតម្លៃ',
  'Sand Dune Tech Joggers': 'Sand Dune Tech Joggers',
  'Sandals': 'Sandals',
  'Sandstone Belted Wrap': 'Sandstone Belted Wrap',
  'Sandstone Cotton Suit': 'Sandstone Cotton Suit',
  'Sandy Beige Chino Polo': 'Sandy Beige Chino Polo',
  'Santal Bloom': 'Santal Bloom',
  'Sapphire Diver Pro': 'Sapphire Diver Pro',
  'Sapphire Drop Earrings': 'ក្រវិលត្បូងកណ្តៀង Sapphire Drop',
  'Satellite': 'ផ្កាយរណប',
  'Satin Bow Detail Bikini': 'Satin Bow Detail Bikini',
  'Satin Bow Stilettos': 'Satin Bow Stilettos',
  'Satin Cami & Short Set': 'Satin Cami & Short Set',
  'Satin Emerald Evening Dress': 'Satin Emerald Evening Dress',
  'Satin Evening Bag': 'កាបូបរាត្រីសមោសរសាច់សូត្ររលោង Satin',
  'Satin Finish Bronzer': 'Satin Finish Bronzer',
  'Satin Head Scarf': 'Satin Head Scarf',
  'Satin Ribbon Bow': 'Satin Ribbon Bow',
  'Satin Slip Skirt': 'Satin Slip Skirt',
  'Satin Strapless Multi-way': 'Satin Strapless Multi-way',
  'Satin Wrap Front Blouse': 'Satin Wrap Front Blouse',
  'Save Card': 'រក្សាទុកប័ណ្ណ',
  'Savings': 'ប្រាក់សន្សំ',
  'Scalloped Edge Unlined': 'Scalloped Edge Unlined',
  'Scalloped Edge Unlined Thong': 'Scalloped Edge Unlined Thong',
  'Scalp Detox Scrub': 'Scalp Detox Scrub',
  'Scarves': 'កន្សែងបង់ក',
  'Sculpting Face Wand': 'Sculpting Face Wand',
  'Seamless 360 Comfort Fit': 'Seamless 360 Comfort Fit',
  'Seamless Bamboo No-Show': 'Seamless Bamboo No-Show',
  'Seamless Comfort Wireless Bra': 'Seamless Comfort Wireless Bra',
  'Seamless Compression Shaper': 'Seamless Compression Shaper',
  'Seamless Invisible Thong': 'Seamless Invisible Thong',
  'Seamless Maternity Shaper': 'Seamless Maternity Shaper',
  'Seamless Mid-Thigh Short': 'Seamless Mid-Thigh Short',
  'Seamless Push Up Bralette': 'Seamless Push Up Bralette',
  'Seamless Workout Tights': 'Seamless Workout Tights',
  'Search': 'ស្វែងរក',
  'Search in': 'ស្វែងរកក្នុង',
  'Search in \$categoryName...': 'Search in \$categoryName...',
  'Search in Product item...': 'ស្វែងរកទំនិញ...',
  'Search in %a...': 'ស្វែងរកក្នុង %a...',
  'Search popular...': 'Search popular...',
  'Search products...': 'ស្វែងរកផលិតផល...',
  'Search shoes...': 'Search shoes...',
  'Search style products...': 'ស្វែងរកផលិតផលតាមម៉ូដ...',
  'Search toys...': 'Search toys...',
  'Search transactions...': 'ស្វែងរកប្រតិបត្តិការ...',
  'Search watches...': 'Search watches...',
  'Seaweed Cleansing Gel': 'Seaweed Cleansing Gel',
  'Security': 'សន្តិសុខ',
  'See All': 'មើលទាំងអស់',
  'SEE MORE': 'មើលបន្ថែម',
  'Select All': 'ជ្រើសរើសទាំងអស់',
  'Select Card Color': 'ជ្រើសរើសពណ៌ប័ណ្ណ',
  'Select Country': 'ជ្រើសរើសប្រទេស',
  'Select how you': 'Select how you',
  'Select how you\'d like to pay': 'Select how you\'d like to pay',
  'select language': 'ជ្រើសរើសភាសា',
  'Selvedge Premium Denim': 'Selvedge Premium Denim',
  'Service': 'សេវាកម្ម',
  'Set as default address': 'កំណត់ជាអាសយដ្ឋានលំនាំដើម',
  'Sets': 'Sets',
  'Sexy Boho Maxi Skirt': 'Sexy Boho Maxi Skirt',
  'Shadow Black Reflective': 'Shadow Black Reflective',
  'Shadow Black Stealth Boot': 'Shadow Black Stealth Boot',
  'Shapewear': 'Shapewear',
  'Share E-Receipt': 'ចែករំលែកបង្កាន់ដៃ',
  'Shea Butter Hand Relief': 'Shea Butter Hand Relief',
  'Shearling Trimmed Gilet': 'Shearling Trimmed Gilet',
  'Sheer Chiffon Robe': 'Sheer Chiffon Robe',
  'Sheer Chiffon Scarf': 'Sheer Chiffon Scarf',
  'Sheer Mesh Underwire': 'Sheer Mesh Underwire',
  'Sheer Nude Polish': 'Sheer Nude Polish',
  'Sheer Tulle Ankle Sock': 'Sheer Tulle Ankle Sock',
  'Sheer Tulle String Bikini': 'Sheer Tulle String Bikini',
  'Sheer-to-Waist Invisible': 'Sheer-to-Waist Invisible',
  'Shirts': 'Shirts',
  'Shoes': 'ស្បែកជើង',
  'Shop by collection': 'ទិញតាមការប្រមូល',
  'Shop by items': 'ទិញតាមមុខទំនិញ',
  'Short Sleeve Hawaiian': 'Short Sleeve Hawaiian',
  'Shorts': 'Shorts',
  'Sicilian Fig': 'Sicilian Fig',
  'Side-Slit Silk Skirt': 'Side-Slit Silk Skirt',
  'Signature Pique Polo': 'Signature Pique Polo',
  'Signup': 'ចុះឈ្មោះ',
  'Silicone-Grip Thigh Shaper': 'Silicone-Grip Thigh Shaper',
  'Silk Flower Fascinator': 'Silk Flower Fascinator',
  'Silk Foundation SPF 20': 'Silk Foundation SPF 20',
  'Silk Liner Gloves': 'Silk Liner Gloves',
  'Silk Satin Bikini Cut': 'Silk Satin Bikini Cut',
  'Silk Satin Kimono Robe': 'Silk Satin Kimono Robe',
  'Silk Sleep Mask & Pillowcase': 'Silk Sleep Mask & Pillowcase',
  'Silk V-Neck Office Blouse': 'Silk V-Neck Office Blouse',
  'Silk-Blend Embroidered Cut': 'Silk-Blend Embroidered Cut',
  'Silk-Blend Luxury Sheer': 'Silk-Blend Luxury Sheer',
  'Silk-Lined Wireless Bra': 'Silk-Lined Wireless Bra',
  'Silver Bangle Watch': 'នាឡិកាកងដៃពណ៌ប្រាក់ Silver Bangle',
  'Silver Charm Bundle': 'Silver Charm Bundle',
  'Silver Chronograph': 'Silver Chronograph',
  'Silver Glitter Heels': 'Silver Glitter Heels',
  'Silver Hard Shell': 'កាបូបសម្បកទន់រឹងពណ៌ប្រាក់',
  'Silver Skeleton Mechanical': 'Silver Skeleton Mechanical',
  'Silver Toning Shampoo': 'Silver Toning Shampoo',
  'Silver-Plated Photo Frame': 'Silver-Plated Photo Frame',
  'Similar items': 'ទំនិញប្រហាក់ប្រហែល',
  'Simple Leather Pouch': 'កាបូបដៃស្បែកសាមញ្ញ',
  'Simple Plastic Flip-Flops': 'Simple Plastic Flip-Flops',
  'Simple, durable, and spacious. Perfect for students and everyday use with reinforced straps and multiple zip pockets.':
      'សាមញ្ញ ធន់ និងធំទូលាយ។ ល្អឥតខ្ចោះសម្រាប់សិស្សានុសិស្ស និងការប្រើប្រាស់ប្រចាំថ្ងៃ ជាមួយខ្សែស្ពាយមាំ និងមានថតខ្សែរ៉ូតជាច្រើន។',
  'Simple, durable, and sustainable. This 100% organic cotton tote is perfect for daily grocery shopping or beach trips.':
      'សាមញ្ញ ធន់ និងមាននិរន្តរភាព។ កាបូបយួរធំផលិតពីកប្បាសសរីរាង្គ ១០០% នេះស័ក្តិសមបំផុតសម្រាប់ការទិញទំនិញប្រចាំថ្ងៃ ឬដើរលេងឆ្នេរខ្សាច់។',
  'Simply browse products, add them to your cart, and proceed to checkout using your preferred order method.':
      'គ្រាន់តែស្វែងរកផលិតផល បន្ថែមទៅក្នុងកន្ត្រក និងបន្ដទៅការទូទាត់តាមវិធីសាស្ត្រដែលអ្នកពេញចិត្ត។',
  'Size': 'ទំហំ',
  'Size guide': 'ការណែនាំអំពីទំហំ',
  'Ski Snowboard Gloves': 'Ski Snowboard Gloves',
  'Skincare': 'Skincare',
  'Skirt': 'Skirt',
  'Skirts': 'Skirts',
  'Sky Blue Mohair Blend': 'Sky Blue Mohair Blend',
  'Sky Blue Stretch Polo': 'Sky Blue Stretch Polo',
  'Slate Blue Satchel': 'កាបូបយួរដៃពណ៌ខៀវប្រផេះ Slate Blue',
  'Slate Grey Checkered Suit': 'Slate Grey Checkered Suit',
  'Sleek All-Black Stealth': 'កាតាបខ្មៅរលោងម៉ូដ Stealth',
  'Sleek and stealthy. This all-black watch is perfect for both formal events and everyday luxury.':
      'Sleek and stealthy. This all-black watch is perfect for both formal events and everyday luxury.',
  'Sleek Black Matte': 'Sleek Black Matte',
  'Sleek Italian Loafers': 'Sleek Italian Loafers',
  'Sleek Maroon Office Tote': 'កាបូបការិយាល័យពណ៌ក្រហមឈាមជ្រូករលោង',
  'Sleek Maroon Purse': 'កាបូបដៃពណ៌ក្រហមឈាមជ្រូករលោង',
  'Sleek Metal Money Clip': 'ដង្កៀបគាបលុយលោហៈរលោង',
  'Sleek Midnight Black': 'Sleek Midnight Black',
  'Sleek Minimalist Crossbody': 'កាបូបស្ពាយចំហៀងរាងស្តើងសាមញ្ញ',
  'Sleep and powerful, the Stealth Black series is made for those who move in style.':
      'Sleep and powerful, the Stealth Black series is made for those who move in style.',
  'Sleeveless Pleated Blouse': 'Sleeveless Pleated Blouse',
  'Slim Dress Belt': 'Slim Dress Belt',
  'Slim Fit Black Shirt': 'Slim Fit Black Shirt',
  'Slim Fit Dark Indigo': 'Slim Fit Dark Indigo',
  'Slim Fit Navy Polo': 'Slim Fit Navy Polo',
  'Slip-on Leather Mules': 'Slip-on Leather Mules',
  'Slippers': 'Slippers',
  'Slit Front Maxi': 'Slit Front Maxi',
  'Smart Aromatherapy Diffuser': 'Smart Aromatherapy Diffuser',
  'Smart Fitness Band': 'Smart Fitness Band',
  'Smart Series 9 Pro': 'Smart Series 9 Pro',
  'Smart Watch': 'Smart Watch',
  'Smart Watch Gift': 'Smart Watch Gift',
  'Smocked Waist Floral Top': 'Smocked Waist Floral Top',
  'Smoked Vanilla Elixir': 'Smoked Vanilla Elixir',
  'Smoothing Plunge Bodice': 'Smoothing Plunge Bodice',
  'Snapback Sports Cap': 'មួកកីឡាប៊ូតុងចុច Snapback Sports',
  'Sneakers': 'Sneakers',
  'Socks': 'Socks',
  'Soft Cloud Platform Sandals': 'Soft Cloud Platform Sandals',
  'Soft Comfort Walking Flats': 'Soft Comfort Walking Flats',
  'Soft Lavender Occasion': 'Soft Lavender Occasion',
  'Soft Nubuck Belt': 'Soft Nubuck Belt',
  'Soft Pastel Pink Tote': 'កាបូបយួរធំពណ៌ផ្កាឈូកស្រទន់ Soft Pastel',
  'Soft Pebble Grain Wallet': 'កាបូបលុយស្បែកគ្រាប់ Soft Pebble',
  'Soft Pink Suede Mocs': 'Soft Pink Suede Mocs',
  'Soft Plush Indoor Slippers': 'Soft Plush Indoor Slippers',
  'Soft Ribbed V-Neck': 'Soft Ribbed V-Neck',
  'Soft Suede Square Messenger': 'Soft Suede Square Messenger',
  'Soft Suede Tote': 'កាបូបយួរធំស្បែកសត្វទន់ Soft Suede',
  'Soft Touch Lavender': 'Soft Touch Lavender',
  'Soft Touch Pastel Mint': 'Soft Touch Pastel Mint',
  'Soft Touch Pastel Pink': 'Soft Touch Pastel Pink',
  'Soft-Cup Lace Teddy': 'Soft-Cup Lace Teddy',
  'Soft-Touch Chenille Slipper': 'Soft-Touch Chenille Slipper',
  'Solar Powered Tech': 'Solar Powered Tech',
  'sold': 'លក់ដាច់',
  'Sonic Silicone Cleanser': 'Sonic Silicone Cleanser',
  'Sonic Wave Sneakers': 'Sonic Wave Sneakers',
  'Sort by': 'តម្រៀបតាម',
  'Sound': 'សំឡេង',
  'Sparkling collection coming soon': 'Sparkling collection coming soon',
  'Sparkly Party Skirt': 'Sparkly Party Skirt',
  'SPECIAL OFFERS': 'SPECIAL OFFERS',
  'Special Offers': 'ការផ្តល់ជូនពិសេស',
  'Special promotion only valid today':
      'ប្រូម៉ូសិនពិសេសនេះមានសុពលភាពតែថ្ងៃនេះប៉ុណ្ណោះ',
  'Specialized yellow-tinted lenses designed to reduce glare and improve clarity during night driving or low-light conditions.':
      'កញ្ចក់ពណ៌លឿងពិសេស ឌីហ្សាញឡើងដើម្បីកាត់បន្ថយពន្លឺចាំង និងបង្កើនភាពច្បាស់លាស់ក្នុងការបើកបរពេលយប់ ឬក្នុងលក្ខខណ្ឌពន្លឺតិច។',
  'Speed Bolt Edition': 'Speed Bolt Edition',
  'Spend \$160+ and enjoy Discount 15% + FREE Delivery!':
      'Spend \$160+ and enjoy Discount 15% + FREE Delivery!',
  'Sport': 'កីឡា',
  'Sport Digital Watch': 'Sport Digital Watch',
  'Sport Fitness Gloves': 'Sport Fitness Gloves',
  'Sport Performance Polo': 'Sport Performance Polo',
  'Sport Polarized': 'វ៉ែនតាកីឡាកញ្ចក់ Polarized',
  'Sports': 'Sports',
  'Sporty Gym Travel Bag': 'កាបូបកីឡានិងធ្វើដំណើរ Sporty Gym',
  'Sporty Lite Flats': 'Sporty Lite Flats',
  'Spring Collection': 'ការប្រមូលម៉ូដរដូវផ្ការីក',
  'Sprint Master V2': 'Sprint Master V2',
  'Square Oversized': 'វ៉ែនតារាងការ៉េខ្នាតធំ Square Oversized',
  'Squat-proof, non-see-through fabric with a hidden waistband pocket. Perfect for yoga, gym sessions, or casual athleisure.':
      'Squat-proof, non-see-through fabric with a hidden waistband pocket. Perfect for yoga, gym sessions, or casual athleisure.',
  'Stability Plus Sport': 'Stability Plus Sport',
  'Stand out from the crowd with the Neon Speedster, optimized for high-speed training.':
      'Stand out from the crowd with the Neon Speedster, optimized for high-speed training.',
  'Star Motif Mesh Hosiery': 'Star Motif Mesh Hosiery',
  'Star Wars: The Darksaber Replica': 'Star Wars: The Darksaber Replica',
  'Star-Print Mesh Chemise': 'Star-Print Mesh Chemise',
  'Starfish Summer Clip': 'Starfish Summer Clip',
  'Start Shopping': 'Start Shopping',
  'Start shopping': 'ចាប់ផ្តើមទិញទំនិញ',
  'Status': 'Status',
  'Stay cozy with this soft cashmere blend wrap. Lightweight yet incredibly warm for those chilly winter days.':
      'Stay cozy with this soft cashmere blend wrap. Lightweight yet incredibly warm for those chilly winter days.',
  'Stay-Up Lace Top Hold-Ups': 'Stay-Up Lace Top Hold-Ups',
  'Steampunk Round': 'វ៉ែនតាមូលរចនាបថ Steampunk',
  'Store up to 6 watches in style. Soft velvet lining to prevent scratches.':
      'Store up to 6 watches in style. Soft velvet lining to prevent scratches.',
  'Stories': 'Stories',
  'Straight Leg Raw Denim': 'Straight Leg Raw Denim',
  'Strapless Shaping Slip': 'Strapless Shaping Slip',
  'Straw Boater Hat': 'មួកចំបើងរាងមូល Straw Boater Hat',
  'Streetwear': 'ស្ទីល Streetwear',
  'Streetwear Letter Embroidered Jeans': 'Streetwear Letter Embroidered Jeans',
  'Streetwear Multi-Pocket Cargo': 'Streetwear Multi-Pocket Cargo',
  'Streetwear-inspired graphic tee featuring a vintage wash and high-quality screen print.':
      'Streetwear-inspired graphic tee featuring a vintage wash and high-quality screen print.',
  'Striking color and luxurious texture. Features a comfortable 85mm stiletto heel.':
      'Striking color and luxurious texture. Features a comfortable 85mm stiletto heel.',
  'Striped Beach Shorts': 'Striped Beach Shorts',
  'Striped Business Shirt': 'Striped Business Shirt',
  'Striped Casual Linen Blouse': 'Striped Casual Linen Blouse',
  'Striped Cotton Shirtdress': 'Striped Cotton Shirtdress',
  'Striped Cotton Slippers': 'Striped Cotton Slippers',
  'Striped Nautical Cardigan': 'Striped Nautical Cardigan',
  'Striped Nautical Tee': 'Striped Nautical Tee',
  'Striped Piped Pajama Set': 'Striped Piped Pajama Set',
  'Striped Retro Athletic': 'Striped Retro Athletic',
  'Structured Onyx Waistcoat': 'Structured Onyx Waistcoat',
  'Structured Slate Overcoat': 'Structured Slate Overcoat',
  'Studio-grade sound quality with genuine leather earcups and gold accents.':
      'Studio-grade sound quality with genuine leather earcups and gold accents.',
  'Style': 'ម៉ូដ',
  'STYLE WEEK': 'សប្តាហ៍នៃស្ទីលទាន់សម័យ',
  'Styles': 'ម៉ូដ',
  'Stylish Bucket Hat': 'មួកធុងមានស្ទីលទាន់សម័យ Stylish Bucket',
  'Subtotal': 'តម្លៃសរុបបណ្តោះអាសន្ន',
  'Subtotal Amount:': 'ចំនួនទឹកប្រាក់សរុបបណ្តោះអាសន្ន:',
  'Suede Penny Loafers': 'Suede Penny Loafers',
  'Suede Sahara Vest': 'Suede Sahara Vest',
  'Suede Stylish Belt': 'Suede Stylish Belt',
  'Suede Trucker Jacket': 'Suede Trucker Jacket',
  'Suede Urban Ankle Boot': 'Suede Urban Ankle Boot',
  'Suits': 'Suits',
  'Summer Beach Slippers': 'Summer Beach Slippers',
  'Summer Breathable Loafers': 'Summer Breathable Loafers',
  'Summer Collection': 'ការប្រមូលម៉ូដរដូវក្តៅ',
  'Summer collection coming soon': 'Summer collection coming soon',
  'Summer Cotton Cargo Shorts': 'Summer Cotton Cargo Shorts',
  'Summer Holiday Cropped Vest': 'Summer Holiday Cropped Vest',
  'Summer Straw Beach Tote':
      'កាបូបយួរធំក្រណាត់កន្ត្រាំងសម្រាប់ដើរលេងឆ្នេររដូវក្តៅ',
  'Summit Peak Hiking Explorer': 'Summit Peak Hiking Explorer',
  'Sun-Bleached Red Hoodie': 'Sun-Bleached Red Hoodie',
  'Sunglasses': 'វ៉ែនតាការពារកម្តៅថ្ងៃ',
  'Sunshine Yellow Casual': 'Sunshine Yellow Casual',
  'Super-Size Snow Tube Sled': 'Super-Size Snow Tube Sled',
  'Supportive Shaping Tights': 'Supportive Shaping Tights',
  'Sweatshirts': 'Sweatshirts',
  'Sweetheart Neckline Push-Up': 'Sweetheart Neckline Push-Up',
  'T&Cs APPLY': 'លក្ខខណ្ឌត្រូវបានអនុវត្ត',
  'T-Shirts': 'T-Shirts',
  'T-Strap Summer Flats': 'T-Strap Summer Flats',
  'Tactical EDC Wallet': 'កាបូបលុយប្រើប្រាស់ប្រចាំថ្ងៃបែបយោធា Tactical EDC',
  'Tactical Tech Bag': 'កាបូបបច្ចេកវិទ្យាបែបយោធា Tactical Tech',
  'Tactical Techwear Vest': 'Tactical Techwear Vest',
  'Tailored to perfection. A masterpiece of sartorial elegance for gala events.':
      'Tailored to perfection. A masterpiece of sartorial elegance for gala events.',
  'Tambour Street Diver Burning Rock': 'Tambour Street Diver Burning Rock',
  'Tan Linen Wedding Slacks': 'Tan Linen Wedding Slacks',
  'Tan Slip-on Loafers': 'Tan Slip-on Loafers',
  'Tapered Ankle Denim': 'Tapered Ankle Denim',
  'Targeted Tummy Control Band': 'Targeted Tummy Control Band',
  'Teal Ocean Active Polo': 'Teal Ocean Active Polo',
  'Tech': 'បច្ចេកវិទ្យា',
  'TECH DEALS': 'ការផ្តល់ជូនផ្នែកបច្ចេកវិទ្យា',
  'Tech Fleece Hoodie': 'Tech Fleece Hoodie',
  'Tech Fleece Performance': 'Tech Fleece Performance',
  'Tech-Knit Compression Bra': 'Tech-Knit Compression Bra',
  'Technical Windbreaker Y2K': 'Technical Windbreaker Y2K',
  'Techwear Messenger Bag': 'Techwear Messenger Bag',
  'Telegram': 'តេឡេក្រាម',
  'Terra Eyeshadow Palette': 'Terra Eyeshadow Palette',
  'Terrain': 'ភូមិសាស្ត្រ',
  'Texture Wave Spray': 'Texture Wave Spray',
  'Textured Geometric Pattern': 'Textured Geometric Pattern',
  'Textured Waffle Polo': 'Textured Waffle Polo',
  'The icon of eyewear. Always in style, no matter the decade.':
      'The icon of eyewear. Always in style, no matter the decade.',
  'The quintessential divers watch, the Submariner set the standard for the genre. A perfect blend of luxury and utility with 18k gold finish.':
      'The quintessential divers watch, the Submariner set the standard for the genre. A perfect blend of luxury and utility with 18k gold finish.',
  'Thermal Fleece Baselayer': 'Thermal Fleece Baselayer',
  'Thermal Fleece-Lined Winter': 'Thermal Fleece-Lined Winter',
  'Thermal Protectant Mist': 'Thermal Protectant Mist',
  'Thermal Winter Gloves': 'Thermal Winter Gloves',
  'Tie-Dye Cotton Scarf': 'Tie-Dye Cotton Scarf',
  'Tie-Dye Dream Hoodie': 'Tie-Dye Dream Hoodie',
  'Tie-Dye Summer Vibes': 'Tie-Dye Summer Vibes',
  'Tie-Dye Ultimate Creation Station': 'Tie-Dye Ultimate Creation Station',
  'Tiered Boho Skirt': 'Tiered Boho Skirt',
  'Tiered Ruffle Sleeve Top': 'Tiered Ruffle Sleeve Top',
  'Tights': 'Tights',
  'Timeless aviator design with polarized lenses for 100% UV protection. Lightweight metal frame for all-day comfort.':
      'វ៉ែនតារចនាបថ Aviator មិនចេះហួសសម័យ ជាមួយកញ្ចក់ Polarized ការពារកាំរស្មី UV 100%។ ស៊ុមលោហៈទម្ងន់ស្រាល ផ្តល់ផាសុកភាពពេញមួយថ្ងៃ។',
  'Timeless elegance meets precision engineering. Features a stainless steel case and sapphire crystal.':
      'Timeless elegance meets precision engineering. Features a stainless steel case and sapphire crystal.',
  'Titanium Lightweight': 'Titanium Lightweight',
  'Titanium Series Carry-On': 'វ៉ាលីយួរឡើងយន្តហោះស៊េរី Titanium',
  'Titanium Ultra-Light': 'វ៉ែនតាស៊ុម Titanium ស្រាលពិសេស Ultra-Light',
  'Top Sale': 'Top Sale',
  'Top Up': 'បញ្ចូលលុយ',
  'Top Up E-Wallet Successful!': 'ការបញ្ចូលលុយក្នុងកាបូបអេឡិចត្រូនិកបានជោគជ័យ!',
  'Total': 'តម្លៃសរុប',
  'Total amount:': 'ចំនួនទឹកប្រាក់សរុប:',
  'Total price': 'តម្លៃសរុប',
  'Tote': 'កាបូបយួរធំ (Tote)',
  'Toy Story: Slinky Dog Bookends': 'Toy Story: Slinky Dog Bookends',
  'Toys': 'ប្រដាប់ក្មេងលេង',
  'Trail Blazer Hiking': 'Trail Blazer Hiking',
  'Trail Blazer X1': 'Trail Blazer X1',
  'Transaction History': 'ប្រវត្តិប្រតិបត្តិការ',
  'Transaction ID': 'លេខសម្គាល់ប្រតិបត្តិការ',
  'Transparent Jelly Handbag': 'Transparent Jelly Handbag',
  'Travel': 'ការធ្វើដំណើរ',
  'Travel Passport Wallet': 'កាបូបលុយដាក់លិខិតឆ្លងដែនសម្រាប់ធ្វើដំណើរ',
  'Trend-focused oversized fit with dropped shoulders. Perfect for a cozy, stylish look.':
      'Trend-focused oversized fit with dropped shoulders. Perfect for a cozy, stylish look.',
  'Tropical Beach Flip-Flops': 'Tropical Beach Flip-Flops',
  'Tummy-Taming Brief': 'Tummy-Taming Brief',
  'Turquoise Beach Tee': 'Turquoise Beach Tee',
  'Turquoise Cuff Bracelet': 'កងដៃចំហពណ៌ខៀវទំពាំងបាយជូរ Turquoise',
  'Turquoise Velvet Heels': 'Turquoise Velvet Heels',
  'Tweed Workwear Set': 'Tweed Workwear Set',
  'Type your password': 'Type your password',
  'Ultra-Fast Pro Runner': 'Ultra-Fast Pro Runner',
  'Ultra-Gentle Dry Shampoo': 'Ultra-Gentle Dry Shampoo',
  'Ultra-Light Cooling Slip': 'Ultra-Light Cooling Slip',
  'Ultra-lightweight and RFID blocking. The ultimate minimalist accessory.':
      'Ultra-lightweight and RFID blocking. The ultimate minimalist accessory.',
  'Ultra-Sheer 15D Glossy': 'Ultra-Sheer 15D Glossy',
  'Ultra-soft fleece joggers in a crisp arctic white, perfect for lounging or winter runs.':
      'Ultra-soft fleece joggers in a crisp arctic white, perfect for lounging or winter runs.',
  'Ultra-Thick Fleece Cabin': 'Ultra-Thick Fleece Cabin',
  'Under-Bust Waist Nipper': 'Under-Bust Waist Nipper',
  'Unisex': 'Unisex',
  'Unparalleled warmth and softness. Made from 100% pure Himalayan cashmere.':
      'Unparalleled warmth and softness. Made from 100% pure Himalayan cashmere.',
  'UP TO 50%': 'រហូតដល់ 50%',
  'Up to 50%': 'រហូតដល់ 50%',
  'Up to 70% Off': 'បញ្ចុះតម្លៃរហូតដល់ 70%',
  'Update Address': 'ធ្វើបច្ចុប្បន្នភាពអាសយដ្ឋាន',
  'Urban Athletic Low': 'Urban Athletic Low',
  'Urban Baseball Cap': 'មួកប៉ារ៉ាកែបទីក្រុង Urban Baseball',
  'Urban Black Polo': 'Urban Black Polo',
  'Urban Bomber Jacket': 'Urban Bomber Jacket',
  'Urban Canvas Satchel': 'កាបូបស្ពាយក្រណាត់កង់វ៉ាស់ទីក្រុង',
  'Urban Cargo Olive': 'Urban Cargo Olive',
  'Urban Cargo Utility': 'Urban Cargo Utility',
  'Urban Casual Brown': 'Urban Casual Brown',
  'Urban Comfort Walking Sneaker': 'Urban Comfort Walking Sneaker',
  'Urban Commuter Laptop Bag': 'Urban Commuter Laptop Bag',
  'Urban Essential Joggers in Matte Black':
      'Urban Essential Joggers in Matte Black',
  'Urban Explorer Backpack': 'កាតាបស្ពាយទីក្រុង Urban Explorer',
  'Urban Explorer Pro': 'កាតាបទីក្រុង Urban Explorer Pro',
  'Urban Explorer Sneakers': 'Urban Explorer Sneakers',
  'Urban Grey Slim-Fit': 'Urban Grey Slim-Fit',
  'Urban High-Top Sneakers': 'Urban High-Top Sneakers',
  'Urban Street Goggles': 'វ៉ែនតារចនាបថផ្លូវដើរទីក្រុង Urban Street',
  'Urban Street Loafers': 'Urban Street Loafers',
  'Urban Streetwear Slides': 'Urban Streetwear Slides',
  'Urban Style Handbag': 'កាបូបយួរដៃរចនាបថទីក្រុង Urban Style',
  'Urban Tactical Joggers': 'Urban Tactical Joggers',
  'Urban Tech Shell Coat': 'Urban Tech Shell Coat',
  'Urban Tech Shorts': 'Urban Tech Shorts',
  'V-Neck Chiffon Blouse': 'V-Neck Chiffon Blouse',
  'Valentine': 'Valentine',
  'Valentine Special Pack': 'Valentine Special Pack',
  'Valid Thru': 'សុពលភាពដល់',
  'VALID THRU': 'សុពលភាពដល់',
  'Varsity Letterman Jacket': 'Varsity Letterman Jacket',
  'Velocity X-Stream': 'Velocity X-Stream',
  'Velvet Cocktail Dress': 'Velvet Cocktail Dress',
  'Velvet Envelope Clutch': 'កាបូបយួរដៃម៉ូដស្រោមសំបុត្រសាច់វល្លិ៍ Velvet',
  'Velvet Evening Blouse': 'Velvet Evening Blouse',
  'Velvet Evening Scarf': 'Velvet Evening Scarf',
  'Velvet Evening Skirt': 'Velvet Evening Skirt',
  'Velvet Luxury Slippers': 'Velvet Luxury Slippers',
  'Velvet Matte Lipstick': 'Velvet Matte Lipstick',
  'Velvet Rose Bouquet Box': 'Velvet Rose Bouquet Box',
  'Velvet Suede Lacquer': 'Velvet Suede Lacquer',
  'Velvet Touch Soft Opaque': 'Velvet Touch Soft Opaque',
  'Velvet Tracksuit Set': 'Velvet Tracksuit Set',
  'Velvet Trim Luxury Brief': 'Velvet Trim Luxury Brief',
  'Velvet Trim Nightgown': 'Velvet Trim Nightgown',
  'Velvet Trim Sleep Sock': 'Velvet Trim Sleep Sock',
  'Velvet Two-Piece Tracksuit Set': 'Velvet Two-Piece Tracksuit Set',
  'Versatile shoes designed for high-intensity interval training and gym workouts.':
      'Versatile shoes designed for high-intensity interval training and gym workouts.',
  'Vertical Stripe Slimming': 'Vertical Stripe Slimming',
  'Vetiver Noir': 'Vetiver Noir',
  'Vibrate': 'រំញ័រ',
  'View Cart': 'មើលកន្ត្រកទំនិញ',
  'Vintage': 'ស្ទីល Vintage',
  'Vintage Athletic Red': 'Vintage Athletic Red',
  'Vintage Black Graphic': 'Vintage Black Graphic',
  'Vintage Brass Compass': 'Vintage Brass Compass',
  'Vintage Brooch Pin': 'ម្ជុលខ្ទាស់អាវម៉ូដ Vintage',
  'Vintage Brown Buckle Sandals': 'Vintage Brown Buckle Sandals',
  'Vintage Brown Suede Tote': 'កាបូបយួរធំស្បែកសត្វពណ៌ត្នោតម៉ូដ Vintage',
  'Vintage Buckle Belt': 'Vintage Buckle Belt',
  'Vintage Camel Hair Coat': 'Vintage Camel Hair Coat',
  'Vintage Canvas Rucksack': 'កាតាបក្រណាត់កង់វ៉ាស់ម៉ូដ Vintage',
  'Vintage Denim Trucker': 'Vintage Denim Trucker',
  'Vintage Fashion Cotton Tee': 'Vintage Fashion Cotton Tee',
  'Vintage Fedora Hat': 'មួកហ្វេដូរ៉ាម៉ូដបុរាណ Vintage Fedora',
  'Vintage Floral Clutch': 'កាបូបយួរដៃផ្កាម៉ូដ Vintage',
  'Vintage Half-Rim': 'វ៉ែនតាស៊ុមពាក់កណ្តាលម៉ូដ Vintage',
  'Vintage Leather Journal': 'Vintage Leather Journal',
  'Vintage Leather Square Bag': 'Vintage Leather Square Bag',
  'Vintage Leather Square Satchel': 'Vintage Leather Square Satchel',
  'Vintage Leather Watch Box': 'Vintage Leather Watch Box',
  'Vintage Light Wash Denim': 'Vintage Light Wash Denim',
  'Vintage Mechanical': 'Vintage Mechanical',
  'Vintage Rose Appliqué': 'Vintage Rose Appliqué',
  'Vintage Rugged Messenger': 'កាបូបស្ពាយចំហៀងធន់មាំម៉ូដ Vintage',
  'Vintage Satin Tap Panty': 'Vintage Satin Tap Panty',
  'Vintage Silk Shirt': 'Vintage Silk Shirt',
  'Vintage Stonewash Denim': 'Vintage Stonewash Denim',
  'Vintage Suede Loafers': 'Vintage Suede Loafers',
  'Vintage Sweat Shorts': 'Vintage Sweat Shorts',
  'Vintage Tan Satchel': 'កាបូបយួរដៃពណ៌ត្នោតខ្ចីម៉ូដ Vintage',
  'Vintage Tan Travel Case': 'ប្រអប់ធ្វើដំណើរពណ៌ត្នោតខ្ចីម៉ូដ Vintage',
  'Vintage Tan Zipper Wallet': 'កាបូបលុយមានខ្សែរ៉ូតពណ៌ត្នោតខ្ចីម៉ូដ Vintage',
  'Vintage Tortoise': 'វ៉ែនតាពណ៌សម្បកអណ្តើកម៉ូដ Vintage',
  'Vintage Velvet Scrunchies': 'Vintage Velvet Scrunchies',
  'Vintage Wash Blue Jogger': 'Vintage Wash Blue Jogger',
  'Vintage Wash Polo': 'Vintage Wash Polo',
  'Vintage Wash Street Hoodie': 'Vintage Wash Street Hoodie',
  'Vintage Wingtip Brogue Shoe': 'Vintage Wingtip Brogue Shoe',
  'Vitamin E Repair Balm': 'Vitamin E Repair Balm',
  'Volcanic Clay Mask': 'Volcanic Clay Mask',
  'Volt Energy Sneakers': 'Volt Energy Sneakers',
  'Volume Lift Mascara': 'Volume Lift Mascara',
  'Volumizing Root Lift': 'Volumizing Root Lift',
  'Waffle-Knit Spa Robe': 'Waffle-Knit Spa Robe',
  'Waist-Cinched Mesh Corset': 'Waist-Cinched Mesh Corset',
  'Wallet': 'Wallet',
  'Wallets': 'កាបូបលុយ',
  'Warm Winter Fur Slippers': 'Warm Winter Fur Slippers',
  'Watch': 'នាឡិកាដៃ',
  'Watches': 'នាឡិកាដៃ',
  'Waterproof City Trench': 'Waterproof City Trench',
  'Waterproof Sport Bag': 'កាបូបកីឡាមិនជ្រាបទឹក',
  'Waterproof Sport Cross': 'កាបូបស្ពាយចំហៀងកីឡាមិនជ្រាបទឹក',
  'Wayfarer Style': 'វ៉ែនតារចនាបថ Wayfarer Style',
  'Website': 'គេហទំព័រ',
  'Wedding': 'Wedding',
  'Wedding Party Team Bride Robe': 'Wedding Party Team Bride Robe',
  'WEEKEND DEAL': 'ការផ្តល់ជូនពិសេសចុងសប្តាហ៍',
  'Weightless Hair Oil': 'Weightless Hair Oil',
  'Western Stitch Leather': 'Western Stitch Leather',
  'What are you searching for?': 'តើអ្នកកំពុងស្វែងរកអ្វី?',
  'What is Evira?': 'តើអ្វីទៅជា Evira?',
  'White': 'ពណ៌ស',
  'White Lace Wedding Heels': 'White Lace Wedding Heels',
  'White Musk & Linen': 'White Musk & Linen',
  'Wigglitz: Collectible Food Fidget Series':
      'Wigglitz: Collectible Food Fidget Series',
  'Wild West Cowboy Leather Boot': 'Wild West Cowboy Leather Boot',
  'Winter collection coming soon': 'Winter collection coming soon',
  'Wireless Bamboo Charger': 'Wireless Bamboo Charger',
  'Wireless Earbuds': 'Wireless Earbuds',
  'Wireless Keyboard': 'Wireless Keyboard',
  'Wishlist': 'Wishlist',
  'wishlist': 'បញ្ជីបំណងប្រាថ្នា',
  'Within 14 days': 'Within 14 days',
  'Women': 'Women',
  'Women Elegant Stone': 'Women Elegant Stone',
  'Wooden Bamboo Frame': 'វ៉ែនតាស៊ុមឈើឫស្សីធម្មជាតិ',
  'Wool Blend Peacoat': 'Wool Blend Peacoat',
  'Wool Blend Varsity Cardigan': 'Wool Blend Varsity Cardigan',
  'Wool Newsboy Cap': 'មួកអ្នកលក់កាសែតសាច់រោមសត្វ Wool Newsboy',
  'Woolen Cozy Mittens': 'Woolen Cozy Mittens',
  'Workaholic Laptop Tote': 'កាបូបយួរធំដាក់កុំព្យូទ័រសម្រាប់អ្នកខំប្រឹងធ្វើការ',
  'Workwear Double Knee': 'Workwear Double Knee',
  'Wrap Front Floral Midi': 'Wrap Front Floral Midi',
  'Wrap Front Skirt': 'Wrap Front Skirt',
  'Y2K': 'ស្ទីល Y2K',
  'Yellow': 'ពណ៌លឿង',
  'Yes, downloading and browsing Evira is completely free.':
      'បាទ/ចាស ការទាញយក និងការស្វែងរកផលិតផលក្នុង Evira គឺឥតគិតថ្លៃទាំងស្រុង។',
  'Yoga Flow Light Support': 'Yoga Flow Light Support',
  'You can cancel your order from the \'My Orders\' section within 30 minutes of purchase.':
      'You can cancel your order from the \'My Orders\' section within 30 minutes of purchase.',
  'You have no recent searches.': 'អ្នកមិនទាន់មានការស្វែងរកថ្មីៗនៅឡើយទេ។',
  'You have to top up your e-wallet':
      'អ្នកត្រូវបញ្ចូលលុយក្នុងកាបូបអេឡិចត្រូនិករបស់អ្នក',
  'Your account has been created!': 'គណនីរបស់អ្នកត្រូវបានបង្កើតរួចរាល់!',
  'Your bag is empty': 'កាបូបរបស់អ្នកនៅទំនេរ',
  'Z.HOME': 'ទំព័រដើម',
  'Zip-Up Modern Polo': 'Zip-Up Modern Polo',
  'Zipped Cuff Tech Pants': 'Zipped Cuff Tech Pants',
  'ZipString Aracna - Kinetic String Toy':
      'ZipString Aracna - Kinetic String Toy',
  'Zoom Pro Elite Running Shoe': 'Zoom Pro Elite Running Shoe',
  '{0} items found': '{0} items found',
  '%a sold': '%a លក់ដាច់',
  '|': '|',
  '| {0} sold': '| {0} sold',
  'គិតថ្លៃដឹក 1\$ សម្រាប់ការបញ្ជាទិញក្រោម 10\$':
      'គិតថ្លៃដឹក 1\$ សម្រាប់ការបញ្ជាទិញក្រោម 10\$',
  'A timeless Harrington jacket featuring a clean silhouette and premium lining, perfect for an "Old Money" look.':
      'A timeless Harrington jacket featuring a clean silhouette and premium lining, perfect for an "Old Money" look.',
  'Elegant Women\'': 'Elegant Women\', // TODO: Translate to Khmer',
  'Hand-Operated Mini Drone (UFO)': 'Hand-Operated Mini Drone (UFO)',
  'Magic Chemistry Set (50 Experiments)':
      'Magic Chemistry Set (50 Experiments)',
  'Men\'': 'Men\', // TODO: Translate to Khmer',
  'Premium Yoga Mat (Pro)': 'Premium Yoga Mat (Pro)',
  'Premium raw denim with a classic straight cut, perfect for an "Old Money" aesthetic.':
      'Premium raw denim with a classic straight cut, perfect for an "Old Money" aesthetic.',
  'Squishy Foam "Donut" Squeeze Set': 'Squishy Foam "Donut" Squeeze Set',
  'OR': 'ឬ',
  'Phone Number': 'លេខទូរសព្ទ',
  'Please enter both phone and password':
      'សូមបញ្ចូលទាំងលេខទូរសព្ទ និងពាក្យសម្ងាត់',
  'Register': 'ចុះឈ្មោះ',
  'Registration successful!': 'ការចុះឈ្មោះទទួលបានជោគជ័យ!',
  'Please enter email, password and phone':
      'សូមបញ្ចូលអ៊ីមែល ពាក្យសម្ងាត់ និងលេខទូរសព្ទ',
  'Continue with Phone': 'បន្តជាមួយលេខទូរសព្ទ',
  'Continue with Email': 'បន្តជាមួយអ៊ីមែល',
  'Continue with Apple': 'បន្តជាមួយ Apple',
  'Sign Up': 'ចុះឈ្មោះ',
  'Sign Up here': 'ចុះឈ្មោះនៅទីនេះ',
  'Secure Payment Methods': 'វិធីសាស្ត្រទូទាត់ប្រកបដោយសុវត្ថិភាព',
  'Registration successful! Please check your email for verification.':
      'ការចុះឈ្មោះជោគជ័យ! សូមពិនិត្យអ៊ីមែលរបស់អ្នកសម្រាប់ការផ្ទៀងផ្ទាត់។',
  'Please fill in all fields': 'សូមបំពេញគ្រប់ចន្លោះ',
  'First name': 'នាមខ្លួន',
  'Last name': 'នាមត្រកូល',
  'Mobile number': 'លេខទូរសព្ទ',
  'Add contact number': 'បន្ថែមលេខទំនាក់ទំនង',
  'Date of birth (DD/MM/YYYY)': 'ថ្ងៃខែឆ្នាំកំណើត (ថ្ងៃ/ខែ/ឆ្នាំ)',
  'Add your birthday to unlock additional offering/reward!':
      'បន្ថែមថ្ងៃកំណើតរបស់អ្នក ដើម្បីទទួលបានរង្វាន់បន្ថែម!',
  'Your address': 'អាសយដ្ឋានរបស់អ្នក',
  'Address book': 'សៀវភៅអាសយដ្ឋាន',
  'Save': 'រក្សាទុក',
  'FAQs & guides': 'សំណួរដែលសួរញឹកញាប់ និងការណែនាំ',
  'Rate this app': 'វាយតម្លៃកម្មវិធីនេះ',
  'Recommend this app': 'ណែនាំកម្មវិធីនេះ',
  'Clear cache': 'សម្អាតទិន្នន័យបណ្តោះអាសន្ន',
  'LOG OUT': 'ចាកចេញ',
  'Account deletion': 'ការលុបគណនី',
  'Delete account': 'លុបគណនី',
  'Order Successful!': 'ការបញ្ជាទិញទទួលបានជោគជ័យ!',
  'Your order has been placed successfully.': 'ការបញ្ជាទិញរបស់អ្នកត្រូវបានដាក់ដោយជោគជ័យ។',
  'Order ID': 'លេខសម្គាល់ការបញ្ជាទិញ',
  'Back to Home': 'ត្រឡប់ទៅទំព័រដើម',
  'View My Orders': 'មើលការបញ្ជាទិញរបស់ខ្ញុំ',
  'Select': 'ជ្រើសរើស',
  'The email address is not valid. Please check and try again.': 'អាសយដ្ឋានអ៊ីមែលមិនត្រឹមត្រូវទេ។ សូមពិនិត្យ ហើយព្យាយាមម្តងទៀត។',
  'This user account has been disabled.': 'គណនីអ្នកប្រើប្រាស់នេះត្រូវបានបិទ។',
  'No user found with this email.': 'រកមិនឃើញអ្នកប្រើប្រាស់ដែលមានអ៊ីមែលនេះទេ។',
  'Incorrect password. Please try again.': 'ពាក្យសម្ងាត់មិនត្រឹមត្រូវទេ។ សូមព្យាយាមម្តងទៀត។',
  'This email is already registered.': 'អ៊ីមែលនេះត្រូវបានចុះឈ្មោះរួចហើយ។',
  'Login with email is currently disabled.': 'ការចូលដោយប្រើអ៊ីមែលត្រូវបានបិទនៅពេលនេះ។',
  'The password is too weak.': 'ពាក្យសម្ងាត់ខ្សោយពេក។',
  'Invalid login details.': 'ព័ត៌មានលម្អិតនៃការចូលមិនត្រឹមត្រូវ។',
  'Too many attempts. Please try again later.': 'ការព្យាយាមច្រើនដងពេកហើយ។ សូមព្យាយាមម្តងទៀតនៅពេលក្រោយ។',
  'An account already exists with the same email address but different sign-in credentials.': 'មានគណនីដែលមានអ៊ីមែលនេះរួចហើយ ប៉ុន្តែប្រើប្រាស់វិធីសាស្ត្រចូលផ្សេងគ្នា។',
  'Apple Sign-In canceled or failed.': 'ការចូលដោយប្រើ Apple ត្រូវបានបោះបង់ ឬបានបរាជ័យ។',
  'Facebook Error': 'បញ្ហា Facebook',
  'Verification failed': 'ការផ្ទៀងផ្ទាត់បានបរាជ័យ',
  'Network error. Please check your internet connection.': 'បញ្ហាបណ្តាញ។ សូមពិនិត្យមើលការភ្ជាប់អ៊ីនធឺណិតរបស់អ្នក។',
  'An error occurred. Please try again.': 'មានបញ្ហាមួយបានកើតឡើង។ សូមព្យាយាមម្តងទៀត។',
  'The verification code is invalid. Please try again.': 'លេខកូដផ្ទៀងផ្ទាត់មិនត្រឹមត្រូវទេ។ សូមព្យាយាមម្តងទៀត។',
  'Verification ID is invalid. Please request a new code.': 'លេខសម្គាល់ការផ្ទៀងផ្ទាត់មិនត្រឹមត្រូវទេ។ សូមស្នើសុំលេខកូដថ្មី។',
  'The session has expired. Please try again.': 'វគ្គផ្ទៀងផ្ទាត់បានផុតកំណត់ហើយ។ សូមព្យាយាមម្តងទៀត។',
  'SMS quota exceeded. Please try again later.': 'កូតាសារ SMS ត្រូវបានប្រើប្រាស់អស់ហើយ។ សូមព្យាយាមម្តងទៀតនៅពេលក្រោយ។',
  'You do not have permission to perform this action.': 'អ្នកមិនមានការអនុញ្ញាតឱ្យធ្វើសកម្មភាពនេះទេ។',
  'The service is currently unavailable. Please try again later.': 'សេវាកម្មនេះមិនអាចប្រើប្រាស់បានទេនៅពេលនេះ។ សូមព្យាយាមម្តងទៀតនៅពេលក្រោយ។',
  'The requested resource was not found.': 'រកមិនឃើញធនធានដែលបានស្នើសុំទេ។',
  'The resource already exists.': 'ធនធាននេះមានរួចហើយ។',
};
